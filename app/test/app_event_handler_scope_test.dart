...
void main() {
  ...
  group('applyDebugTreasuryCredit', () {
    ...
    test('applies treasury credit outside human orders phase', () {
      final game = Game(
        id: 'g-treasury3',
        worldState: const WorldState(
      ...
      final result = applyDebugTreasuryCredit(currentGame: game, event: event);
      expect(result.game, isNotNull);
      expect(result.game!.players.single.treasury, 60);
  ...
  group('applyDebugWorkerPoolCredit', () {
    ...
    test('worker pool mutation round-trips through game json', () {
      ...
      final applied = applyDebugWorkerPoolCredit(currentGame: game, event: event);
