...
Finder _surfacesOf(Finder labourSection) => find.descendant(
  of: labourSection,
...
void main() {
  ...
  group('ProductionLabourSection step-button surface contract '
      '(SPEC/ui/production-panel.md § Labour Controls; Refs #2862)', () {
    testWidgets('each per-tier + / − control wraps its icon in a 26x26 '
    ...
    testWidgets('enabled ± control paints buttonGradient inside 1 px '
        '--border outline at full opacity', (WidgetTester tester) async {
      // Peasant + with fabric ≥ 2 → recruit cost row affordable → enabled.
      final player = _gpWithPool(stockpile: {CommodityCatalog.fabric.id: 2});
      await tester.pumpWidget(_mount(player: player));
      await pumpSettleCapped(tester);
      ...
      final Opacity opacityWidget = tester.widget<Opacity>(opacityFinder.first);
      expect(opacityWidget.opacity, equals(1.0));
    });

    testWidgets('disabled ± control fades the entire surface to '
      ...
      final plusControl = find.byKey(
        const ValueKey<String>('production_labour_plus_peasants'),
      );
      expect(plusControl, findsOneWidget);
    ...
    testWidgets('Disband retains CtNinePatchButton chrome (NOT '
