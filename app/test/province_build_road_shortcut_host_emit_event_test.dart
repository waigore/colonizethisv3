// Pins host-level Build road shortcut-assignment tap flow (Refs #4260).
...
const _kGameId = 'g_br_shortcut_emit';
...
void main() {
  ...
  setUpAll(() async {
    gamesBox = await openAppTestHiveBox(suiteId: 'province_br_shortcut_emit');
  });
  ...
  Future<void> expectEmits(WidgetTester tester, List<OpenCivilianUnitsPanelEvent> opened) async {
    final shortcut = provinceShortcutHostIconAction(Icons.add_road);
    ...
    expect(event.buildRoadShortcutTargetTileKey, _kTile);
  ...
  for (final host in provinceShortcutHostCases) {
    testWidgets(
      '${host.wide ? 'wide' : 'narrow'} host: Build road shortcut emits Engineer-only event',
      (tester) async {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await expectProvinceShortcutHostIconNegative(
          tester,
          enabledAction: provinceShortcutHostIconAction(Icons.add_road),
          anyAction: provinceShortcutHostIconAction(Icons.add_road, enabledOnly: false),
          opened: opened,
          wide: host.wide,
          wideDisabledReason:
              'Without an assignable Engineer the Build road inline action must not be enabled.',
        );
      },
    );
  }
