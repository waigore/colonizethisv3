...
void main() {
  ...
  // Refs #3656: the event-feed assertions are driven by AppEventBus events and
  // read only player/port world-state (display names, portsByProvinceSeaboard
  // for the naval anchor) — never generated map cells/topology. A lightweight
  // game (human + AI opponent, one old-world unit, a port/sea-zone entry) and a
  // minimal mapViewData replace the ~7-11s getDebugInitGameResult() map
  // generation that just let the canvas mount.
  final Game game = buildMapAreaChromeTestGame();
