...
const String _kGameId = 'g_brr_golden';
...
class _GameServiceBuildRailroadGolden extends GameService {
  _GameServiceBuildRailroadGolden(super.box, super.adapter);
...
Game goldenBuildRailroadGame() {
  return Game(
    ...
    worldState: WorldState(
      ...
      tileState: TileMapState().setRoadLevel(_kTileKey, 1),
...
RegionMapViewData goldenBuildRailroadRegion() {
...
PerPlayerWorkTargetSelectionCache _buildSelectionCache({
  ...
  cache.refresh(
    WorkTargetSelectionSnapshot(
      ...
      tileMapByRegion: _GameServiceBuildRailroadGolden._tileMapByRegion,
    ),
  );
  return cache;
}
...
void main() {
  ...
  setUpAll(() async {
    Hive.init('./.dart_tool/test_hive_province_brr_golden');
    gamesBox = await Hive.openBox<dynamic>(HiveBoxNames.games);
  });

  Future<void> pumpWideHost(WidgetTester tester) async {
    final game = goldenBuildRailroadGame();
    final region = goldenBuildRailroadRegion();
    ...
    const boundaryKey = ValueKey('province_brr_shortcut_wide_golden');
      ...
      wrapGoldenBoundary(
        ...
        overrides: [
          ...
          gameServiceProvider.overrideWith(
            (ref) =>
                _GameServiceBuildRailroadGolden(gamesBox, GameSaveAdapter()),
          ),
  ...
  Future<void> pumpNarrowHost(WidgetTester tester) async {
    ...
    const boundaryKey = ValueKey('province_brr_shortcut_narrow_golden');
  ...
  testWidgets(
    'golden: wide province side panel shows enabled Build railroad shortcut '
    '(Refs #4383)',
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final buildRailroadShortcut = find.byWidgetPredicate(
        (Widget w) =>
            w is CtIconAction &&
            w.onPressed != null &&
            w.icon == Icons.directions_railway,
      );
      expect(buildRailroadShortcut, findsOneWidget);
    },
  );
}
