...
/// Profile/release open-to-interactive measurement for GAME40001 (Refs #4688).
///
/// **Linux desktop binding host:**
/// `cd app && xvfb-run -a flutter drive --driver=test_driver/integration_test.dart \
///   --target=integration_test/technology_panel_surface_open_profile_test.dart \
///   --profile -d linux`
///
...
///
/// Attach `ui_surface_open surface=technology … host=linux_desktop_profile` or
...
void main() {
  ...
  setUpAll(() async {
    gamesBox = await openAppTestHiveBox(suiteId: 'technology_profile_surface_open');
  });
  ...
  testWidgets(
    'GAME40001 interactiveReady within 1s on profile/release binding host',
    (WidgetTester tester) async {
      ...
      final elapsedMs = ctAppPerfSurfaceOpenElapsedMs('technology');
      ...
      if (kProfileMode || kReleaseMode) {
        expect(
          ...
          reason:
              'GAME40001 open-to-interactive exceeded $kUiSurfaceOpenBudgetMs ms',
        );
      }
    },
  );
  ...
  testWidgets(
    'GAME40001 same-turn re-open interactiveReady within 1s on profile/release',
    (WidgetTester tester) async {
      final game = buildTechnologyPanelTestGame();
      final player = game.players.first;
      ...
      Future<int?> openOnce() async {
        ...
        await pumpSettleCapped(tester);
        expect(find.text('Slots'), findsOneWidget);
        return ctAppPerfSurfaceOpenElapsedMs('technology');
      }
