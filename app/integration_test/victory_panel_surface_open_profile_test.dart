...
/// Profile/release open-to-interactive measurement for GAME70001 (Refs #4688).
///
/// **Linux desktop binding host:**
/// `cd app && xvfb-run -a flutter drive --driver=test_driver/integration_test.dart \
///   --target=integration_test/victory_panel_surface_open_profile_test.dart \
///   --profile -d linux`
///
...
///
/// Attach `ui_surface_open surface=victory … host=linux_desktop_profile` or
...
void main() {
  ...
  setUpAll(() async {
    gamesBox = await openAppTestHiveBox(suiteId: 'victory_profile_surface_open');
  });
  ...
  testWidgets(
    'GAME70001 interactiveReady within 1s on profile/release binding host',
    (WidgetTester tester) async {
      ...
      final elapsedMs = ctAppPerfSurfaceOpenElapsedMs('victory');
      ...
      if (kProfileMode || kReleaseMode) {
        expect(
          ...
          reason:
              'GAME70001 open-to-interactive exceeded $kUiSurfaceOpenBudgetMs ms',
        );
      }
    },
  );
  ...
  testWidgets(
    'GAME70001 same-turn re-open interactiveReady within 1s on profile/release',
    (WidgetTester tester) async {
      final game = buildPanelTestGame();
      ...
      Future<int?> openOnce() async {
        ...
        await pumpSettleCapped(tester);
        expect(find.byKey(VictoryScreenKeys.standingsSectionKey), findsOneWidget);
        return ctAppPerfSurfaceOpenElapsedMs('victory');
      }
