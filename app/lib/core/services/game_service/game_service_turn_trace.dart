...
import 'game_service_turn_trace_orders.dart';

TurnResolutionResult gameServiceResolveTurnWithTrace(
  GameService service, {
  required Game game,
  List<TurnTraceAiSection>? aiTraceSections,
  required TurnResolverConfig config,
}) {
  if (!service.state.turnTraceEnabled) {
    return resolveTurnForGameWithConfig(game: game, config: config);
  }
  final session = service.state.turnTraceSessionsByGameId.putIfAbsent(
    game.id,
  ...
  final result = resolveTurnForGameWithConfig(game: game, config: tracedConfig);
  if (result is TurnResolutionComplete) {
    final exportedAiTraceSections =
        session.aiTraceSections ??
        gameServiceBuildAiTraceSections(
    ...
    service.state.turnTraceSessionsByGameId.remove(game.id);
  }
  return result;
}

void gameServiceExportTurnTrace(
  GameService service, {
