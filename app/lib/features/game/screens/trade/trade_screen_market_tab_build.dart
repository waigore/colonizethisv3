...
import 'trade_screen_market_tab_build_chrome.dart';
...
extension MarketTabContentBuild on MarketTabContent {
  Widget buildMarketTabBody(
    ...
    final TradeMarketStagingContext staging =
        TradeMarketStagingContext.forMarketBuild(
          game: game,
          playerId: playerId,
          orders: orders,
          productionInputConsumption: productionInputConsumption,
    ...
    final int stagedDistinctBidCount = stagedDistinctBidCommodityCount(
      orders,
      playerId,
    );
    ...
    final bool warningVisible = clampedRemaining == 0 && totalStagedBid > 0;

    final ({int budgetTotal, int budgetRemaining, bool warningVisible})
    ...
    final Widget header = buildMarketTabCounselAndHeader(
      l10n: l10n,
      ...
      bidBudgetHeader: bidBudgetHeader,
  ...
  static Map<CommodityId, int> _consumptionForDesiredOutput(
    ...
    final List<AssignedRecipe> assignments = assignedRecipesFromDesiredOutput(
