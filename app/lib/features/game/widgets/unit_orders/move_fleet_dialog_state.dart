// Move fleet dialog state. SPEC/ui/move-fleet-dialog.md.
//
...
class MoveFleetDialogState extends MoveUnitsDialogState<MoveFleetDialog> {
  late final List<FleetMovePick> _picks;
  FleetMovePick? _selected;
  ...
  Widget buildMoveDialogDestinations(BuildContext context) {
    final l10n = appL10n(context);
    final seaPicks = _picks.whereType<FleetPickSeaZone>().toList();
    final portPicks = _picks.whereType<FleetPickPort>().toList();
  ...
  Widget _row(FleetMovePick pick, int index) {
    ...
    if (kCtE2EEnabled && pick is FleetPickSeaZone) {
      return KeyedSubtree(
