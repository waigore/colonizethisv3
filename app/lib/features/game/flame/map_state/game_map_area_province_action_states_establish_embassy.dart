...
import 'package:colonizethis_orders/colonizethis_orders.dart'
    show embassyShortcutAppliesToMinorTribeProvince;
import 'package:colonizethis_world/colonizethis_world.dart';

typedef ProvinceEstablishEmbassyActionState = ({
  bool showControl,
  bool enabled,
  bool pending,
  String? ownerId,
...
/// Validator-backed MAP20001 Political Embassy shortcut state (Refs #4739).
abstract final class GameMapAreaProvinceActionStatesEstablishEmbassy {
  static const ProvinceEstablishEmbassyActionState hidden = (
    showControl: false,
    enabled: false,
    pending: false,
    ownerId: null,
  ...
  static ProvinceEstablishEmbassyActionState compute({
    required Game game,
    ...
    if (!embassyShortcutAppliesToMinorTribeProvince(
      game: game,
    ...
    final alreadyPending = pending.any(
      (candidate) =>
          ...
          candidate.overtureStage == OvertureStage.embassy,
    );
    if (alreadyPending) {
      return (
        showControl: true,
    ...
    for (final candidate in enumerateDiplomaticPanelActionsForTarget(
      ...
      if (candidate.order.type == DiplomaticOrderType.establishOverture &&
          candidate.order.overtureStage == OvertureStage.embassy) {
        action = candidate;
        break;
      }
    }
