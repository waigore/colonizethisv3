...
import '../tool/check_combat_test_support_file_size.dart';
...
void main() {
  group('runCheckCombatTestSupportFileSize', () {
    ...
    test('fails when a support lib file exceeds the ceiling', () {
      ...
      _writeFile(
        root,
        'packages/colonizethis_combat_test_support/lib/src/fat.dart',
        List.generate(12, (i) => '// line $i').join('\n'),
      );

      final errors = <String>[];
      final code = runCheckCombatTestSupportFileSize(
        root.path,
        ceiling: 10,
        info: (_) {},
        err: errors.add,
    ...
    test('ignores generated files and passes under the ceiling', () {
      ...
      _writeFile(
        root,
        'packages/colonizethis_combat_test_support/lib/src/models.g.dart',
        List.generate(12, (i) => '// generated $i').join('\n'),
      );
      _writeFile(
        root,
        'packages/colonizethis_combat_test_support/lib/src/ok.dart',
        '// small\n',
      );
