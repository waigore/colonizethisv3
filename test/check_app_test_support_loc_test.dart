...
import '../tool/check_app_test_support_loc.dart';

void main() {
  group('countAppTestSupportPhysicalLoc', () {
    test('sums dart physical lines under support/', () {
      final temp = Directory.systemTemp.createTempSync('app-support-loc-');
      ...
      expect(countAppTestSupportPhysicalLoc(support), 5);
    });
  });

  group('runCheckAppTestSupportLoc', () {
    test('passes on current repo tree under ratchet ceiling', () {
      expect(runCheckAppTestSupportLoc('.'), 0);
    });

    test('fails when measured support LOC exceeds ceiling', () {
      final temp = Directory.systemTemp.createTempSync('app-support-loc-');
      addTearDown(() => temp.deleteSync(recursive: true));
      final support = Directory(p.join(temp.path, 'app', 'test', 'support'))
        ..createSync(recursive: true);
      File(
      ...
      final errors = <String>[];
      final code = runCheckAppTestSupportLoc(
        temp.path,
        ceiling: 5,
        info: (_) {},
        err: errors.add,
