...
void main() {
  group('runCheckAppNoMaterialChip', () {
    test('passes when every features file uses palette-token primitives '
...
''');
    ...
    test('passes when Chip appears inside a // comment or /// dartdoc', () {
      final temp = Directory.systemTemp.createTempSync(
...
''');
  ...
  group('bannedChipConstructionPattern (regex shape)', () {
    test('matches the unprefixed Material Chip constructor', () {
      const samples = <String>['Chip(', 'Chip (', 'Chip\t('];
    ...
    test('does NOT match sibling chip-family Material constructors '
    ...
    test('does NOT match Ct-* catalog widgets containing "Chip" as suffix', () {
      const safe = <String>[
        'CtChoiceChip(',
        'CtChip(',
        '_LegendChip(',
