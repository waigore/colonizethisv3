...
import '../tool/check_diplomacy_test_support_loc.dart';

void main() {
  group('countDiplomacyTestSupportPhysicalLoc', () {
    test('sums dart physical lines under lib/', () {
      final temp =
          Directory.systemTemp.createTempSync('diplomacy-support-loc-');
      try {
        ...
        expect(countDiplomacyTestSupportPhysicalLoc(support), 5);
      } finally {
  ...
  group('runCheckDiplomacyTestSupportLoc', () {
    test('passes on current repo tree under ratchet ceiling', () {
      expect(runCheckDiplomacyTestSupportLoc('.'), 0);
    });
    ...
    test('fails when measured support LOC exceeds ceiling', () {
      ...
      try {
        ...
        final errors = <String>[];
        final code = runCheckDiplomacyTestSupportLoc(
          temp.path,
          ceiling: 5,
          info: (_) {},
          err: errors.add,
