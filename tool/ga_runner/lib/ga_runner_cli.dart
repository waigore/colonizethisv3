...
Future<int> _startFresh({
  ...
  // Emit the resolved master seed before scheduling so the run is replayable
  // even when the seed was generated from entropy (Refs #3486).
  final seedSource = config.seedAutoGenerated ? 'entropy' : 'config';
  final seedLine = 'ga:master_seed seed=${config.seed} source=$seedSource';
  _log.i(seedLine);
  emitStdout(seedLine);
