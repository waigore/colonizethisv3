...
/// Parsed `ga-config.json`. SPEC/program/ga-runner.md. Refs #3439, #3486, #3488.
class GaConfig {
  const GaConfig({
    ...
    this.pruneObserverTraces = false,
  ...
  /// When true, the runner deletes each round's heavy `observer-traces`
  /// subtree after the game is scored. Fitness only needs the final snapshot
  /// and `run-summary.json`, both read during scoring, so pruning keeps disk
  /// usage bounded for long (high `max_turns`) runs. Defaults to `false` so
  /// traces are retained for inspection unless explicitly opted in.
  final bool pruneObserverTraces;
  ...
  factory GaConfig.fromJson(Map<String, dynamic> json) {
    ...
    final pruneObserverTraces =
        json['prune_observer_traces'] as bool? ?? false;
    return GaConfig(
  ...
  Map<String, dynamic> toJson() => <String, dynamic>{
    ...
    'prune_observer_traces': pruneObserverTraces,
  };
}
