...
/// must not construct the Material `ChoiceChip` widget (including the
/// `ChoiceChip.elevated` named constructor) as visible chrome. Single-select
/// toggle chips must resolve through `CtChoiceChip` so chip chrome stays inside
/// the editorial-monocle token surface end-to-end. The sibling
/// `repo.app_no_material_filterchip` and `repo.app_no_material_chip` rules
/// cover `FilterChip` and the un-prefixed Material `Chip`.
///
/// SPEC:
...
///    chip chrome through the catalog widget, not a raw `ChoiceChip`.
///
/// Per-line skips:
...
int runCheckAppNoMaterialChoiceChip(
  String repoRoot, {
  ...
  if (!featuresDir.existsSync()) {
    logE('check_app_no_material_choicechip: app/lib/features not found.');
    return 1;
  }
  ...
  for (final entity in featuresDir.listSync(
    ...
    final relativePath = p
        ...
        .replaceAll('\\', '/');
    if (_shouldSkipAppNoMaterialChoiceChipFile(relativePath)) {
      continue;
    }
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      final match = bannedChoiceChipConstructionPattern.firstMatch(line);
      if (match == null) {
        continue;
      }
      violations.add(
        '$relativePath:${i + 1}: ${match.group(0)!} -> use CtChoiceChip for '
        'single-select toggle chips (see SPEC/ui/pixel-art-ui-catalog.md '
        '§ Material design ban)',
      );
    }
  }
  ...
  if (violations.isEmpty) {
    logI('check_app_no_material_choicechip: no violations found.');
    return 0;
  }

  logE(
    'check_app_no_material_choicechip: found ${violations.length} '
    'violation(s) under app/lib/features/ (banned Material ChoiceChip '
...
///
/// Exposed for `test/check_app_no_material_choicechip_test.dart`.
bool shouldSkipAppNoMaterialChoiceChipFile(String relativePath) {
  return _shouldSkipAppNoMaterialChoiceChipFile(relativePath);
}

bool _shouldSkipAppNoMaterialChoiceChipFile(String relativePath) {
  ...
  if (_appNoMaterialChoiceChipAllowedFiles.contains(relativePath)) {
    return true;
  }
  for (final prefix in _appNoMaterialChoiceChipAllowedDirPrefixes) {
...
/// Banned construction syntax: `ChoiceChip(` plus the `ChoiceChip.elevated(`
...
/// that contain `ChoiceChip` as a suffix — including the catalog widget
/// `CtChoiceChip(` and any user-defined identifier ending in `ChoiceChip` —
/// because those are preceded by a word character. Theme types such as
/// `ChoiceChipTheme(` are not flagged because the `Theme` token between
/// `ChoiceChip` and the parenthesis prevents the `\s*\(` tail from matching.
///
/// Exposed for unit tests.
final RegExp bannedChoiceChipConstructionPattern = RegExp(
  r'\bChoiceChip(?:\.elevated)?\s*\(',
);

const Set<String> _appNoMaterialChoiceChipAllowedFiles = <String>{
...
const Set<String> _appNoMaterialChoiceChipAllowedDirPrefixes = <String>{
  ...
  // raw Material ChoiceChip.
  'app/lib/features/game/widgets/chrome/',
};

void main() {
  exit(runCheckAppNoMaterialChoiceChip(Directory.current.path));
}
