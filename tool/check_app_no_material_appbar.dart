...
/// must not construct the Material `AppBar` widget as visible chrome. Top
/// chrome must resolve through `CtTopBar` (embedded in `CtScreenShell`) so the
/// 36 px top bar stays inside the editorial-monocle token surface end-to-end.
/// The sibling `repo.app_no_material_scaffold` rule covers the `Scaffold`
/// host that an `AppBar` typically accompanies.
///
/// SPEC:
...
int runCheckAppNoMaterialAppBar(
  String repoRoot, {
  ...
  if (!featuresDir.existsSync()) {
    logE('check_app_no_material_appbar: app/lib/features not found.');
    return 1;
  }
  ...
  for (final entity in featuresDir.listSync(
    ...
    final relativePath = p
        ...
        .replaceAll('\\', '/');
    if (_shouldSkipAppNoMaterialAppBarFile(relativePath)) {
      continue;
    }
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      final match = bannedAppBarConstructionPattern.firstMatch(line);
      if (match == null) {
        continue;
      }
      violations.add(
        '$relativePath:${i + 1}: ${match.group(0)!} -> use CtTopBar '
        '(embedded in CtScreenShell) for screen top chrome (see '
  ...
  if (violations.isEmpty) {
    logI('check_app_no_material_appbar: no violations found.');
    return 0;
  }

  logE(
    'check_app_no_material_appbar: found ${violations.length} '
    'violation(s) under app/lib/features/ (banned Material AppBar '
    'construction; use CtTopBar / CtScreenShell):',
  );
...
bool shouldSkipAppNoMaterialAppBarFile(String relativePath) {
  return _shouldSkipAppNoMaterialAppBarFile(relativePath);
}

bool _shouldSkipAppNoMaterialAppBarFile(String relativePath) {
  ...
  if (_appNoMaterialAppBarAllowedFiles.contains(relativePath)) {
    return true;
  }
  for (final prefix in _appNoMaterialAppBarAllowedDirPrefixes) {
...
/// Banned construction syntax: bare `AppBar(`. Allows trailing whitespace
/// between the identifier and the opening parenthesis.
///
/// The leading `\b` word boundary keeps the pattern from matching identifiers
/// that contain `AppBar` as a suffix — including the separate Material
/// `SliverAppBar(` and any user-defined identifier ending in `AppBar` —
/// because those are preceded by a word character. Theme types such as
/// `AppBarTheme(` are not flagged because the `Theme` token between `AppBar`
...
void main() {
  exit(runCheckAppNoMaterialAppBar(Directory.current.path));
}
