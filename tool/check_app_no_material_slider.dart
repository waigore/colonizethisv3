...
/// must not construct the Material `Slider` widget (including the
/// `Slider.adaptive` named constructor) as visible chrome. Continuous and
/// discrete (`divisions`) value controls must resolve through `CtSlider` so
/// slider chrome stays inside the editorial-monocle token surface end-to-end.
///
/// SPEC:
/// - `SPEC/ui/pixel-art-ui-catalog.md` § Material design ban (Ct-* counterparts)
/// - `SPEC/ui/pixel-art-ui-catalog.md` § Pixel-art component catalog — CtSlider
...
///    slider chrome through the catalog widget, not a raw `Slider`.
///
/// Per-line skips:
...
int runCheckAppNoMaterialSlider(
  String repoRoot, {
  ...
  if (!featuresDir.existsSync()) {
    logE('check_app_no_material_slider: app/lib/features not found.');
    return 1;
  }
  ...
  for (final entity in featuresDir.listSync(
    ...
    final relativePath = p
        ...
        .replaceAll('\\', '/');
    if (_shouldSkipAppNoMaterialSliderFile(relativePath)) {
      continue;
    }
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      final match = bannedSliderConstructionPattern.firstMatch(line);
      if (match == null) {
        continue;
      }
      violations.add(
        '$relativePath:${i + 1}: ${match.group(0)!} -> use CtSlider for '
        'continuous and discrete value controls (see '
  ...
  if (violations.isEmpty) {
    logI('check_app_no_material_slider: no violations found.');
    return 0;
  }

  logE(
    'check_app_no_material_slider: found ${violations.length} '
    'violation(s) under app/lib/features/ (banned Material Slider '
    'construction; use CtSlider):',
  );
...
///
/// Exposed for `test/check_app_no_material_slider_test.dart`.
bool shouldSkipAppNoMaterialSliderFile(String relativePath) {
  return _shouldSkipAppNoMaterialSliderFile(relativePath);
}

bool _shouldSkipAppNoMaterialSliderFile(String relativePath) {
  ...
  if (_appNoMaterialSliderAllowedFiles.contains(relativePath)) {
    return true;
  }
  for (final prefix in _appNoMaterialSliderAllowedDirPrefixes) {
...
/// Banned construction syntax: bare `Slider(` plus the `Slider.adaptive(`
...
/// that contain `Slider` as a suffix — including the catalog widget
/// `CtSlider(`, the separate Material `RangeSlider(`, and any user-defined
/// identifier ending in `Slider` — because those are preceded by a word
/// character. Theme types such as `SliderTheme(` are not flagged because the
/// `Theme` token between `Slider` and the parenthesis prevents the `\s*\(`
/// tail from matching.
///
/// Exposed for unit tests.
final RegExp bannedSliderConstructionPattern = RegExp(
  r'\bSlider(?:\.adaptive)?\s*\(',
);

const Set<String> _appNoMaterialSliderAllowedFiles = <String>{
...
const Set<String> _appNoMaterialSliderAllowedDirPrefixes = <String>{
  // Ct-* catalog widgets implementing design-system primitives. Consumers
  // in features/** still resolve slider chrome through these widgets, not
  // raw Material Slider.
  'app/lib/features/game/widgets/chrome/',
};

void main() {
  exit(runCheckAppNoMaterialSlider(Directory.current.path));
}
