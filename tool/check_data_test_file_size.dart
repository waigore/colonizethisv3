...
// (`repo.data_test_file_size`). Refs #4121, #4412.
import 'dart:convert';
import 'dart:io';
...
const int dataTestFileSizeCeiling = 320;

const String _dataTestsRelDir = 'packages/colonizethis_data/test';

/// Empty allowlist: every data test file must stay ≤320 physical lines.
/// Override in tests via [grandfatheredPaths].
const List<String> dataTestFileSizeGrandfatheredForTests = <String>[];

int runCheckDataTestFileSize(
  String repoRoot, {
  Iterable<String>? targetFiles,
  Iterable<String>? grandfatheredPaths,
  void Function(String line)? info,
  void Function(String line)? err,
  int ceiling = dataTestFileSizeCeiling,
}) {
  final logI = info ?? stdout.writeln;
  final logE = err ?? stderr.writeln;

  final testsDir = Directory(p.join(repoRoot, _dataTestsRelDir));
  if (!testsDir.existsSync()) {
    logE('check_data_test_file_size: $_dataTestsRelDir not found.');
    return 1;
  }

  final grandfathered =
      (grandfatheredPaths ?? dataTestFileSizeGrandfatheredForTests)
  ...
  if (missingGrandfathered.isNotEmpty) {
    logE(
      'check_data_test_file_size: stale grandfather entries (file no longer '
...
int maxDataTestFilePhysicalLinesForTests() => dataTestFileSizeCeiling;

void main(List<String> args) {
  final files = repoLintStrictIncrementalFilesArgListOrExit(args);
  exit(
    runCheckDataTestFileSize(
