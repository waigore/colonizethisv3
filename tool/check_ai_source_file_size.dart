...
// (`repo.ai_source_file_size`). Refs #4079 Slice C.
//
// Complements the repository-wide 1000 NCL gate and the AI no-part gate so
// Phase-9 planning concern splits cannot silently re-merge into kitchen-sink
// modules. Ceiling matches `repo.domain_package_source_file_size` (500
// physical lines). Shrink-only grandfather allowlist fails when entries are
// missing or the named file is now under-cap (same pattern as save/data).
import 'dart:convert';
import 'dart:io';
...
const int aiSourceFileSizeCeiling = 500;

const String _aiSrcRelDir = 'packages/colonizethis_ai/lib/src';

final RegExp _generatedSuffix = RegExp(r'\.(g|freezed|mocks|gen)\.dart$');

/// Empty allowlist after #4079 Slice C splits: every AI `lib/src` file must
/// stay ≤500 physical lines. Override in tests via [grandfatheredPaths].
...
int runCheckAiSourceFileSize(
  ...
  logE(
    ...
    'under $_aiSrcRelDir (ceiling $ceiling; Refs #4079):',
  );
