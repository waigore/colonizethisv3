...
/// must not construct the Material `Card` widget (including the `Card.filled`
/// and `Card.outlined` named constructors) as visible chrome. In-screen
/// sections must resolve through `CtPanel` (the horizontally-banded
/// editorial-monocle section surface) so panel chrome stays inside the
/// palette-token surface end-to-end.
///
/// SPEC:
/// - `SPEC/ui/pixel-art-ui-catalog.md` § Material design ban (Ct-* counterparts)
/// - `SPEC/ui/pixel-art-ui-catalog.md` § Pixel-art component catalog — CtPanel
...
int runCheckAppNoMaterialCard(
  ...
  if (!featuresDir.existsSync()) {
    logE('check_app_no_material_card: app/lib/features not found.');
    return 1;
  }
  ...
  for (final entity in featuresDir.listSync(
    ...
    final relativePath = p
        ...
        .replaceAll('\\', '/');
    if (_shouldSkipAppNoMaterialCardFile(relativePath)) {
      continue;
    }
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      final match = bannedCardConstructionPattern.firstMatch(line);
      if (match == null) {
        continue;
      }
      violations.add(
        '$relativePath:${i + 1}: ${match.group(0)!} -> use CtPanel for '
        'in-screen sections (horizontally-banded editorial-monocle surface; '
        'see SPEC/ui/pixel-art-ui-catalog.md § Material design ban)',
      );
    }
  }

  if (violations.isEmpty) {
    logI('check_app_no_material_card: no violations found.');
    return 0;
  }

  logE(
    'check_app_no_material_card: found ${violations.length} '
    'violation(s) under app/lib/features/ (banned Material Card '
    'construction; use CtPanel):',
  );
...
bool shouldSkipAppNoMaterialCardFile(String relativePath) {
  return _shouldSkipAppNoMaterialCardFile(relativePath);
}

bool _shouldSkipAppNoMaterialCardFile(String relativePath) {
  ...
  if (_appNoMaterialCardAllowedFiles.contains(relativePath)) {
    return true;
  }
  for (final prefix in _appNoMaterialCardAllowedDirPrefixes) {
...
/// Banned construction syntax: bare `Card(` plus the `Card.filled(` and
/// `Card.outlined(` named constructors. Allows trailing whitespace between the
...
/// that contain `Card` as a suffix (e.g. a user-defined `ResourceCard`) because
/// those are preceded by a word character. Theme types such as `CardTheme(` are
/// not flagged because the `Theme` token between `Card` and the parenthesis
/// prevents the `\s*\(` tail from matching.
///
/// Exposed for unit tests.
final RegExp bannedCardConstructionPattern = RegExp(
  r'\bCard(?:\.(?:filled|outlined))?\s*\(',
);

const Set<String> _appNoMaterialCardAllowedFiles = <String>{
...
void main() {
  exit(runCheckAppNoMaterialCard(Directory.current.path));
}
