...
int runCheckDiplomacyLibFileSize(
  ...
  final grandfathered =
      (grandfatheredPaths ?? diplomacyLibFileSizeGrandfathered)
  ...
  if (missingGrandfathered.isNotEmpty) {
    logE(
      'check_diplomacy_lib_file_size: stale grandfather entries (file no longer '
...
List<String> _collectFilesToCheck(
  ...
  const prefix = '$_diplomacyLibRelativePath/';
...
int maxDiplomacyLibFilePhysicalLinesForTests() => diplomacyLibFileSizeCeiling;

void main(List<String> args) {
  final files = repoLintStrictIncrementalFilesArgListOrExit(args);
  exit(
    runCheckDiplomacyLibFileSize(
