...
/// must not construct the Material `SwitchListTile` widget (or its `.adaptive`
/// named constructor) as visible chrome. The combined label + switch row
/// composition is reproduced by laying out `CtSectionLabel` (or body text in
/// the active theme `TextTheme` slot) alongside `CtToggleSwitch`, so toggle
/// chrome stays inside the editorial-monocle token surface end-to-end and
/// avoids the Material `ListTile` row chrome banned alongside `SwitchListTile`.
///
/// SPEC:
...
///    `repo.app_no_material_scaffold`, and
///    `repo.app_no_material_filterchip` rules so the Material-widget ban
...
///    `features/**` must still resolve toggle row chrome through the
///    catalog widgets (`CtSectionLabel` + `CtToggleSwitch`), not a raw
///    `SwitchListTile`.
///
/// Per-line skips:
...
int runCheckAppNoMaterialSwitchListTile(
  String repoRoot, {
  ...
  if (!featuresDir.existsSync()) {
    logE('check_app_no_material_switchlisttile: app/lib/features not found.');
    return 1;
  }
  ...
  for (final entity in featuresDir.listSync(
    ...
    final relativePath = p
        ...
        .replaceAll('\\', '/');
    if (_shouldSkipAppNoMaterialSwitchListTileFile(relativePath)) {
      continue;
    }
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      final match = bannedSwitchListTileConstructionPattern.firstMatch(line);
      if (match == null) {
        continue;
      }
      violations.add(
        '$relativePath:${i + 1}: ${match.group(0)!} -> compose '
        'CtSectionLabel + CtToggleSwitch (see '
        'SPEC/ui/pixel-art-ui-catalog.md § Pixel-art component catalog) '
        'to reproduce the combined label + switch row without Material '
        'ListTile chrome',
      );
    }
  }
  ...
  if (violations.isEmpty) {
    logI('check_app_no_material_switchlisttile: no violations found.');
    return 0;
  }

  logE(
    'check_app_no_material_switchlisttile: found ${violations.length} '
    'violation(s) under app/lib/features/ (banned Material SwitchListTile '
    'construction; compose CtSectionLabel + CtToggleSwitch):',
  );
...
///
/// Exposed for `test/check_app_no_material_switchlisttile_test.dart`.
bool shouldSkipAppNoMaterialSwitchListTileFile(String relativePath) {
  return _shouldSkipAppNoMaterialSwitchListTileFile(relativePath);
}

bool _shouldSkipAppNoMaterialSwitchListTileFile(String relativePath) {
  ...
  if (_appNoMaterialSwitchListTileAllowedFiles.contains(relativePath)) {
    return true;
  }
  for (final prefix in _appNoMaterialSwitchListTileAllowedDirPrefixes) {
...
/// Banned construction syntax: `SwitchListTile(` and
/// `SwitchListTile.adaptive(`. Allows trailing whitespace between the
/// identifier (or named constructor) and the opening parenthesis, as Dart's
/// formatter often inserts a space or newline before nested argument lists.
///
/// The leading `\b` word boundary keeps the pattern from matching
/// identifiers that contain `SwitchListTile` as a suffix (e.g.
/// `MySwitchListTile`).
///
/// Exposed for unit tests.
final RegExp bannedSwitchListTileConstructionPattern = RegExp(
  r'\bSwitchListTile(?:\.adaptive)?\s*\(',
);

const Set<String> _appNoMaterialSwitchListTileAllowedFiles = <String>{
...
const Set<String> _appNoMaterialSwitchListTileAllowedDirPrefixes = <String>{
  ...
  // in features/** still resolve toggle row chrome through these widgets,
  // not raw Material SwitchListTile.
  'app/lib/features/game/widgets/chrome/',
};

void main() {
  exit(runCheckAppNoMaterialSwitchListTile(Directory.current.path));
}
