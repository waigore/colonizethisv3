...
int runCheckDebugConsoleLibFileSize(
  ...
  int ceiling = debugConsoleLibFileSizeCeiling,
}) {
  final logI = info ?? stdout.writeln;
  final logE = err ?? stderr.writeln;

  final libDir = Directory(p.join(repoRoot, _debugConsoleLibRelDir));
  if (!libDir.existsSync()) {
    logE(
      'check_debug_console_lib_file_size: $_debugConsoleLibRelDir not found.',
    );
    return 1;
  }

  final grandfathered =
      (grandfatheredPaths ?? debugConsoleLibFileSizeGrandfatheredForTests)
  ...
  if (missingGrandfathered.isNotEmpty) {
    logE(
      'check_debug_console_lib_file_size: stale grandfather entries (file no '
...
List<String> _collectFilesToCheck(
  ...
  const prefix = '$_debugConsoleLibRelDir/';
...
int maxDebugConsoleLibFilePhysicalLinesForTests() =>
    debugConsoleLibFileSizeCeiling;

void main(List<String> args) {
  final files = repoLintStrictIncrementalFilesArgListOrExit(args);
  exit(
    runCheckDebugConsoleLibFileSize(
