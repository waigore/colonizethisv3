...
///    dropdown chrome through the catalog widget, not a raw `DropdownButton`.
///
/// Per-line skips:
...
int runCheckAppNoMaterialDropdownButton(
  String repoRoot, {
  ...
  if (!featuresDir.existsSync()) {
    logE('check_app_no_material_dropdownbutton: app/lib/features not found.');
    return 1;
  }
  ...
  for (final entity in featuresDir.listSync(
    ...
    final relativePath = p
        ...
        .replaceAll('\\', '/');
    if (_shouldSkipAppNoMaterialDropdownButtonFile(relativePath)) {
      continue;
    }
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      final match = bannedDropdownButtonConstructionPattern.firstMatch(line);
      if (match == null) {
        continue;
      }
      violations.add(
        '$relativePath:${i + 1}: ${match.group(0)!} -> use CtDropdown '
        '(nine-patch trigger + modal picker) (see '
  ...
  if (violations.isEmpty) {
    logI('check_app_no_material_dropdownbutton: no violations found.');
    return 0;
  }

  logE(
    'check_app_no_material_dropdownbutton: found ${violations.length} '
    'violation(s) under app/lib/features/ (banned Material DropdownButton '
    'construction; use CtDropdown):',
  );
...
///
/// Exposed for `test/check_app_no_material_dropdownbutton_test.dart`.
bool shouldSkipAppNoMaterialDropdownButtonFile(String relativePath) {
  return _shouldSkipAppNoMaterialDropdownButtonFile(relativePath);
}

bool _shouldSkipAppNoMaterialDropdownButtonFile(String relativePath) {
  ...
  if (_appNoMaterialDropdownButtonAllowedFiles.contains(relativePath)) {
    return true;
  }
  for (final prefix in _appNoMaterialDropdownButtonAllowedDirPrefixes) {
...
/// Banned construction syntax: `DropdownButton(` and the generic-typed form
/// `DropdownButton<T>(`. Allows an optional `<...>` type-argument segment and
/// trailing whitespace before the opening parenthesis.
///
/// The leading `\b` word boundary plus the requirement that the optional
/// generic segment and `\s*\(` immediately follow keeps the pattern from
/// matching the sibling Material types `DropdownButtonFormField(` and
/// `DropdownButtonHideUnderline(` (the `FormField` / `HideUnderline` token
/// between `DropdownButton` and the parenthesis prevents both the `<...>`
/// segment and the `\s*\(` tail from matching) and any user-defined identifier
/// ending in `DropdownButton` (preceded by a word character).
///
/// Exposed for unit tests.
final RegExp bannedDropdownButtonConstructionPattern = RegExp(
  r'\bDropdownButton(?:<[^>]*>)?\s*\(',
);

const Set<String> _appNoMaterialDropdownButtonAllowedFiles = <String>{
...
const Set<String> _appNoMaterialDropdownButtonAllowedDirPrefixes = <String>{
  // Ct-* catalog widgets implementing design-system primitives. Consumers
  // in features/** still resolve dropdown chrome through these widgets, not
  // raw Material DropdownButton.
  'app/lib/features/game/widgets/chrome/',
};

void main() {
  exit(runCheckAppNoMaterialDropdownButton(Directory.current.path));
}
