...
| `manage-pr-agent` | One orderly pass over open PRs: enforce one PR per issue via `consolidate-prs`, unblock PRs stalled > 60 min via `fix-pr`, and throttle CI to at most 2 PRs with running workflows by pausing excess via `[skip ci]` empty commits plus cancelling their in-flight runs. Never waits for CI/merges. |
