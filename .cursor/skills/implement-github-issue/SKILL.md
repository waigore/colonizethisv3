...
- **Game-app UI surface budget (standing):** Work in `colonizethis_app` panels/dialogs/overlays must meet the hard 1 s **full-load** open budget and create-on-show / dispose-on-hide (`colonizethis-ui-surface-budget.mdc`, `SPEC/program/ui-surface-budget.md`) even when the issue does not mention performance. Do not add unused dialogs, widgets, or `FlameGame`s.
...
| **Dark theme / palette fidelity** (UI issues only) | Require a reference palette/spec section: the canonical OKLCH token table in `SPEC/ui/pixel-art-ui-catalog.md` § Editorial-monocle palette (preferred and always-on) **or** a per-screen mockup under `SPEC/ui/mockups/` if one is present. The mockups directory is optional; the catalog palette is the always-available source of truth. If the catalog palette is missing or stale, stop and request that the canonical palette be added/refreshed first. |
