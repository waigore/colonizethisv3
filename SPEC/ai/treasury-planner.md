...
1. `lockRecoveryLiquidityBuyerIds(game)` returns one rotating affluent GP per turn when any GP has `player.treasury >= treasuryAffluenceThreshold()` (same band as F10 speculative bidding). When **no** GP meets that band but at least one GP is broke, F15 returns up to `kLockRecoveryRichestBuyerPoolSize` (`2`) Great Powers with the highest **current** treasury among eligible factions (ties broken by ascending faction id) **each turn** — all pool members bid concurrently; affluent path still rotates one buyer via `turnNumber % affluent.length`. `lockRecoveryDesignatedBuyerId(game)` is the first pool entry (diagnostics / legacy single-buyer callers). Returns the empty list when **no** GP is broke (steady-state F1–F5 / F10 path only).
2. Each liquidity buyer (when non-empty) adds a synthetic `need` entry for the lock-recovery food commodity (highest prior-turn `MarketActivity.totalOfferQuantity` among food ids; default alphabetical first food when activity is empty) with quantity `floor(treasuryBudgetForBids / pricesByCommodityId[liquidity])` where `treasuryBudgetForBids` is the same budget passed to `TradeOrderSuggester` (Refs #3122 — current treasury minus pending phase-pre-13 costs minus carry-forward bid notional, floored at `0`). The F10 `kSpeculativeBidStockpileTarget` ceiling does **not** apply to this synthetic bid (Refs #2924 F14). The buyer never commits more treasury than it currently holds. Other commodities are stripped from `need` for that GP so the single `bidTypeCap` slot cannot be spent on fabric/bronze deficits; F10 speculative bidding is suppressed for the designated buyer that turn.
3. Each liquidity buyer **removes** that commodity from `available` so it does not offer and bid the same commodity (validator mutual-exclusion rule 3).
4. All broke GPs keep urgent offers on their surplus food; liquidity buyers' bids at priority `2` match those offers in the same tier. When a liquidity buyer is affluent its own `offerPriority` is moderate, but `forceBidPriority` overrides the emitted bid to `kTreasuryOfferPriorityUrgent` so tier alignment holds (Refs #2924 F12).
5. Broke GPs who are **not** liquidity buyers **do not** emit deficit or speculative bids (their single `bidTypeCap` slot would target non-grain commodities that do not match the urgent grain offers). This suppression keys off **current** `player.treasury < cheapestRegimentBuildTreasuryCost()`, not the F8 offer-inflow forecast, so an optimistic forecast cannot resume deficit bidding while the GP is still broke (Refs #2924 F13). F1–F5 deficit bids resume once `treasuryForecast >= cheapestRegimentBuildTreasuryCost()`.
...
The lock-recovery branch activates for each liquidity buyer even when its own `treasuryForecast >= cheapestRegimentBuildTreasuryCost()`, because affluent buyers provide buy-side liquidity for other broke GPs' urgent offers.

### Richest-buyer pool when no affluent GP (Refs #2924 F15)

When every Great Power is below `treasuryAffluenceThreshold()` but at least one is broke, seed-42 diagnostics showed round-robin across the full GP list stranded buy-side liquidity on ~14-treasury buyers while richer ~55-treasury GPs never bid, keeping per-deal seller credits far below `cheapestRegimentBuildTreasuryCost()`. F15 rotates lock-recovery buying among the two highest-treasury **eligible** GPs who can afford at least one unit of the lock-recovery food commodity instead of the full sorted GP list. Eligibility excludes GPs under the geographic peer-war lock with zero NW provinces (when `topology` is supplied to `lockRecoveryLiquidityBuyerIds`) and, on topology-less unit-test paths, GPs with `oldWorldProvincesOwned < kObserverConquestMinOwProvincesPerGp` (10).

- Given at least one broke GP and **no** GP with `treasury >= treasuryAffluenceThreshold()`, when `lockRecoveryLiquidityBuyerIds` runs, then it returns the up-to-two highest-treasury qualifying faction ids (ascending id tie-break), all of which may emit lock-recovery liquidity bids that turn.
- Given the same fixture and `playerId` equals that faction id, when `runTreasuryPlanner` runs, then it emits at least one urgent `TradeOrderType.bid` for the lock-recovery food commodity and emits offers only for non-liquidity commodities.
- Given the same fixture and `playerId` is broke but **not** in the richest-buyer pool for that turn, when `runTreasuryPlanner` runs, then it emits offers only (no bids).

### Geographic peer-war lock sell-only (Refs #2924 F15)

When `expandIsGeographicPeerWarLockNoNwTreasuryRecovery(game, snapshot)` is true for the active player with `treasury < cheapestRegimentBuildTreasuryCost()`, that GP emits offers only for the turn (no bids). The same predicate excludes the GP from `lockRecoveryLiquidityBuyerIds` when `topology` is supplied (per-GP `buildPlayerView` + `AIWorldSnapshot.fromPlayerView`). When `snapshot` is omitted but `topology` is present, `runTreasuryPlanner` derives the snapshot for the active player only. This keeps seed-42 gp3–gp6 from buying grain back after selling it under the EXPAND geographic peer-war lock; gp1/gp2 remain eligible as richest-buyer pool members.

- Given the geographic peer-war lock with zero NW provinces and `treasury < cheapestRegimentBuildTreasuryCost()`, when `runTreasuryPlanner` runs with that snapshot, then it emits offers only (no bids) even when `playerId` would otherwise be in the richest-buyer pool.
...
- Given at least one broke GP but `playerId != lockRecoveryDesignatedBuyerId(game)` and `player.treasury < cheapestRegimentBuildTreasuryCost()`, when `runTreasuryPlanner` runs, then it emits offers only (no bids).
