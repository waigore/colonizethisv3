...
**EXPAND regiment-rebuild trap:** A GP below the OW quota that is at peace with all other Great Powers, with non-empty `invadableProvinceIdsSorted`, and with positive but small standing regiment count (fewer than `kBelowQuotaPeaceMinRegimentsBeforeDeclareWar`) is forced to rebuild regiments via the economy build pass (build threshold reduced to zero so any economy domain weight will pick a regiment candidate when treasury allows). This breaks the seed-42 turn-100 trap where a GP that has exited an early war with low regiments and zero treasury but is no longer "broke" (`regimentCount > 0`) would otherwise sit idle at peace because the existing `needRegimentsToExpand` / `brokeBelowQuotaAtPeace` triggers both require `regimentCount == 0` (observer seed-42 gp3 turn-100 trap; Refs #2509). Follow-up when treasury remains zero: economy production priority or mutual-plateau declare unlock (deferred — stronger-peer waiver regressed seed-42).
