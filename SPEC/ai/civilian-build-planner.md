...
- **Slice 4 (live economy wiring — this slice):** `kCivilianBuildPlannerEnabled` flag + `PlannerContext.civilianBuildPlannerEnabled`; the economy build pass passes `includeCivilianBuilds` and a `CivilianBuildScoringInput` (owned counts / phase name / Spy demand) into `pickBuildOrder` when enabled, and is byte-identical to the prior path when disabled; ACWire1, ACWire2, ACWire3.
- **Deferred:** smooth `[0,1]` phase ramping (hysteresis), paper ledger (AC7), research bias (AC6), and a dedicated tech-steal posture signal (Spy demand currently uses war posture only).
