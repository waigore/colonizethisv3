...
## Paper budget (deferred)

Paper is shared across research, worker training, and civilian builds: reserve research paper up to `researchPaperReserveShare`, then allocate the remainder via the recruitment-planner phase emit order against a running ledger, dropping candidates that would push remaining paper below `0`.
...
- **Slice 5 (research bias — this slice):** `runResearchPlannerWithDecision` front-loads not-yet-unlocked civilian-gating tech candidates (`kCivilianGatingTechIds` / `isCivilianGatingTech` in `ai_victory_config.dart`) within the unchanged per-turn slot target, gated by `PlannerContext.civilianBuildPlannerEnabled` (inert by default); AC6.
- **Deferred:** smooth `[0,1]` phase ramping (hysteresis), paper ledger (AC7), and a dedicated tech-steal posture signal (Spy demand currently uses war posture only).
