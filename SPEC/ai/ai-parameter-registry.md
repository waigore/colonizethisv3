...
Victory-config constants may live in topical libraries re-exported by `ai_victory_config.dart`; registry completeness is evaluated against every in-scope `const int` / `const double` `k*` across those modules (Refs #4072).
