...
The **first** point at which a dialogue event is generated is **game start**: when the player is about to begin play (after creating a new game or loading a save), the app/TUI must show a **game_start_intro** dialogue **before** any game interaction is allowed.
...
| Mode | Meaning | App (Flutter/Flame) | TUI (ctterm) |
|------|---------|---------------------|--------------|
| **modal** | Full attention; blocks view until dismissed or choice made. | Full-screen or modal dialog (e.g. CtDialogShell); Jenny dialogue view. | Dedicated screen (e.g. Pending Overtures 100019) or modal overlay. |
| **overlay** | Shown over the game (e.g. corner or centre); player can continue to look at the world. | Flame overlay (e.g. dialogue box over map); Jenny in a compact panel. | Status area or inline panel; optional dedicated screen. |
...
Implementation: [dialogue-system.md](../program/dialogue-system.md) defines the payload; [dialogue-presentation.md](../ui/dialogue-presentation.md) (app) and [ctterm.md](../tui/ctterm.md) / TUI dialogue spec (ctterm) define how each client interprets the mode.
