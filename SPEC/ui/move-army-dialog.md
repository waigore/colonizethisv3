...
| |  |     Light walls soak some of the attack… |  | |  selected-row siege gist only (#4764)
...
- Given the selected destination has `requiresDeclareWarOnConfirm == true` and the user taps `Confirm`, when the war-confirmation sub-dialog is mounted, then the sub-dialog renders inside a `CtDialogShell` with `borderColor` resolving to `EditorialMonoclePalette.danger` and `borderWidth` equal to `CtDialogShell.dangerBorderWidth` (1px), and the title text reads `moveArmy_invadeProvinceTitle` (`Declare war?`). No Material `AlertDialog` widget is rendered as part of the sub-dialog (per issue #2867 R9 and `SPEC/ui/pixel-art-ui-catalog.md` § Material design ban). When the destination owner has a qualifying third-party court, the body also includes the same Effect line(s) as the `GAME30001` Declare War confirm (Refs #4409).
- Given the `Declare war?` sub-dialog is mounted under `AppThemes.editorialMonocle` for a Spain-owned invasion whose persisted formal ally is France, when `app/test/diplomacy_declare_war_third_party_goldens_test.dart` captures the keyed `CtDialogShell`, then the UI layer matches `app/test/goldens/move_army_declare_war_named_ally.png` and the body includes the same France Effect line as GAME30001 (Refs #4409).
...
- Given full military intel and `fortLevel == 1` on the **selected** invasion destination, when the dialog renders, then the UI layer shows `moveArmy_fortWoodSiege` and a default-visible muted gist `moveArmy_fortSiegeGistWood` (light walls soak some of the attack; the defender has 1 extra gun) (`app/test/move_army_fort_siege_gist_copy_test.dart`, Refs #4764).

- Given full intel and `fortLevel == 2` (resp. `3`) on that selected destination, when the dialog renders, then the UI layer shows `moveArmy_fortSiegeGistStone` (medium / 2 guns) (resp. `moveArmy_fortSiegeGistModern`, heavy / 3 guns) and does not print soak HP or damage-reduction percents (Refs #4764).

- Given `fortLevel == 0` (**Open field**) or unknown intel (`moveArmy_defendersUnknown`), when the invasion row renders, then the UI layer omits the siege gist (Refs #4764).
...
2. **Invasion intel — full visibility (#4216):** Fixture with fully visible invasion tiles, two combat-capable defenders, stone fort (level 2), and `playerView` from `buildPlayerView` so rows show defender totals, fort/siege label, and own-army line. The invasion destination is the only row, so it starts selected and also shows the muted Stone siege gist (`moveArmy_fortSiegeGistStone`, Refs #4764).
...
Automated widget tests: `app/test/move_dialogs_specs_army_test.dart` (army pins; fleet pins in `move_dialogs_specs_fleet_test.dart`); invasion intel pins in `app/test/move_army_invasion_intel_test.dart` (#4216); siege gist pins in `app/test/move_army_fort_siege_gist_copy_test.dart` and `app/test/move_army_invasion_intel_ui_test.dart` (Refs #4764).
