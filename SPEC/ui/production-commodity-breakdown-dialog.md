...
- **Viewport-adaptive dialog width (Refs #2862 S8c / C6):** The `CtDialogShell.maxWidth` cap is selected from the surrounding `MediaQuery.size.width` viewport. When the viewport width is **greater than or equal to** `kProductionBreakdownDialogWideViewportThreshold` (`900` logical px), the dialog uses the wider cap `kProductionBreakdownDialogWideMaxWidth` (`900` logical px) and **drops the visible horizontal `Scrollbar` chrome** around the 7-column `DataTable` (`Commodity` + 5 `EconomyPreviewStockpilePhase` columns + `Total`) so the dialog does not advertise a scroll affordance the user does not need at the wide viewport. The wide path MUST tighten the `DataTable.columnSpacing` / `horizontalMargin` (e.g. `columnSpacing: 24`, `horizontalMargin: 12`). When the viewport width is **strictly less than** the threshold, the dialog falls back to the existing `720` logical px cap and the table is hosted inside a horizontal `Scrollbar` (with `thumbVisibility: true`) + `SingleChildScrollView` so every column remains reachable on narrow viewports. The wide path MUST NOT hide any phase column. The narrow path MUST NOT skip the `Scrollbar` chrome — reviewers rely on the visible scrollbar to discover off-screen columns.
- **Wide-path full-width column distribution (Refs #3509):** On the wide path (viewport width ≥ `kProductionBreakdownDialogWideViewportThreshold`), the `DataTable` MUST occupy the **full width** of the `CtDialogShell` content column — no unused horizontal band between the table's trailing edge and the content inset. The dialog measures the content-column width via a `LayoutBuilder` and pins explicit per-column widths (helper `productionBreakdownWideColumnContentWidths`): after subtracting the `DataTable` chrome (`horizontalMargin` on both outer edges + `columnSpacing` between columns), the remaining content budget is distributed so the **Commodity** column receives twice the per-numeric width and the five phase columns plus **Total** each receive an equal share (owner decision **B**). The per-numeric width is floored so all six numeric columns are exactly equal (±1 logical px); the **Commodity** column absorbs the rounding remainder and so is strictly wider than each numeric column. Because the table is sized to fit exactly, the wide path mounts the `DataTable` **without** a horizontal `SingleChildScrollView` / `Scrollbar`. Long phase labels (`softWrap: true`, e.g. `Pending build costs`, `Riches to treasury`) wrap within their equal column share inside the `headingRowHeight` (`40`); commodity names keep `maxLines: 1` + ellipsis within the wider **Commodity** column. Full-width stretching applies **only** on the wide path; the narrow path keeps intrinsic column sizing inside its horizontal scroll viewport.
...
- **Wide-path full width (Refs #3509):** Given the viewport width is ≥ `kProductionBreakdownDialogWideViewportThreshold` (`900` logical px) and the `getDebugInitGameResult()` fixture, when `ProductionCommodityBreakdownDialog` renders, then the laid-out `DataTable` width equals the full `CtDialogShell` content-column width (the dialog frame width minus the `2 × CtSpacing.l` body padding; the `DecoratedBox` frame border is painted over the child and does not consume layout space), within ±1 logical px, leaving no unused horizontal gap to the right of the table.

- **Wide-path column distribution (Refs #3509):** Given the wide-path table from the AC above, when the per-column header widths are measured (keyed `prodBreakdownHeaderCol_<index>`), then the **Commodity** column (index `0`) width is strictly greater than each individual phase / **Total** column width, and the five phase columns plus **Total** (indices `1`..`EconomyPreviewStockpilePhase.values.length + 1`) each receive equal width within ±1 logical px.

- **Wide-path no horizontal scroll (Refs #3509):** Given the wide path, when `ProductionCommodityBreakdownDialog` renders, then no `SingleChildScrollView` whose `scrollDirection == Axis.horizontal` and no `Scrollbar` ancestor wraps the `DataTable` (the table is sized to fit the content column exactly), and `WidgetTester.takeException()` returns `null` (no `RenderFlex` overflow from wrapped phase headers).
