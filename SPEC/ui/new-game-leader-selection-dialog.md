...
|--------|------|------------|-------------|
| `NewGameLeaderSelectionDialog` | `StatefulWidget` | `baseConfig` (`GameSetupConfig`), `naming` (`ResolvedNamingConfig`), `initialLeaderByGpId` (`Map<String, String>`), `blessedProfileNames` (`List<String>`), `onCancel` (`VoidCallback`), `onConfirmed` (`void Function(List<String>, Map<String, String>, int, bool, double, Map<String, String?>, AdvancedStartType)`) | Bus-registered modal (id `new_game_leader_selection`) opened by the shell New Game button via `OpenDialogEvent(newGameLeaderSelectionDialogId)`. Invokes `onConfirmed` with the chosen lineup, parameters, per-AI-slot blessed profile names, and advanced-start preset; the scope's builder then kicks off `runNewGameSetupAfterLeaderPick`. |
...
|                                                          |
|  [=O] Infinite mode                                      |  CtToggleSwitch + label
|       Skips the year-1800 calendar stop. Owning 31 or    |  --muted helper (wraps)
|       more Old World provinces still ends the campaign.  |
|       You cannot change this after Start.                |
|                                                          |
...
- Infinite mode: `CtToggleSwitch` (no Material `CheckboxListTile`) beside the label `shell_leaderDialog_infiniteModeLabel` ("Infinite mode", `EditorialMonoclePalette.fg` w600), with helper `shell_leaderDialog_infiniteModeHelper` always visible (toggle off or on): skips the year-1800 calendar stop; owning 31 or more Old World provinces still ends the campaign; the choice cannot be changed after **Start** (`EditorialMonoclePalette.muted`, indented beneath, mockup `.toggle-row` / `.toggle-hint`). The UI layer must not present "no victory condition" or "The game will continue indefinitely" as the explanation of this control. Meaning matches `GAME70001` `victory_conditionsInfiniteMode` (calendar stop skipped; province win remains).
...
- Given `DLG10001` is open (infinite-mode toggle off or on), when The Player reads the Infinite mode control, then the UI layer shows label `Infinite mode` and helper `shell_leaderDialog_infiniteModeHelper`, and does not present the strings `no victory condition` or `The game will continue indefinitely`.

- Given `DLG10001` is shown at 320–360 dp width, when the Infinite mode helper lays out, then the helper wraps with no horizontal overflow (`SPEC/ui/mobile-adaptation.md`).
