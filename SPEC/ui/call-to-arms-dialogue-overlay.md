...
Scrim is `Material(color: EditorialMonoclePalette.dialogScrim)` (canonical `--dialog-scrim` token; no `Colors.black54`). Title (`game_callToArms_title`) renders with `EditorialMonoclePalette.accent`, `FontWeight.w600`, and `letterSpacing == 0.05 * resolvedTitleFontSize` (0.05em tracks any text-scale override). Intro line (`game_callToArms_intro`) renders with `EditorialMonoclePalette.muted` color and `FontStyle.italic`. The intro copy explicitly references the **formal alliance / treaty** ("A treaty ally is at war …"), not the informal `RelationLevel.allied` relation-score band shown as **Friendly** in the diplomacy panel — Call to Arms only fires for a formal alliance, so the copy must not imply the informal high-relation band triggers mutual defence (Refs #3625). A single `CtBrassDivider` sits between the intro line and the call-row list.
