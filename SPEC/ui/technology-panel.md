...
- **Breakdown dialog:** Tapping the RP delta opens `ResearchFundingBreakdownDialog` (`CtDialogShell`, `maxWidth 360`, scrim `--dialog-scrim`) listing the base funding RP, the +20% industrial bonus row (only when it applies), the emphasised effective RP total, the treasury cost this turn, and an italic `--danger` debt-block note when the spend is blocked (sequential vs starting-treasury variants). A single `Close` `CtNinePatchButton` dismisses it without mutating orders.
