...
| Finish-time: completes next turn | Remaining RP `R > 0` and `R ≤ anticipatedRpPerTurn` | Muted **Completes next turn** line (optional year in parentheses). |
| Finish-time: N turns | Remaining RP `R` and `anticipatedRpPerTurn = A > 0` with `R > A` | Muted **Finishes in N turns**, `N = ceil(R / A)`. |
| Finish-time omitted | Funding None, debt-blocked, or sequential-blocked (`A == 0`) | No finish-time line. |
...
- `researchFinishEstimate` / `researchFinishCalendarYear` (`research_slot_finish_estimate.dart`) — remaining-RP ÷ this-turn-RP finish-time helper and campaign-cap year suppression (Refs #4511).
...
- **Slots — funding & turn preview** — desktop editable `TechnologyPanel` (non-null `onOrdersChanged`) seeded by `technologyFundingPreviewFixture` with three prerequisite-free tier-1 techs assigned to slots 0–2 at varied funding levels (Medium / High / Low) and committed progress, with ample treasury so no slot is debt-blocked. Exercises the per-slot funding toggles, the dual-segment (committed + anticipated) turn preview with the RP delta and gold row (Refs #3512), and the finish-time line (Refs #4511).
- **Slots — funding & turn preview (mobile)** — same fixture inside the shared `mobileViewport` frame (360 × 640 dp) so the funding toggles, dual-segment preview, and finish-time line can be reviewed at the mobile width per `SPEC/ui/mobile-adaptation.md` § 6 (Refs #3512, #4511).
...
- **Tree dialog — in-progress finish line** — Tree-opened seated funded tech with remaining RP and ample treasury so the dialog shows the same finish-time sentence as Slots (Refs #4511; [tech-tree-widget.md](tech-tree-widget.md)).
...
- **Finish-time line (Refs #4511):** When `anticipatedRpPerTurn > 0` and remaining RP `R = cost − committedProgress > 0`, the slot card shows a muted wrap-safe estimate (key `techSlotFinishLine_<slotIndex>`) derived from the same sequential preview: **Completes next turn** when `R ≤ anticipatedRpPerTurn`, otherwise **Finishes in N turns** with `N = ceil(R / anticipatedRpPerTurn)`. The line is an at-current-funding estimate, not a lock-in. When a `TurnTimeMapping` is available, the UI layer may append `yearAtTurn(currentTurn + N)` in parentheses; **suppress** that year when `currentTurn + N` is after the campaign-cap turn (`turnNumberForStartCalendarYear(1800)`, **201** under default `gdd01`) unless `Game.infiniteMode` is true. Funding **None**, debt-blocked, and sequential-blocked seats (`anticipatedRpPerTurn == 0`) omit the line. The RP-delta breakdown restates remaining RP and implied turns when the estimate exists. The Tree node dialog reuses the same helpers for a seated spending tech (key `techTreeFinishLine`; [tech-tree-widget.md](tech-tree-widget.md) § Description dialog).
...
- **Finish-time completes next turn (Refs #4511):** **Given** an assigned editable slot whose remaining RP `R = cost − committedProgress` is an integer `R > 0` and sequential preview `anticipatedRpPerTurn = A > 0` with `R ≤ A`, **when** the slot card renders, **then** the UI layer shows the finish-time line (key `techSlotFinishLine_<slotIndex>`) with the localized **Completes next turn** copy.

- **Finish-time N-turn ceil (Refs #4511):** **Given** remaining RP `R > A > 0`, **when** the slot card renders, **then** the UI layer shows **Finishes in N turns** where `N = ceil(R / A)`, and a larger `A` from funding or spy insight yields a smaller or equal `N` from the same sequential preview.

- **Finish-time omitted when not spending (Refs #4511):** **Given** funding **None**, or a debt-blocked or sequential-blocked slot with `anticipatedRpPerTurn == 0`, **when** the slot card renders, **then** the UI layer renders no `techSlotFinishLine_<slotIndex>` widget.

- **Finish-time year suppressed past campaign cap (Refs #4511):** **Given** a finish estimate with `turnsRemaining = N`, current turn `T`, and mapping `TurnTimeMapping.gdd01`, **when** `T + N > 201` and `Game.infiniteMode` is false, **then** the UI layer omits the calendar year from the finish-time line; **when** `T + N ≤ 201` or `infiniteMode` is true, the UI layer may append `yearAtTurn(T + N)`.

- **Finish-time restated in breakdown (Refs #4511):** **Given** a slot that shows a finish-time line, **when** the player opens the RP-delta breakdown, **then** the dialog restates remaining RP and implied turns and still lists funding / industrial / spy / treasury rows as today.

- **Finish-time mobile wrap (Refs #4511):** **Given** a 320–360 logical-px viewport, **when** a slot card shows the finish-time line, **then** the line wraps without overflow and funding toggles remain tappable.

- **Finish-time does not nag Next turn (Refs #4511):** **Given** `DLG60001` Next turn, **when** confirmation opens, **then** the dialog still does not list empty or unfunded research seats (`UXD-001` / **P1**).
