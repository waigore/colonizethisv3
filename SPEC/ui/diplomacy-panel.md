...
- **Outgoing economic diplomacy (list row only):** On the **same row**, below the relation line, when the human Great Power has **active or pending** economic diplomacy toward this faction (receiver-centric copy): **Active subsidy:** `Outgoing subsidy: {percent}% to {displayName}` when `Game.subsidyStates` has `payerId` = human GP and `targetId` = this row’s faction (percent model, 5–20 step 5 — Refs #3753 R3). **Pending grant:** `Pending grant aid: £N (resolves end of turn)` when current-turn orders include `grantAid` toward this faction. **Pending subsidy:** `Pending subsidy: {percent}% (resolves end of turn)` when current-turn orders include `setSubsidy` toward this faction (`amount` carries the subsidy percentage). Omit each line when not applicable. Do **not** duplicate this block on the Diplomacy Detail screen for current product (list row is the source of truth).
  - **Subsidy buy/sell gloss (Refs #4546):** Active and pending subsidy lines expose the same first-order price consequences as `DIPL20001` via `Tooltip` (hover / long-press) and `semanticsLabel`, using `subsidyPriceEffectSummary` from `colonizethis_diplomacy` — pay `{percent}%` more when buying from that court and receive `{percent}%` less when selling to them on fills with that court only. The compact percent line text is unchanged; hidden relation scores and formula constants are not printed.
...
- **Subsidy row tooltip (Refs #4546):** Given the human player has an active or pending subsidy toward the row’s faction at `{percent}%`, when the player inspects that subsidy line (hover / long-press or equivalent accessible text), then the UI exposes the same buy/sell meaning as `DIPL20001` via `Tooltip` / `semanticsLabel` and does not print hidden relation scores or formula constants.
...
- **GP overture + FTP default shortlist (AC-7 / Refs #4265):** Given a Great Power row at peace with no overture, when the default action cluster renders, then the validator-enabled next overture stage (typically `Consulate`) and other enabled treaty/economic actions appear on the default row; disabled stages (`Embassy`, `NAP`, `Join Empire`, `Establish FTP`, `Offer Peace`, etc.) are absent until **More actions** is expanded.
