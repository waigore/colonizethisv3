...
|-------------------|--------------|---------------|--------------|
| Diplomatic action button | Enabled per validator probe | Confirm (+ parameter dialogs) → adds/cancels draft diplomatic order | Pending shows **Cancel** on the default row; disabled non-pending actions sit under **More actions** with inline refusal copy when expanded. |
