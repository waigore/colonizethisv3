...
**Join Empire confirm — Minor absorb**, **Join Empire confirm — Tribe colony**, and **Join Empire confirm — GP absorb** render `CtConfirmDialog` with faction-kind Effect lines (absorption vs colony; Refs #4729).
...
- **Join Empire confirm Minor absorb (Refs #4729):** Given `GAME30001` **Join Empire** confirm toward a Minor Nation, when the dialog renders, then Effect states that if they accept that court is absorbed (land, armies, and fleets become the player’s; the court leaves the map); Cost remains a single £ figure; the body contains no province-count numeral and no relation scores (−50/−10).
- **Join Empire confirm Tribe colony (Refs #4729):** Given the same confirm toward a Tribe, when the dialog renders, then Effect states that if they accept they stay on the map under their own ownership as the player’s colony, their land does not become the player’s, and that land does not count toward the 31 Old World provinces; Cost remains a single £ figure; the body does not say they are absorbed or that their provinces transfer.
- **Join Empire confirm GP absorb (Refs #4729):** Given the same confirm toward a nearly defeated Great Power, when the dialog renders, then Effect states absorption of remaining land, armies, and fleets and that the court leaves the map; Cost stays **No treasury charge.**
- **Join Empire confirm goldens (Refs #4729):** Given `CtConfirmDialog` titled `Join Empire` under `AppThemes.editorialMonocle` for Minor (Bavaria), Tribe (Aztec), and nearly defeated GP (Spain) at 360 dp, and for Tribe at 320 dp when Effect lines wrap, when `app/test/diplomacy_confirm_join_empire_goldens_test.dart` captures each keyed `RepaintBoundary`, then the UI layer matches the committed baselines under `app/test/goldens/diplomacy_confirm_join_empire_*.png`.
- **Join Empire confirm Widgetbook (Refs #4729):** Given Diplomacy Panel Widgetbook use cases `Join Empire confirm — Minor absorb`, `Join Empire confirm — Tribe colony`, and `Join Empire confirm — GP absorb` under `AppThemes.editorialMonocle`, when each builder pumps, then `CtConfirmDialog` shows the faction-kind Effect lines above.
