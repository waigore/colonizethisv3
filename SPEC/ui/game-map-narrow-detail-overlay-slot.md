...
- **Counter-espionage** (when enabled) → `OpenCivilianUnitsPanelEvent(spyOnly: true, counterSpyShortcutTargetTileKey: <province-level key>)`. Assign on an eligible Spy commits `counter_spy` without map pick.
