...
- **Subpanel 1 — Available (left):** Compact layout with:
  ...
  - **Section headings:** **Food**, **Raw Materials**, **Manufactured**, and **Workers** are rendered with `CtSectionLabel` (small-caps `--muted` text on a 1px `--accent-dim` bottom border) so the Available subpanel adopts the dark editorial-monocle chrome consistent with the Ct-* catalog (see `SPEC/ui/pixel-art-ui-catalog.md` § Editorial-monocle palette and `Refs #2859` S10 CtSectionLabel).
  ...
  - **Workers section:** Worker tiers are rendered as `CtResourceCell` rows (one cell per tier in the order peasant → apprentice → journeyman → master) showing the worker icon, the tier display name, and the worker count as the `quantity` value; no delta is shown because pending recruit/train queue counts are surfaced separately by the Labour Controls block below.
  - **Effective labour:** Single line under the Workers section, right-aligned, rendered with `--accent` colour, the monospace style with tabular figures, and a 1px `--accent-dim` top border so it reads as a totals row consistent with the dark-theme labour summary on Allocation.
...
- **Available (commodity cells):** Given the Available subpanel renders for the viewed player on any viewport, when the **Food**, **Raw Materials**, or **Manufactured** sections build their commodity rows, then each commodity is rendered as a `CtResourceCell` whose `iconBuilder` paints the canonical `ResourceIcon` for that commodity id (20×20 px slot), whose `name` equals the commodity display name (no trailing colon), whose `quantity` equals the player's current stockpile quantity for that id, and whose `delta` is `null` when the projected end-of-turn net change for that commodity is zero and otherwise the signed integer delta rendered as `+N` in `--success` or `-N` in `--danger` per `CtResourceCell` R10.
...
- **Available (effective labour total):** Given the Workers section finishes rendering for the viewed player, when the **Effective labour** total line builds, then the UI layer renders a single right-aligned `Text` widget whose color resolves to `EditorialMonoclePalette.accent`, whose `TextStyle` enables `FontFeature.tabularFigures()` with the monospace fallback stack `SF Mono` / `Menlo` / `monospace`, and whose container is decorated with a 1px `EditorialMonoclePalette.accentDim` top border so the total reads as a dark-theme summary row.
