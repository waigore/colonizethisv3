...
- **Net Changes:** The Available panel displays the **projected net change per commodity** for the preview sequence **Pending build costs → Extraction → Riches-to-treasury → Consumption → Production**, using the **same rules** as the live resolver for build-cost affordability and economy phases ([stockpiles-and-production.md](../game/stockpiles-and-production.md), [turn-resolution-phases.md](../program/turn-resolution-phases.md), [extraction-pipeline.md](../program/extraction-pipeline.md)). Pending build costs come from `currentOrders.buildUnitOrdersByPlayerId` and are applied in-order per player using build affordability checks (`canAffordBuild`) and deductions (`applyBuildCostDeduction`), so unaffordable queued builds do not contribute preview deltas. Land and overseas extraction are combined into one delivered amount per commodity before consumption. Inputs: current `Game` (including fleets for overseas interception ordering), `MapTopology`, `tileMapByRegion` (when omitted or empty, extraction from tiles is skipped—same as resolver), `currentOrders`, current human **desired output** map converted to production assignments for that player only. **Mid-turn stability:** The preview uses the **current** world state only and already-assigned pending orders from `currentOrders` (no lookahead for unresolved Work or Movement outcomes). Riches appear as stockpile decreases (and treasury is not shown in this panel). If the net change for a commodity is **zero**, the parentheses are **omitted** (show quantity only).
