...
**S9b candidate art (feature branch):** Candidate PNGs upscale the S9a-reverted level-1 hamlet cluster (~1.31× nearest-neighbor), extend the widest building row to the level-4 bbox width using building palette pixels, and replace full-width top/bottom grass bands with corner anchors plus sparse side grass so width/height/center offsets match level 4 within ±2 px while opaque pixel count stays **below** production level 2 (complexity monotonicity). Automatable checks live in `app/test/ct_region_map_town_icon_cache_test.dart` group `S9b candidate level-1 town icons`.
