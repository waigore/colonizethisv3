...
- **S9b candidate art (preview branch):** Nearest-neighbor upscale of the S9a-reverted level-1 hamlet cluster so its opaque bounding box matches the level-4 footprint (scale = max(L4 width / L1 width, L4 height / L1 height), centered in the L4 bbox). Preserves filled pixel-art roofs/walls from #3871; no grass perimeter, triple-copy, or outline-only regeneration. Preferred order when PixelLab is available: inpaint → Bitforge (level-2 style ref) → Pixflux; programmatic bbox-fill is the fallback.
