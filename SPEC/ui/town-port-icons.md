...
- **S9b candidate art (preview branch):** Dual-cluster layout: two nearest-neighbor copies of the S9a-reverted hamlet at **0.95×** scale, side-by-side inside the level-4 opaque bounding box; each building row is **reposition-spread** across the full L4 width (opaque count unchanged per row); sparse roof/wall **anchor pixels** on the L4 top/bottom rows extend bbox height without grass bands. Preserves filled pixel-art roofs/walls from #3871; candidate opaque count stays **below level 2** for promotion monotonicity. Preferred order when PixelLab is available: inpaint → Bitforge (level-2 style ref) → Pixflux; programmatic dual-cluster spread is the fallback.
