...
- **Given** any blocking overlay backed by `CtDialogueView` presents a Yarn node whose body is a narrative `line` immediately followed by a choice with **exactly one** trivial option (e.g. `-> Continue`, `-> I shall.`), **when** the node is presented, **then** it renders as **one** `CtDialogShell` step with **one** confirmation button labelled with the Yarn option text, the narrative message is shown **exactly once**, and a **single** tap advances the line and selects the sole option (no second message-bearing step). Refs #3628; see [ct-dialogue-view.md](ct-dialogue-view.md) § Combined line+choice presentation.
- **Given** any blocking overlay backed by `CtDialogueView` presents a Yarn node whose narrative `line` is followed by a choice with **two or more** options, **when** the choice step is shown, **then** the retained narrative line text and all Yarn option buttons render together in one `CtDialogShell` body (via the shared `CtDialogueLineChoiceBody`), with no collapse and no message-only step. Refs #3628.
