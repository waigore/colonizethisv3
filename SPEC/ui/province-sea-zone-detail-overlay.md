...
- **Material defaults forbidden:** the Naval section body MUST NOT (a) author the pending preview lines as a bare `Text(line)` with `style: null` so the rendered colour resolves through `DefaultTextStyle` fall-through, (b) fall back to the bare `Brightness.dark` Material `bodyMedium` colour (`Colors.white`) for the pending preview lines, or (c) consume `Theme.of(context).colorScheme.onSurface` for the pending preview lines. The pending preview lines MUST set `color: EditorialMonoclePalette.muted` directly. The fleet-summary line itself (`provinceOverlay_fleetSummary`) is left untouched on this slice and continues to inherit the ambient dark `bodyMedium` colour (which under `editorialMonocle` resolves to `EditorialMonoclePalette.fg`); a future slice may pin it explicitly if the contract evolves.
