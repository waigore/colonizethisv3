...
**Tile `Prospected` inline action (province context only):** The `Prospected` row includes an inline action icon with tooltip/accessibility label **`Prospect with explorer`**. The action is shown only when Tile details are visible (not `???`) and only in province context (never sea-zone). Show/enable state must derive from the same PlayerView + order-suggestion/order-validation basis as work-target selection for Explorer `prospect` (no UI-side rule duplication): visibility gate, mineral eligibility, not already prospected for the human player, and current order-engine acceptance semantics. If the human player has no Explorer units, keep the icon visible but disabled; disabled state uses grayscale styling and no tap action. Explorers with pending work still count as Explorer units for icon availability and panel filtering.
...
**Civilian:** Own units — for each unit, if a matching **`WorkOrder`** exists in `draftOrders.workOrdersByPlayerId[humanPlayerId]` (same `unitId`), show a localized **work-order target** line; otherwise show localized **unit status** (not raw enum/id). Other players — only if `foreignCivilianVisibleToPlayer` allows (tile visibility ≠ unknown; not enemy Spy); show localized status.
...
- Given a province Tile section with visible tile details, when prospect assignment is currently valid under Explorer `prospect` suggestion/validation rules, then the UI layer shows an enabled inline icon next to `Prospected` with tooltip/accessibility label `Prospect with explorer`.
...
- Given state drift invalidates prospect assignment between render and tap, when the user taps the icon, then the UI layer performs a silent no-op and the icon remains or becomes disabled on the next render.
