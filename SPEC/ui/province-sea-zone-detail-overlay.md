...
- **Work-target selection cache policy:** UI decisions for `explore`, `build_improvement`, `upgrade_town`, `build_road`, `build_port`, `build_fort`, and `build_rail` use an app-owned per-player work-target selection cache keyed by `(playerId, workTarget)` and exact `tileKey` members; overlay rebuild churn must not recompute eligibility. Cache refresh is boundary-based: initialize on active game load/start and refresh on turn-resolution completion.
...
- Given map scrolling/panning/rebuild churn with unchanged selected tile and unchanged turn snapshot, when the overlay re-renders, then `explore`/`build_improvement` icon state is derived from the app-owned work-target selection cache and does not recompute target eligibility.
- Given turn resolution advances to the next turn snapshot, when the UI refreshes overlay-related caches, then the app-owned work-target selection cache is refreshed for subsequent `explore`/`build_improvement` icon decisions.
