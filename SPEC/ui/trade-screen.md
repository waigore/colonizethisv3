...
- `TradeScreen.dealBookBidsTotalsKey` — `ValueKey<String>('tradeScreenDealBookBidsTotals')` (per-panel totals row inside the bids panel; renders `Total spent: N` where `N = Σ (deal.quantity × floor(deal.pricePerUnit))` across the player's filled bids only — carry-forwards excluded; always mounted — Refs `#2993` E6).
...
- **Given** the `TradeScreen` is mounted with a human player and `shellPlayerContextProvider.canMutateViaUi == false` (per-GP observe variant — distinct from the `showPlayerChrome == false` global-observe sentinel covered by variant `c`), **then** the widget tree still contains exactly one widget keyed `TradeScreen.marketSectionFoodKey`, exactly one widget keyed `TradeScreen.marketSectionRawMaterialsKey`, and exactly one widget keyed `TradeScreen.marketSectionManufacturedKey` under the `tradeScreenMarketTabBody` subtree — the section grouping documented in [§ Market tab — sectioned grouping (`#3093` slice)](#market-tab--sectioned-grouping-3093-slice) is read-only chrome and never depends on `canMutateViaUi`.
- **Given** the same conditions, **then** for every tradeable commodity `c` the widget tree still contains exactly one widget keyed `TradeScreen.marketRowResourceIconKey(c.id)` (a `ResourceIcon` of size `marketRowResourceIconSize`) and exactly one widget keyed `TradeScreen.marketRowPriceCoinIconKey(c.id)` (a `StrictAssetIcon` of size `marketRowPriceCoinIconSize` pointing at `marketRowPriceCoinAssetPath`) — the row icons documented in [§ Market tab — row icons (`#3093` slice)](#market-tab--row-icons-3093-slice) are decorative chrome that never depends on `canMutateViaUi`.
- **Given** the same conditions and a tradeable commodity `c` with `Player.stockpile.quantities[c.id] == N > 0` and no staged offers / industry-allocation reservations for `c`, **then** the widget tree still contains exactly one widget keyed `TradeScreen.marketRowSellableReadoutKey(c.id)` whose visible text is `(N)` — the sellable readout documented in [§ Market tab — sellable readout + offer-side clamp (`#3093` slice)](#market-tab--sellable-readout--offer-side-clamp-3093-slice) is read-only chrome that reflects the player's stockpile / reservation / staged-offer state regardless of `canMutateViaUi`.
