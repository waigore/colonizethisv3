...
- **Given** at least one leftover order on a Deal Book side, **when** that panel renders, **then** the leftover section heading is **Still open**, each leftover row is `{displayName} — {quantity}` only, and no `(priority` substring appears.
