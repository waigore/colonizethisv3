...
- **CtResourceCell:** Compact icon + name + quantity (+ optional signed delta) row used by the production panel, province overlay, and any panel that lists commodities, workers, or other identifier-keyed resources. Mirrors the `GAME20001-production-panel.html` `.resource-cell` mockup. Replaces ad-hoc `Row` builds that scatter resource layout and delta-color logic across feature files. **Key props:** `iconBuilder` (consumer-supplied widget that paints the leading 20x20 pixel icon — typically a `ResourceIcon` from `resource_icon.dart`); `name` (display label); `quantity` (`int`; rendered as a thousands-separated monospace number, color `--accent-dim`); `delta` (`int?`; sign-driven colour rules below); optional `padding`. **Pixel-art requirements:** horizontal row with 5px gap, 4px vertical / 6px horizontal cell padding; leading 20x20 icon region; flexible name column (`Expanded`, `min-width:0`) with overflow ellipsis using the dark-theme body font; trailing monospace quantity (`--accent-dim`); optional trailing monospace delta with 2px gap from quantity. **Compact typography (mockup `.resource-cell`, Refs #2862 S9 / C9–C10):** the name + quantity render at `CtResourceCell.nameFontSize` / `CtResourceCell.quantityFontSize` (10 logical px, the `clamp(9px,1.3vw,10px)` ceiling) and the delta at `CtResourceCell.deltaFontSize` (9 logical px, the `clamp(8px,1.1vw,9px)` ceiling) so canonical commodity / worker display names render in full inside the production Available 3-column / 2-column grid (ellipsis is a last-resort fallback only). The trailing quantity + optional delta are composed into a **single loose `Flexible` cluster** (one flex child whether or not a delta is shown) so the `Expanded` name column's flex share — and therefore the rendered name width — is unchanged when a delta is added; the cluster ellipsizes only as a last resort at pathologically narrow widths. Delta rendering: `delta > 0` → `+N`, color `--success`; `delta < 0` → `-N` (numeric sign), color `--danger`; `delta == 0` → `0` (no `+` prefix), color `--muted`; `delta == null` → delta region not laid out. No asset dependency beyond the consumer-supplied `iconBuilder`. **Forbidden Material counterpart:** `ListTile`, `Chip`.
