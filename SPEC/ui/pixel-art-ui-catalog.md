...
- **CtDropdown:** Compact flat select + modal list inside `CtDialogShell`. Replaces `DropdownButton`. **Visual contract — trigger sizing (per #4062 / DLG10001 mockup `.dropdown-wrapper select`):** visual min-height **34 dp**, label font **12 px** (`--fg`), padding **6 dp** vertical / **8 dp** left / **26 dp** right (chevron inset), flat `--bg-deep` surface, **1 px** border (`--border` idle; `--accent` on hover or while the picker is open). No gradient and no brass corner brackets. **Touch target:** layout height stays 34 dp; an invisible hit-area expansion (OverflowBox) guarantees ≥ `kMinTouchTargetSize` (44 dp) in the smallest dimension per [mobile-adaptation.md](mobile-adaptation.md) § 1. **Visual contract — chevron rotation (per #2859 R5d / S6):** The trigger renders a **10×10 px** `Icons.expand_more` (chevron-down) glyph coloured `--accent-dim` (no hex literals). When the picker opens, the chevron rotates 180° over **120 ms** using `Curves.easeOut`; when it closes, it rotates back over the same window. Driven by `AnimatedRotation` (`turns` `0.0` → `0.5`) keyed by `CtDropdown.kChevronAnimatedRotationKey`. **Visual contract — picker rows (per #4062):** each option is a compact flat row (~**32 dp** visual min-height), not a nine-patch button. **Visual contract — picker selected-row highlight (per #2859 R5c / S6):** each row paints a 1 dp left-edge border (`width = kCtDropdownPickerSelectedLeftEdgeWidth`). When the row's value equals the trigger `value`, the left edge is `--accent` and the outer surface is `--accent-dim`; non-selected rows use a transparent left edge and no tint. The selected row exposes `CtDropdown.kCtDropdownPickerSelectedRowKey` on the outer `DecoratedBox`. When `value` is `null`, no row is selected. All colours resolve from § *Editorial-monocle palette* tokens; no hard-coded hex literals.
- **CtScreenShell:** Full-screen pixel-art shell: dark editorial-monocle background + framed content area + 36 px `CtTopBar` top chrome. Replaces visible use of `Scaffold`/`AppBar` in user-facing screens. **Visual contract (per #2859 R4 / S5):** the scaffold paints the dark theme's `scaffoldBackgroundColor` (`--bg`); the framed body is a single outer `CtPanel` containing a leading `CtTopBar` (`CtGradients.topBarGradient` + 1 px `--accent-dim` bottom border, fixed 36 px height) followed by the consumer `child`. When `showBackButton` is `false` (default) the embedded `CtTopBar` omits the leading `CtBackButton`; when `true` the shell forwards through to `CtTopBar` so the chevron-left glyph and `Navigator.maybePop()` semantics come from the shared `CtBackButton` primitive (no Material `AppBar`, no `Icons.arrow_back`). When `showTitleBar` is `false`, the embedded `CtTopBar` (and its trailing spacing) is omitted entirely so the shell frames the `child` without any title band — used by the in-game map shell, whose own 36 px top bar is the only chrome (issue #2861 M2). **Key props:** `title` (required `String`), `child` (required `Widget`), `showBackButton` (`bool`, default `false`), `showTitleBar` (`bool`, default `true`). **Forbidden Material counterpart:** `Scaffold`+`AppBar` for user-facing screen chrome.
- **CtGradients:** Shared utility (not a widget) exposing the canonical gradients used by every Ct-* component: `.buttonGradient` (top→bottom `--surface-lite`→`--surface` for tappable button surfaces), `.actionButtonGradient` (top→bottom `--surface-lite`→`--bg-deep` for compact neutral panel-header `CtActionTextButton`, mockup `.action-btn`), `.panelGradient` (top→bottom `--surface`→`--bg` for `CtPanel` / `CtDialogShell`), `.rowGradient` (left→right `--bg`→`--surface` for list rows), `.topBarGradient` (top→bottom `--surface-lite`→`--surface` for `CtScreenShell` / `CtTopBar`). All colors resolve from § *Editorial-monocle palette* tokens (`--bg`, `--surface`, `--surface-lite`). No hard-coded hex literals.
...
- **CtResourceCell:** Compact icon + name + quantity (+ optional signed delta) row used by the production panel, province overlay, and any panel that lists commodities, workers, or other identifier-keyed resources. Mirrors the `GAME20001-production-panel.html` `.resource-cell` mockup. Replaces ad-hoc `Row` builds that scatter resource layout and delta-color logic across feature files. **Key props:** `iconBuilder` (consumer-supplied widget that paints the leading 20x20 pixel icon — typically a `ResourceIcon` from `resource_icon.dart`); `name` (display label); `quantity` (`int`; rendered as a thousands-separated monospace number, color `--accent-dim`); `delta` (`int?`; sign-driven colour rules below); optional `padding`. **Pixel-art requirements:** horizontal row with 5px gap, 4px vertical / 6px horizontal cell padding; leading 20x20 icon region; flexible name column (`Expanded`) with overflow ellipsis using the dark-theme body font; trailing monospace quantity (`--accent-dim`); optional trailing monospace delta with 2px gap from quantity. **Compact typography (mockup `.resource-cell`, Refs #2862 S9 / C9–C10 / #3999):** the name + quantity render at `CtResourceCell.nameFontSize` / `CtResourceCell.quantityFontSize` (10 logical px) and the delta at `CtResourceCell.deltaFontSize` (9 logical px). The trailing quantity + optional delta sit in a right-aligned `FittedBox` (`scaleDown`) under a `LayoutBuilder` budget so amounts stay visible (scaling instead of collapsing) at tight Available grid widths; when `delta == null` no empty delta slot is reserved after the quantity so the painted trailing cluster's right inset matches the leading icon inset (`CtSpacing.s`, Refs #3999 wide-layout inset parity). Equal-width cards with the same quantity and the same delta presence share a panel-wide quantity right-edge column (label-length invariant). Delta rendering: `delta > 0` → `+N`, color `--success`; `delta < 0` → `-N` (numeric sign), color `--danger`; `delta == 0` → `0` (no `+` prefix), color `--muted`; `delta == null` → delta glyphs omitted (slot still reserved). No asset dependency beyond the consumer-supplied `iconBuilder`. **Forbidden Material counterpart:** `ListTile`, `Chip`.
- **CtActionTextButton:** Compact **neutral** secondary text button for panel-header actions that do not warrant `CtNinePatchButton`'s nine-patch chrome. Mirrors the `GAME20001-production-panel.html` `.action-btn` mockup (lines 39–40) and is the neutral counterpart to the danger `CtDangerTextButton` (`.reset-btn` / `.disband-btn`). Used by the production Available header **Breakdown** affordance (Refs #2862 S10 / C11). **Visual contract:** background painted from `CtGradients.actionButtonGradient` (`--surface-lite` → `--bg-deep`); 1px `--border` border; Cinzel display label at 10 logical px with `0.04em` letter-spacing in `--accent-dim`, lifting to `--accent-bright` on pointer hover over 120 ms (`Curves.easeOut`); 10px horizontal / 3px vertical padding; no brass corner brackets, engraved drop-shadow, or 48 dp min-height. Disabled state wraps the control in 0.4 opacity (`CtNinePatchButton.disabledOpacity`) and suppresses pointer events via `IgnorePointer`. **Primary variant (`primary: true`; issue #3514 owner decision #5):** Compact **primary** header pill for unit-panel header actions (Train / Tile / Combine) per the mockup `.train-btn` / `.hdr-btn` primary family — gradient surface from `CtGradients.buttonGradient` (`--surface-lite` → `--surface`); 1px `--accent-dim` border lifting to `--accent` on hover; `--accent` label foreground lifting to `--accent-bright` on hover; label weight `w700`; still **no** brass corner brackets, engraved drop-shadow, or 48 dp min-height (it is a flat compact pill, not a `CtNinePatchButton`). The primary variant reads as visually distinct from the neutral secondary pill so a primary header action is distinguishable from a neutral one. **Icon variant (`icon != null`; issue #3514 owner decisions #6/#7):** When `icon` is set the button renders a compact `Icon + label` row (icon coloured to the resolved foreground), used by unit-panel **row actions** such as the civilian **Assign** pill (mockup `.u-actions button`) so Assign keeps its icon + label. **Key props:** `onPressed` (`VoidCallback?`); `label` (`String`, required); `enabled` (default `true`); `primary` (default `false`); `icon` (`IconData?`, default `null` → text-only); `iconSize` (`double`, default `14`); optional `semanticLabel`; optional `tooltip`. All colours resolve from § *Editorial-monocle palette* tokens (`--surface-lite`, `--surface`, `--bg-deep`, `--border`, `--accent-dim`, `--accent`, `--accent-bright`); no hard-coded hex literals. **Forbidden Material counterpart:** `TextButton`, `OutlinedButton` for neutral secondary text actions.
- **CtCircularLocateButton:** Compact **circular icon-only** action pill for the right-most "Locate" affordance on unit-panel rows (issue #3514 owner decision #6; mockup `.u-actions .locate-btn` — `width:22px; height:22px; border-radius:50%`). **Visual contract:** circular `BoxDecoration` of `diameter` (default `22` dp) painted from `CtGradients.actionButtonGradient` (`--surface-lite` → `--bg-deep`); 1px `--border` border lifting to `--accent-dim` on pointer hover over 120 ms (`Curves.easeOut`); centred glyph in `--accent-dim`, lifting to `--accent-bright` on hover. Disabled wraps the control in `0.4` opacity (`CtNinePatchButton.disabledOpacity`) and suppresses pointer events via `IgnorePointer`. **Key props:** `onPressed` (`VoidCallback?`); `icon` (`IconData`, required); `tooltip` (`String?`); `semanticLabel` (`String?`, defaults to `tooltip`); `enabled` (default `true`); `diameter` (`double`, default `22`); `iconSize` (`double`, default `13`). All colours resolve from § *Editorial-monocle palette* tokens; no hard-coded hex literals. **Forbidden Material counterpart:** circular `IconButton` / `FloatingActionButton` mini for row-action locate affordances.
...
| `app/lib/features/game/flame/region_map_component_render_markers_selection.dart` | Same as above. |
| `app/lib/features/game/flame/region_map_component_render_markers_settlements.dart` | Same as above. |
| `app/lib/features/game/flame/region_map_component_render_markers_units.dart` | Same as above. |
| `app/lib/features/game/flame/minimap/game_region_minimap.dart` | Same as above (minimap canvas). |
| `app/lib/features/game/flame/render/resource_icon_disc_palette.dart` | Pixel-art palette data (per-resource hue lookup table; generated by `tool/generate_resource_icon_disc_palette.dart`). |
