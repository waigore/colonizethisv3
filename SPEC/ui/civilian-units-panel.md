...
- **Shared row-action layout convention:** Civilian rows use the bordered row card chrome (see § Layout / wireframe — Row card chrome). Inside the card, the action cluster sits to the right of the detail stack; actions are ordered left-to-right and top-align to the first detail line. Narrow widths switch action button labels to icon-only via the same collapse heuristic used by [`UnitsEntityActionRow`](components/units-entity-action-row.md).
- **Dedicated locate control:** Each row includes a compact **Locate** action at the **right end** of the action cluster (mockup `.u-actions .locate-btn`), rendered as an icon-only `CtNinePatchButton` via `UnitsEntityAction(iconOnly: true, icon: Icons.my_location, label: …, tooltip: …)`. The locate control is **not** placed in the title row. Tapping it emits **`LocateMapTileEvent` only** — the panel stays open (no **`ClosePanelEvent`**), same contract as naval's per-fleet locate icon.
