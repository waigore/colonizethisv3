...
| Relocate (Spy only) | Idle Spy, no pending work | Map pick: `StartCivilianRelocateSelectionEvent` → tile pick → `CivilianMoveRequestedEvent`. Station spy shortcut (`relocateShortcutTargetTileKey`): bypass map pick and commit that tile (leave-intel warn unchanged). | Stages `MoveOrder`; dismiss of leave-intel warn leaves orders unchanged. |
...
| Spy Station spy shortcut (Refs #4439) | `spyOnly` + `relocateShortcutTargetTileKey` | Rows filtered to Spies; **Relocate** on an eligible idle Spy commits that tile without map pick. |
| Spy Counter-espionage shortcut (Refs #4528) | `spyOnly` + `counterSpyShortcutTargetTileKey` | Rows filtered to Spies; **Assign** on an eligible idle Spy commits `counter_spy` for that province-level key without map pick. |
...
| Field        | Source | Notes |
|-------------|--------|--------|
| **Status**  | `Unit.status` (+ Spy overrides) | Default: `idle` / `working` as short labels. **Spy-only** plain-language overrides (Refs #4219, **UXD-002**, #4679): foreign idle presence on Minor/Tribe → **Holding intel: {province}**; foreign idle on rival GP land → **Holding intel: {province} — may speed research**; pending/active `counter_spy` → **Counter-espionage**; owned-land idle reserve → **Reserve**; pending `MoveOrder` destination row uses pending-relocate assigned-to copy (status may show generic Idle until move resolves). |
...
- **Assign (engineer shortcut mode):** In engineer-filtered shortcut mode opened from province Tile `Build road`, pressing **Assign** on an eligible idle/no-pending Engineer bypasses the generic choose-order menu and directly commits pending `WorkOrder(target: build_road, targetTileKey: <exact selected tile key>)`. Engineer rows with pending/in-progress work keep standard row content and do not show Assign. If assignment is no longer valid at click-time (state drift), the UI performs a silent no-op and does not commit a pending work order.
- **Assign (Rail Builder shortcut mode):** In Rail-Builder-filtered shortcut mode opened from province Tile `Build railroad` (Refs #4383), pressing **Assign** on an eligible idle/no-pending Rail Builder bypasses the generic choose-order menu and directly commits pending `WorkOrder(target: build_rail, targetTileKey: <exact selected tile key>)`. Rail Builder rows with pending/in-progress work keep standard row content and do not show Assign. If assignment is no longer valid at click-time (state drift), the UI performs a silent no-op and does not commit a pending work order.
- **Assign (merchant shortcut mode):** In merchant-filtered shortcut mode opened from province Tile `Purchase land` (Refs #4274), pressing **Assign** on an eligible idle/no-pending Merchant bypasses the generic choose-order menu and directly commits pending `WorkOrder(target: purchase_land, targetTileKey: <exact selected tile key>)`. Merchant rows with pending/in-progress work keep standard row content and do not show Assign. If assignment is no longer valid at click-time (state drift), the UI performs a silent no-op and does not commit a pending work order. The UI must not enter map work-target selection mode for this shortcut.
- **Relocate (Station spy shortcut mode):** In Spy-filtered shortcut mode opened from province Civilian **Station spy** (Refs #4439), pressing **Relocate** on an eligible idle Spy (no pending work/`MoveOrder`, occupancy ok, projected tile ≠ shortcut tile) bypasses map pick and does **not** emit `StartCivilianRelocateSelectionEvent`. The UI commits `CivilianMoveRequestedEvent` for `relocateShortcutTargetTileKey`. Reuse `spyLeaveIntelWarningNeeded` and the existing leave-intel `ConfirmDialogEvent` (`map_relocate_leaveIntel_*`); dismiss leaves orders unchanged. If Relocate is no longer valid at click-time, the UI performs a silent no-op. **Research-insight gist (Refs #4679):** When `relocateShortcutTargetTileKey` is set, each Spy row shows the same muted gist as `MAP20001` Station spy for that tile (rival GP may-speed / already-insight; omit on Minor/Tribe).
- **Assign (Counter-espionage shortcut mode):** In Spy-filtered shortcut mode opened from province Civilian **Counter-espionage** (Refs #4528), pressing **Assign** on an eligible idle Spy (no pending work/`MoveOrder`) bypasses the Assign menu and map pick and does **not** emit `StartCivilianWorkTargetSelectionEvent`. The UI commits `WorkOrder(target: counter_spy, targetTileKey: counterSpyShortcutTargetTileKey)` (province-level `regionId|provinceId|0|0`). If Assign is no longer valid at click-time, the UI performs a silent no-op.
- **Assign menu Counter-espionage copy:** The Spy Assign menu item reads **Counter-espionage** (not **Counter spy**) and shows the same whole-realm gist as the overlay control; tooltip/semantics may add that one Spy is enough.
- **Explicit shortcut contract:** `OpenCivilianUnitsPanelEvent` carries explicit shortcut target fields and unit-filter flags: `prospectShortcutTargetTileKey` opens direct-assign `prospect`, `exploreShortcutTargetTileKey` opens direct-assign `explore`, `buildImprovementShortcutTargetTileKey` opens direct-assign `build_improvement` (with `builderOnly: true`), `buildRoadShortcutTargetTileKey` opens direct-assign `build_road` (with `engineerOnly: true`), `buildFortShortcutTargetTileKey` opens direct-assign `build_fort` (with `engineerOnly: true`), `buildPortShortcutTargetTileKey` opens direct-assign `build_port` (with `engineerOnly: true`), `buildRailShortcutTargetTileKey` opens direct-assign `build_rail` (with `railBuilderOnly: true`), `purchaseLandShortcutTargetTileKey` opens direct-assign `purchase_land` (with `merchantOnly: true`), `upgradeTownShortcutTargetTileKey` opens direct-assign `upgrade_town` (with `builderOnly: true`), `relocateShortcutTargetTileKey` opens Spy Relocate for that exact tile (with `spyOnly: true`), `counterSpyShortcutTargetTileKey` opens Spy Assign for `counter_spy` on that province-level key (with `spyOnly: true`). At most one shortcut target field is non-null for a given panel open request.
...
- **Given** the user taps **Relocate** on an eligible Spy row, **when** the tap is handled and no Relocate shortcut tile key is set, **then** the UI layer emits `StartCivilianRelocateSelectionEvent(unitId)` and enters map relocate selection mode (orange hover, valid-tile highlights, relocate prompt copy).
- **Given** the panel is opened from province Civilian **Station spy** (`spyOnly` + `relocateShortcutTargetTileKey`), **when** the panel renders, **then** the UI layer filters rows to Spy units.
- **Given** the panel is opened from that Station spy shortcut, **when** the user taps **Relocate** on an eligible idle Spy, **then** the UI layer does not emit `StartCivilianRelocateSelectionEvent` and commits `CivilianMoveRequestedEvent` for the exact shortcut tile key.
- **Given** Relocate from that shortcut would leave a foreign province with zero projected own Spies, **when** the user taps **Relocate**, **then** the UI layer shows the existing leave-intel confirm; dismiss leaves orders unchanged.
- **Given** Relocate from that shortcut is invalid at click-time, **when** the user taps **Relocate**, **then** the UI layer performs a silent no-op.
- **Given** the panel is opened from province Civilian **Counter-espionage** (`spyOnly` + `counterSpyShortcutTargetTileKey`), **when** the panel renders, **then** the UI layer filters rows to Spy units.
- **Given** the panel is opened from that Counter-espionage shortcut, **when** the user taps **Assign** on an eligible idle Spy, **then** the UI layer does not emit `StartCivilianWorkTargetSelectionEvent` and commits `WorkOrder(target: counter_spy)` for the province-level shortcut tile key.
- **Given** Assign from that shortcut is invalid at click-time, **when** the user taps **Assign**, **then** the UI layer performs a silent no-op.
- **Given** `UNIT10001` Assign menu for a Spy, **when** the menu renders, **then** the item reads **Counter-espionage** and shows the whole-realm gist.

- **Given** a Spy is idle in a non-owned province owned by a Minor Nation or Tribe, **when** the row renders, **then** the status line shows **Holding intel: {province display name}** without a may-speed-research clause.

- **Given** a Spy is idle in a non-owned province owned by a rival Great Power, **when** the row renders, **then** the status line shows **Holding intel: {province display name} — may speed research**.

- **Given** the panel is opened from **Station spy** with `relocateShortcutTargetTileKey` on rival GP land (no Spy in that court yet), **when** a Spy row renders, **then** the UI layer shows `spyResearchInsight_maySpeedResearchGist` below assigned-to.

- **Given** the same shortcut targets Minor/Tribe land, **when** a Spy row renders, **then** the UI layer does not show a research-speed gist.
...
- **Given** province-level orders (`explore`, `counter_spy`), **when** the user clicks a tile on the map in work-target selection mode, **then** the UI layer derives the province from the clicked tile key and submits the work order with the canonical province-level tile key (e.g. `regionId|provinceId|0|0`) per orders spec.
...
- **Upgrade town payoff stories (Refs #4747):** **Upgrade town pending pause gist** and **Upgrade town shortcut pause gist** pin the same after-this-work / **Takes 1 turn** line as `MAP20001` on a Builder pending `upgrade_town` row and an enabled `upgradeTownShortcutTargetTileKey` row.
...
- `app/test/civilian_units_panel_spy_station_shortcut_test.dart` — Station spy `spyOnly` filter, Relocate bypasses map pick, leave-intel dismiss, silent no-op (Refs #4439).
- `app/test/spy_research_insight_gist_test.dart` — Spy status rival-GP clause, Relocate shortcut gist, MAP20001 Station spy gist (Refs #4679).
- `app/test/civilian_units_panel_spy_counter_espionage_shortcut_test.dart` — Counter-espionage `spyOnly` filter, Assign bypasses map pick, silent no-op, Assign menu label + gist (Refs #4528).
