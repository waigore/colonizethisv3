...
- **`/add_money <amount>`** — `<amount>` must parse as an integer. Valid range for execution: `1..9999` inclusive. Values `>9999` are **clamped to 9999 in the parser** (single source of clamp); the parsed invocation carries both `requestedAmount` and `creditedAmount`. Values `<1` or non-integer input are rejected with deterministic errors. No ruleset or economy-phase modifiers apply; the app listener does **not** gate apply on `TurnPhase`.
- **`/add_worker <worker_tier> <amount>`** — `<worker_tier>` must be one canonical `WorkerPool` field id: `peasants`, `apprentices`, `journeymen`, or `masters` (parser accepts case-insensitive input and emits the canonical lowercase id on the bus). `<amount>` follows the same integer range and parser clamp semantics as `/add_money` (`requestedAmount` / `creditedAmount`, cap `9999`). No ruleset or economy-phase modifiers apply; the app listener does **not** gate apply on `TurnPhase`. Credits apply immediately to the active human player's industrial `Player.workerPool` for the selected tier only.
...
- **Orders-phase gate policy** — `/add_resource`, `/flip_province`, and `/reveal_province` are allowed only during human Orders phase. Outside Orders phase, the app listener rejects apply for those commands with deterministic feedback and leaves game state unchanged. **`/add_money` and `/add_worker` are not Orders-phase gated**; they apply immediately whenever the session listener runs (subject only to normal turn-resolution blocking for session commands).
...
- Given panel input `/add_worker peasants 500` with the active human player’s `workerPool.peasants` at `10`, when the player submits, then the system emits one `CreditDebugWorkerPoolEvent` with `workerTierId=peasants`, `requestedAmount=500`, `creditedAmount=500`, and after apply the human player’s `workerPool.peasants` is `510` with deterministic success feedback including the new tier count.
- Given panel input `/add_worker journeymen 12000`, when the player submits, then the system emits one `CreditDebugWorkerPoolEvent` with `requestedAmount=12000` and `creditedAmount=9999`, and success feedback states both the requested amount (`12000`) and the credited amount (`9999`) plus the resulting `journeymen` count.
- Given panel input `/add_worker MASTERS 10`, when the player submits, then the parser resolves case-insensitive input and emits canonical `workerTierId=masters`.
- Given panel input `/add_worker nobles 5` or `/add_worker peasants abc`, when the player submits, then the system emits no `CreditDebugWorkerPoolEvent` and shows deterministic parser validation feedback.
...
- Given a valid `/add_money 50` command outside human Orders phase, when the app listener applies the event, then the system credits the human treasury by `50`, persists updated game state, and shows success feedback including the new treasury balance.
- Given a valid `/add_worker apprentices 3` command outside human Orders phase, when the app listener applies the event, then the system credits the human `workerPool.apprentices` by `3`, persists updated game state, and shows success feedback including the new `apprentices` count.
...
- Given panel input `/help`, when displayed, then `/add_worker` documentation includes every supported worker tier id (`apprentices`, `journeymen`, `masters`, `peasants`) exactly once in stable sorted order.
