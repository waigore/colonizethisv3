...
- Given `/flip_province` targets a non-human capital province and the owning faction has no eligible reassignment capital in that same region under existing capital-loss rules, when the app listener applies the event, then the system rejects before ownership transfer, keeps game state unchanged, and returns deterministic feedback `Debug flip_province rejected: cannot flip target capital because owning faction has no eligible replacement capital.`.
