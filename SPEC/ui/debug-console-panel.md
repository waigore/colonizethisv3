...
- **`/flip_province <regionId|localId>`** — deterministic id retry form for ambiguity paths; resolves only prefixed province ids.
- **`/reveal_province <regionId|localId | province_display_name>`** — reveals one target province for the human player using either prefixed province id or global display-name lookup (Old World + New World).
...
- Given panel input `/flip_province oldWorld|P1`, when the player submits and parser validation succeeds, then the system emits one `FlipDebugProvinceOwnershipEvent` with `provinceId=oldWorld|P1`.
...
- Given panel input `/reveal_province oldWorld|P1`, when the player submits and parser validation succeeds, then the system emits one `RevealDebugProvinceEvent` with `target=oldWorld|P1`.
- Given panel input `/reveal_province New Bordeaux` and exactly one province in the world matches by trimmed case-insensitive display-name equality, when the command applies in human Orders phase, then the system sets all target province land tiles and directly adjacent sea-zone water tiles to `fullyVisible` for the human player and persists the updated `WorldState.playerVisibilityByTile`.
- Given panel input `/reveal_province P1`, when the player submits, then the parser rejects the token as invalid id format and shows deterministic guidance to retry with `regionId|localId`.
- Given panel input `/reveal_province New Bordeaux` and multiple provinces match globally by trimmed case-insensitive display-name equality, when apply runs, then the system rejects deterministically with candidate prefixed province ids and id-retry guidance, and does not mutate game state.
