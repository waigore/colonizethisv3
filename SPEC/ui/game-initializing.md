...
- **In scope:** Shell flow after the user confirms **Start** on the **new game setup dialog** (`new_game_leader_selection`, `NewGameLeaderSelectionDialog`): nation + leader per slot, optional fair-assignment flag, and **game / world seed** (see [game-setup.md](game-setup.md) § Shell new game dialog). Then show progress, run setup on the **main isolate** with **async yields** between coarse steps (Option A — no background isolate). The dialog includes an optional control (default off) that sets **`GameSetupConfig.enforceFairAssignment`**: when on, the pipeline runs fair-assignment connectivity repair for OW (GP+minor) and NW (tribes) per [game-setup.md](../game/game-setup.md); when off, setup uses the fast single-pass assignment path (see [game-setup-pipeline.md](../program/game-setup-pipeline.md) step 6b).
