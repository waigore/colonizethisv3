...
- `app/test/unit_panels_320dp_min_viewport_test.dart` — CivilianUnitsPanel and MilitaryUnitsPanel (Refs #2870 S10). NavalUnitsPanel is deferred to a follow-up slice because its row body depends on the bundled nine-patch image asset and a sea-zone-shaped fixture not present in the shared debug-init-game.
