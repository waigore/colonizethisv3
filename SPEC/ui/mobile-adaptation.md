...
- `app/test/dialogs_320dp_min_viewport_test.dart` — GameParametersDialog, ExitConfirmDialog, TurnNewsDialog, GameMapOptionsDialog, TurnResolutionProcessingDialog, and CombatModeChoiceDialog (Refs #2870 S8/S10). The TurnNewsDialog group pins both the empty-state path (title + muted empty-copy + trailing `Close`) and a populated digest path (three `TurnNewsLine` entries inside the `ConstrainedBox(maxHeight: 320)` `ListView` body) so the universal turn-start modal contract from [turn-news-dialog.md](turn-news-dialog.md) is exercised at the minimum viewport. The GameMapOptionsDialog group pins all three `CtToggleSwitch` rows + `Close` action under the shared [CtDialogShell](pixel-art-ui-catalog.md) chrome so the in-game map display options dialog from [empire-overview.md](empire-overview.md) § Map display options stays within the ~288 dp content column. The TurnResolutionProcessingDialog group pins the title + `CtLoadingIndicator` row body so the worker-isolate "processing turn" modal from [turn-resolution.md](../program/turn-resolution.md) stays within the ~288 dp content column without wrapping the spinner / phase text off the visible canvas. The CombatModeChoiceDialog group pins both the regular-province path (`Auto-Resolve` + `Quick Battle`) and the capital-siege path (Quick Battle only) so the combat-mode picker from [combat-mode-choice-dialog.md](combat-mode-choice-dialog.md) stays within the ~288 dp content column.
