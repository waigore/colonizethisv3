...
- **AC-11 (combined-step golden — #3628):** Given the `tribe_first_contact` Yarn node rendered under `AppThemes.editorialMonocle`, when the overlay is advanced to the `-> Continue` choice step, then a `matchesGoldenFile` baseline (`app/test/goldens/dialogue_combined_tribe_first_contact_choice.png`) captures the scout narrative and the `Continue` option rendered together in one `CtDialogShell` body — proving the combined layout that AC-10 asserts structurally.
