...
Same chrome contract as `OVL10001`: `CtFullScreenDialogueShell` + centered `CtDialogShell`, title `tribeFirstContactOverlay_title` (`First Contact`), `CtBrassDivider`, Yarn body. Scrim uses `EditorialMonoclePalette.dialogScrim`. The Yarn body is rendered by the shared `CtDialogueLineChoiceBody` ([`ct-dialogue-view.md`](ct-dialogue-view.md) § Combined line+choice presentation). Because the `tribe_first_contact` node is a single line immediately followed by a single trivial `-> Continue` option, it collapses into **one** step: the scout narrative (with the interpolated tribe and capital names) renders above a **single** `Continue` confirmation control, and one tap dismisses the herald (Refs #3628).
...
- **AC-10 (single-step collapse — #3628):** Given the `tribe_first_contact` Yarn node (a line immediately followed by `-> Continue`), when the herald is presented, then the scout narrative text — including the interpolated `{$tribeName}` and `{$capitalName}` — and a **single** `Continue` `CtNinePatchButton` render together in **one** `CtDialogShell` step (no separate line step and no option-only step), and one tap of `Continue` advances the line, selects the sole option, and invokes `widget.onDismissed` exactly once.
- **AC-11 (combined-step golden — #3628):** Given the `tribe_first_contact` Yarn node rendered under `AppThemes.editorialMonocle`, when the overlay renders the combined line+option step, then a `matchesGoldenFile` baseline (`app/test/goldens/dialogue_combined_tribe_first_contact_choice.png`) captures the scout narrative and the single `Continue` option rendered together in one `CtDialogShell` body — proving the collapsed layout that AC-10 asserts structurally.
