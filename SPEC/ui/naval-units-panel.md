...
**Home Fleet map shortcut (Refs #4343):** When `showNavalFleetMarkerFlow` selects the Home Fleet (sole fleet or after `DLG31003`), the shell emits tile-scoped **`OpenNavalUnitsPanelEvent`** with the marker’s `locationScopeKey` / `tileScopeTileKey` / Home Fleet id — it does **not** open Move or Mission dialogs.
