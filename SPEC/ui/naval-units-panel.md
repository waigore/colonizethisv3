...
**Home Fleet map shortcut (Refs #4343, #4448):** When `showNavalFleetMarkerFlow` selects a **non-empty** Home Fleet (sole fleet or after `DLG31003`), the shell runs **`showHomeFleetDetachThenSailFlow`** (Split, then `DLG30001` for the new fleet). An **empty** Home Fleet still emits tile-scoped **`OpenNavalUnitsPanelEvent`** with the marker’s `locationScopeKey` / `tileScopeTileKey` / Home Fleet id. Sea-going in-port / at-sea marker routing is unchanged.
