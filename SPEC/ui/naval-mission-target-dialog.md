...
- **Given** `DLG31002` Blockade with the owner's capital port selected under `AppThemes.editorialMonocle`, **when** `app/test/naval_mission_goldens_test.dart` captures the keyed `RepaintBoundary`, **then** the UI layer's `matchesGoldenFile` baseline `app/test/goldens/naval_mission_target_blockade_capital.png` matches the committed PNG (Refs #4516).
