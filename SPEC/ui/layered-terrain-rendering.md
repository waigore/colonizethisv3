...
- **Plains resource variants (L1)**: For **interior** plains cells only, the renderer selects standalone overlay assets with **RGBA transparency** (transparent “background” around the resource-specific art):
  ...
  - **Plantation pipeline (Refs #3961):** One shared 64×64 PixelLab Pixflux plantation base (`pytool/assets/terrain/tile_plains_plantation_base.png`; prompt below). Shipped overlays today use `pytool/recolour_plains_plantation_tiles.py` (luminance-preserving mid-tone recolour of yellow-green field pixels; buildings/soil/path/alpha unchanged). **PO sample gate (in progress):** subtler sugar_cane / cotton / spices fields must use hand-painted field gradients via `pytool/paint_plains_plantation_field_gradients.py` (3 candidates/crop under `pytool/assets/terrain/plantation_field_candidates_3961/`); tobacco stays; final SPEC mid-tones + shipped PNGs update only after PO locks a letter per crop (`--promote`).
  ...
  - **PIL field mid-tones (shipped until PO lock):** sugar_cane `(124,179,66)`; tobacco `(128,108,42)`; cotton `(214,208,178)`; spices `(196,98,42)`.
...
- Given the four shipped plantation PNGs, when compared, then building/layout alpha silhouettes match the shared plantation base and each resource’s field-highlight mid-tone matches the PIL table above (pinned by `app/test/plains_plantation_terrain_goldens_test.dart`).
- Given `paint_plains_plantation_field_gradients.py` with the shared plantation base, when the System writes candidates, then the System emits exactly three 64×64 PNGs each for sugar_cane / cotton / spices under `plantation_field_candidates_3961/`, preserves base alpha, does not emit tobacco, and leaves shipped `app/assets/images/terrain/tile_plains_*.png` unchanged unless `--promote` is invoked after PO lock (pinned by `pytool/test_paint_plains_plantation_field_gradients.py`).
- Given PO-locked letters per retune crop and `finalize_plantation_field_retune_3961.py --picks`, when the System runs without `--dry-run`, then the System promotes the chosen candidate PNGs, patches SPEC field-mask mean RGB pins and `plains_plantation_terrain_goldens_test.dart` mid-tone constants from `CANDIDATE_MEANS.json`, and does not modify tobacco art (pinned by `pytool/test_finalize_plantation_field_retune_3961.py`).
- Given an interior plains strip with `resourceId` in `{grain, meat, horses, sugarCane, tobacco, cotton, spices}` at 64px cell size, when the region map renders L1 in `terrainOnly` mode, then the UI layer’s `matchesGoldenFile` baseline `app/test/goldens/plains_plantation_terrain_strip_64.png` matches (plantation tiles read as the same class of full-cell plains resource decal as OW grain/meat/horses).
