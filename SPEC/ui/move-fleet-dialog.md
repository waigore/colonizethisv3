...
|--------|------|------------|-------------|
| `MoveFleetDialog` | `StatefulWidget` | `game` (`Game`), `topology` (`MapTopology`), `humanPlayerId` (`String`), `fleet` (`Fleet`), `bus` (`AppEventBus`) | Local `showDialog` modal opened from `NavalUnitsPanel` Move, map marker in-port branch, or `DLG31001` Sail/Move. Emits a naval move request on confirm and per-row `LocateMapTileEvent`s on locate. |
