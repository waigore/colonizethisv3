...
- Action row: two `CtNinePatchButton`s — Confirm (primary, `dangerVariant: false`) and Cancel (secondary, `dangerVariant: false`). Confirm is disabled (`onPressed: null`) until `_selected != null`; Cancel is always enabled.
