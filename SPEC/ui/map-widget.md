...
- **Visual:** Each indicator is a **filled disc** (circle inscribed in the indicator square), not a second copy of the commodity resource bitmap. Effective discs use per-resource tint from `discColorForResourceId` in `app/lib/features/game/flame/resource_icon_disc_palette.dart` (generated palette). Blocked discs use a fixed neutral gray fill. This matches program paint order language (“extraction discs”) in `SPEC/program/map-region-map-render.md`.
