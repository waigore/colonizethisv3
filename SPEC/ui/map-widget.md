...
- **Given** the only fleet at a port marker is the Home Fleet, **when** the player taps the marker, **then** the UI layer emits `OpenNavalUnitsPanelEvent` with that marker’s `locationScopeKey` / `tileScopeTileKey` / Home Fleet id and does not open `DLG30001` or `DLG31001`.
