...
| **Human fleet markers** | One icon per distinct human naval **location scope** (port or sea zone) on the region map, anchored at the port tile or **sea-zone centroid** from the tile map. Icon: `ui_icon_map_fleet.png`. **Stack badge** when multiple fleets share the scope. **Grayscale** when every fleet at the marker has a pending naval move/mission draft; **Chebyshev ≤2 terrain “reveal” halo** (painting only; no stored fog change) when any fleet there has a pending naval **move** draft. **Tap** (before civilian markers when overlapping) emits **`OpenNavalUnitsPanelEvent`** with `locationScopeKey`, optional `initialSelectedFleetId`, and `tileScopeTileKey` for panel chrome. | No separate toggle; follows map visibility and draft projection from `RegionMapViewData.fleetTileMarkers`. |
