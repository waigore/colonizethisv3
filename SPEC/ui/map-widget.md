...
1. **Sea and land terrain** (Wang coast/interior, plains variants, desert transitions, etc.).
2. **Road/rail transport sprites** when the base-layer mode includes roads and `roadLevel > 0` on **land** only: cardinal mask bits (**N=1, E=2, S=4, W=8**) from neighboring **land** cells with `roadLevel > 0`; road family for levels 1/2, rail family for level 4. Transport sits **above** bare land/plains interior passes and **below** L2 feature terrain overlays (forest/mountains on plains, etc.) so corridor art reads on open terrain while feature art remains on top.
3. **L2 feature terrain overlays** (transparent `tile_plains_*` / feature tiles where applicable).

Then **base overlays** (second group), still bottom → top:

4. **Great Power province ownership tint** (when that layer is enabled).
5. **Resource icons** (when the base-layer mode includes resources).
6. **Improvement labels** `I{n}` (when the mode includes improvements and `n > 0`).
...
Implementation: `_paintTiles` covers steps 1–3; `_paintGreatPowerLandOwnershipTint` and `_paintOverlay` cover steps 4–6 so Z-order matches this spec (see `SPEC/program/map-region-map-render.md`).
