...
- **Given** the viewing player owns a revealed land tile that is **not** in `ConnectivityResult.connected` and **Highlight land not bound to the capital** is ON, **when** `MAP10001` renders, **then** that tile receives the capital-link disconnected hatch and a connected owned neighbor without production does **not** (Refs #4370).
