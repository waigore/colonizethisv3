...
- **Placement:** Discs are anchored to the resource icon box and rendered immediately to the right in a horizontal right fan (`+x` step with overlap, default no vertical stagger).
...
- **Given** map markers, Civilian Units panel rows, and TUI civilian location rows are shown in the same turn, **when** a civilian has pending or assigned work, **then** each surface derives location from one shared projected-location rule (pending target tile, else assigned tile, else standing tile) so display and locate targets stay aligned.
