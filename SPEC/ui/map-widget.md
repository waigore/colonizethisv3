...
|-------|---------|-----------|
| **Base (tile)** | Terrain; optional resource icons; optional improvement labels (`{n} of {cap}` on the viewing player's owned revealed tiles with a player-visible resource; `{n}` only on foreign tiles and owned tiles whose resource is hidden; never `I{n}`); optional road/rail transport sprite overlay (cardinal mask `0..15` from `roadLevel` land connectivity); towns; capitals; optional **fort glyphs** offset near town tiles when `TownMarkerView.mapVisibleFortLevel` is set (Refs #4280; [town-port-icons.md](town-port-icons.md)). Town/port icon behavior (including render sizes and glyph policy) follows [town-port-icons.md](town-port-icons.md). **Player-constrained visibility:** capital markers and town/port icons on a cell are omitted when that cell’s `CellViewData.visibility` is `unrevealed` (fog parity; does not apply in full visibility mode). Fort glyphs follow the same town-cell visibility predicate and foreign military-intel gate documented in town-port-icons. | Base terrain always on; resource and label sublayers controlled by [Base layer display mode](#base-layer-display-mode). |
...
| **Capital-link disconnected land** | **Diagonal hatch** on viewing-player **owned land** tiles whose tile key is **not** in that player's `ConnectivityResult.connected` set (`CellViewData.capitalLinkDisconnected`). Sea, foreign, and unowned tiles unchanged. Connected owned land (including zero-yield) is **not** hatched. **Player-constrained visibility:** hatch is **not** drawn on `unrevealed` tiles; **`visible` and `fogged`** use the same hatch. **Global observe** (`ShellPlayerContext.panelPlayerId == null`): no hatch keys are populated / layer is inert. Extraction disc colours/counts unchanged when both layers paint (Refs #4370). Painted **after** GP tint and **before** resource icons / extraction discs. | Yes. In-game toggle **Highlight land not bound to the capital** ([empire-overview.md](empire-overview.md)); default **ON**; persisted on `MapViewState.showCapitalLinkDisconnectedHighlight`. |
...
| **Human army markers** | One stack icon per province that hosts one or more **human** armies, anchored at the province **town** tile and painted in the **bottom-right** sub-rect of that cell (`ArmyTileMarkerLayout.originFrac`) so town glyphs, fort glyphs, and civilian markers on the same cell stay hittable. Icon: province-label `ui_icon_map_presence_regiment.png`. **Stack badge** when `stackCount > 1`; `stackCount == armyIds.length` and **includes Home Army** (including zero-regiment Home Army). **Grayscale** when at least one listed **field** army exists and every listed field army has a pending `ArmyMoveOrder`. **Tap** (icon sub-rect, after work-target lock, before civilian/town/detail): emits **`OpenArmyStackMarkerEvent`** (`provinceId`, `armyIds`, `fieldArmyIds`, `tileKey`). Shell: if ≥1 field army at the tile has cached destinations, `showOverlayArmyMoveFlow` with **those field ids only** (Home Army never a `DLG20002` row or `DLG20001` target); if that stack includes a non-empty Home Army and every stationed field army has empty cached destinations, Split then `DLG20001` for the new field army; empty Home-only → `OpenMilitaryUnitsPanelEvent` / `UNIT20001`. Observe / `canMutateViaUi == false`: marker stays visible; tap snackbar + return (no dialogs, no panel). Does **not** open `MAP20001`. Payload is **not** `unitMarkers`. | No separate toggle; follows `RegionMapViewData.armyTileMarkers`. |
| **Human fleet markers** | One icon per distinct human naval **location scope** (port or sea zone) on the region map. **In port:** anchored at the **same sea drawable cell** as the harbor icon (`TownMarkerView.portIconX` / `portIconY`), computed from `portsByProvinceSeaboard` per [town-port-icons.md](town-port-icons.md) (strict sea-only placement). **At sea:** anchored at the **sea-zone centroid** from the tile map. Icon: `ui_icon_map_fleet.png`. **Stack badge** when multiple fleets share the scope. **Grayscale** when every fleet at the marker has a pending naval move/mission draft; **Chebyshev ≤2 terrain “reveal” halo** (painting only; no stored fog change) when any fleet there has a pending naval **move** draft. **Tap** (before civilian markers when overlapping) emits **`OpenNavalMissionMenuEvent`** with `locationScopeKey`, `fleetIds`, optional `initialSelectedFleetId`, and `tileScopeTileKey`; the shell runs **`showNavalFleetMarkerFlow`** (Refs #4343, #4448): optional [naval-mission-fleet-picker-dialog.md](naval-mission-fleet-picker-dialog.md) when stacked, then branch on the **selected** fleet — **non-empty Home Fleet** → Split then `DLG30001` for the **new** sea-going fleet ([naval-units-fleet-management.md](naval-units-fleet-management.md)); **empty Home Fleet** → tile-scoped **`OpenNavalUnitsPanelEvent`** ([naval-units-panel.md](naval-units-panel.md)); **sea-going in port** at the capital and eligible for Transfer → [in-port-fleet-marker-actions-dialog.md](in-port-fleet-marker-actions-dialog.md) (`DLG31004`) then `DLG30001` or `DLG40001`; other **sea-going in port** → [move-fleet-dialog.md](move-fleet-dialog.md) (`DLG30001`); **sea-going at sea** → [naval-mission-menu-dialog.md](naval-mission-menu-dialog.md) (`DLG31001`, including Sail/Move → `DLG30001`) and optional [naval-mission-target-dialog.md](naval-mission-target-dialog.md). Marker taps never fail silently. **Draft projection** (human naval move drafts in the app shell) must use the same harbor sea cell as map construction: `colonizethis_map` `harborDrawableSeaTileKeyForPortProvince` for in-port positions and **dock** destinations — not province town tile keys (`tileKeyForProvinceLocation`). | No separate toggle; follows map visibility and draft projection from `RegionMapViewData.fleetTileMarkers`. |
...
6. **Capital-link disconnected land hatch** (when that layer is enabled; Refs #4370).
7. **Resource icons** (when the base-layer mode includes resources).
8. **Improvement labels** `{n} of {cap}` / `{n}` (when the mode includes improvements and `n > 0`; Refs #4408).
...
**Label placement:** Improvement label at the **top-left** of the tile cell (small inset from top and left edges). **Do not** draw `I0`, `0 of {cap}`, or a lone `0`; draw only when improvement level is **greater than zero**. `R{n}` text labels are removed once transport sprite overlays are enabled.

**Improvement headroom (Refs #4408):** `{cap}` is `CellViewData.improvementTechCap` (`extractionCapForResourceOnTerrain` for the viewing player). Owned revealed tiles with a player-visible resource paint `{n} of {cap}` (compact `n/c` only if `{n} of {cap}` overflows the cell). Marks at cap (`n >= cap`) use a muted colour; marks with headroom stay accented. Foreign improved tiles and owned improved tiles whose resource is **not** player-visible paint `{n}` only (no cap). Unrevealed tiles paint no mark. Global observe omits `improvementTechCap` so every mark is `{n}`-only. Never paint `I{n}`, tech ids, or `extractionCap*` names.

Implementation: `_paintTiles` covers steps 1–4; `_paintGreatPowerLandOwnershipTint`, capital-link hatch, and `_paintOverlay` cover steps 5–8 so Z-order matches this spec (see `SPEC/program/map-region-map-render.md`).
...
| Flags (R / I / Roads) | Terrain | Resource icon | Improvement mark (n > 0) | Road/rail sprite (`roadLevel` > 0) |
|------|---------|---------------|----------------------------|---------------------------|
...
| Fort (near town) | `ui_icon_com_fort_1_64.png`, `ui_icon_com_fort_2_64.png`, `ui_icon_com_fort_3_64.png` — see [town-port-icons.md](town-port-icons.md) § Fort map icons |
...
- **Scope:** Indicators encode per-tile extraction throughput for the **human player’s owned tiles only** using `CellViewData.resourceExtractionEffectiveUnits` and `CellViewData.resourceExtractionBlockedUnits` (fallback: `resourceExtractionUnits` as all-effective when split fields are absent). **Disconnected improved tiles** (human-owned land with visible extractable production `F > 0` but not in the human `ConnectivityResult.connected` set) use `E = 0` and `B = F` (capital-link blocked yield); they must not be omitted while `F > 0` (Refs #4151).
...
- **Dark-theme selection prompt overlay tokens:** Under `AppThemes.editorialMonocle` the banner chrome MUST resolve every colour from `EditorialMonoclePalette` so the prompt reads as dark in-game chrome rather than the legacy Material light-on-black overlay. Hard-coded Material primaries (`Colors.black`, `Colors.white`) are forbidden on this surface.
  - **Banner background:** the surrounding `DecoratedBox` paints with `EditorialMonoclePalette.bgDeep` at alpha `kMapSelectionPromptBackgroundAlpha = 0.85` and a 1 px `EditorialMonoclePalette.accentDim` border so the banner reads as a framed dark surface against the lit map.
  - **Prompt text:** the `"Select a tile, or click cancel"` body resolves its `TextStyle.color` to `EditorialMonoclePalette.fg` (canonical body foreground, matching the Military owner sub-header and Economic improved-row patterns from the province overlay dark slices).
  - **Cancel button surface:** the inline `cancel` action MUST render as a `CtNinePatchButton` (Ct-* catalog counterpart per `SPEC/ui/pixel-art-ui-catalog.md` § Material design ban — Material `TextButton` is forbidden on the production UI surface). The button paints the canonical brass chrome inherited from the catalog widget: `CtGradients.buttonGradient` surface (top→bottom `--surface-lite` → `--surface`), 1 px `EditorialMonoclePalette.border` outer border, four 10 × 10 px brass corner brackets in `EditorialMonoclePalette.accent`, and engraved label text whose foreground resolves from the dark `titleSmall` slot (`EditorialMonoclePalette.accent` idle / `EditorialMonoclePalette.accentBright` hover). The banner overrides `minHeight` to `kMapSelectionPromptCancelMinHeight = 34` logical px (compact inline affordance) and `padding` to `EdgeInsets.symmetric(horizontal: 12, vertical: 4)` so the control stays proportional to the surrounding 8 logical px banner padding.
  - **Material defaults forbidden:** the banner MUST NOT consume `Theme.of(context).colorScheme.surface`, `colorScheme.onSurface`, `colorScheme.primary`, or any other Material colour-scheme lookup for the background, border, prompt text, or cancel button colours. All colours resolve from `EditorialMonoclePalette` tokens (directly or through the `CtNinePatchButton` catalog widget).
...
- **Interaction lock while active:** While selection mode is active, map interactions other than valid tile selection and region switching are suppressed (e.g., province/tile detail taps, civilian/fleet/army marker panel interactions).
...
- **Optional callback:** The widget may expose **onProvinceHovered**(prefixed province id, or null when hover leaves) so the parent can show tooltips or other feedback. **onTileHovered** reports the hovered full tile key (or null when the pointer leaves the grid), including **unrevealed** tiles, so MAP10001 chrome can name place, owner, and sight without opening `MAP20001`.
- **Owner / sight readout (MAP10001 chrome; no new screen ID):** When the in-game shell is **not** in work-target selection, pointer hover over a tile shows a compact dismiss-on-leave panel (extraction-legend / cargo-hold family: `--surface` fill, 1 px `--border`, no Material tooltip chrome). Copy uses `CellViewData` plus the same owner resolution as `MAP20001` Political (`ownerNameForProvinceOverlay`). Lines: **Place** (`provinceDisplayName` or sea-zone display name); **Owner:** `{name}` / `Unclaimed` on land, or a **Sea zone** identity line (not an owner row) on water; **Sight** as one of **Fully visible**, **Fogged — terrain only**, **Unknown — no intel yet** from `CellViewData.visibility` (never raw `TileVisibility` / `visible` enum text). Warp water (local id in `RegionMapViewData.warpMarkers`) adds one passage-to-the-other-world line. Unrevealed hover still shows authoritative owner (or Unclaimed) and must not name terrain, resource, or improvement. Hide this readout while work-target selection is active (selection-prompt / afford banner remains the only hover chrome). Touch players without pointer hover rely on the Political **Sight** row on `MAP20001`.
...
- **onTileHovered** still reports the tile key for `unrevealed` cells (owner/sight chrome); **onProvinceHovered** stays null for those cells so glow does not apply.
...
- **Work-order afford preview (Refs #4262 / #4630 / #4663 / #4668):** The **Work order afford preview** Widgetbook folder (`widgetbook_host/lib/catalogs/catalog_work_order_afford_preview.dart`) isolates `GameMapCanvasStackSelectionPrompt` with synthetic `WorkOrderAffordPreview` payloads: **Selection prompt — can afford**, **Selection prompt — material shortfall**, **Selection prompt — treasury shortfall**, **Selection prompt — free target (no cost)**, **Selection prompt — Purchase land tradeable gist**, **Selection prompt — Purchase land riches gist**, and **Selection prompt — transport step yield** raise / road cap / disconnected / binds capital / port on coast. When work-target selection is `purchase_land` on a legal tile, the banner shows the qualitative payoff gist beside cost/afford (`purchase_land_payoff_copy.dart`). When selection is `build_road` / `build_port` / `build_rail`, the banner shows the transport-step payoff gist (`transport_step_yield_copy.dart`). When selection is `build_fort` on a legal town tile, the banner shows the build-fort payoff gist (`build_fort_payoff_copy.dart`; selection-prompt goldens `app/test/build_fort_payoff_goldens_test.dart`). When selection is `prospect` on a legal tile, the banner shows the mineral-known + duration gist (`prospect_payoff_copy.dart`; selection-prompt goldens `app/test/prospect_payoff_goldens_test.dart`). When selection is `upgrade_town` on a legal town tile, the banner shows the town-workshop start / pause-until-4 / resume gist (`upgrade_town_payoff_copy.dart`; Widgetbook **Selection prompt — Upgrade town payoff pause**; Refs #4747).
...
- **Given** a map widget with `CellViewData.visibility` populated and the visibility mode set to **player-constrained**, **when** the widget renders a tile whose visibility is `unrevealed`, **then** the tile area is drawn as solid black, no terrain or resource icons or improvement labels or transport sprites are shown, the hover selector and province glow are not drawn on that tile, **onTileHovered** still reports that tile’s key, and tap/click selection still invokes province-selection callbacks.
- **Given** the in-game map is not in work-target selection and the pointer hovers a fully visible owned land tile, **when** the hover readout renders, **then** the UI layer shows that tile’s province display name, `Owner: {ownerNameForProvinceOverlay}`, and the Fully visible sight phrase (no raw `TileVisibility` / `visible` enum text).
- **Given** the pointer hovers a fogged other-faction land tile, **when** the hover readout renders, **then** the UI layer shows the authoritative owner and the Fogged — terrain only phrase.
- **Given** the pointer hovers an unrevealed land tile, **when** the hover readout renders, **then** the UI layer shows the authoritative owner (or Unclaimed) and the Unknown — no intel yet phrase, and does not reveal terrain, resource, or improvement detail.
- **Given** the pointer hovers a sea-zone tile whose local id is in `warpMarkers`, **when** the hover readout renders, **then** the UI layer shows the sea-zone display name, that it is a sea zone (not an owner row), the correct sight phrase, and one warp/passage-to-the-other-world line.
- **Given** work-target selection is active, **when** the pointer hovers a valid or invalid tile, **then** the UI layer does not show this owner/sight readout (existing selection-prompt / afford banner remains the only hover chrome).
- **Given** a 320 dp-wide viewport, **when** the hover readout renders, **then** the UI layer does not overflow horizontally.
...
- **Given** the map is in work target selection mode for a **material- or treasury-backed** work target and the player hovers a valid tile, **when** the selection prompt overlay renders, **then** the UI layer shows required cost chips and either **Can afford** (`workOrderAfford_canAfford`) or a plain shortfall line from `previewWorkOrderAffordAtTile` after other pending work; free targets (`explore`, `prospect`, `counter_spy`) show no cost chips.
- **Given** the map is in work target selection mode for `build_improvement` on a legal hovered tile, **when** the prompt renders, **then** the UI layer shows the same next-yield gist as `MAP20001` (Refs #4627).
- **Given** the map is in work target selection mode for `build_road` / `build_port` / `build_rail` on a legal hovered tile, **when** the prompt renders, **then** the UI layer shows the same transport-step payoff gist as `MAP20001` (Refs #4663; `app/test/transport_step_yield_goldens_test.dart`).
- **Given** the map is in work target selection mode for `build_fort` on a legal town tile, **when** the prompt renders, **then** the UI layer shows the same build-fort payoff gist as `MAP20001` (Refs #4668; `app/test/build_fort_payoff_goldens_test.dart`).
- **Given** the map is in work target selection mode for `explore` on a legal tile, **when** the prompt renders, **then** the UI layer shows the same Explore province-reveal + duration gist as `MAP20001` (Refs #4733; `app/test/explore_payoff_goldens_test.dart`).
- **Given** the map is in work target selection mode for `prospect` on a legal tile, **when** the prompt renders, **then** the UI layer shows the same mineral-known + duration gist as `MAP20001` (Refs #4741; `app/test/prospect_payoff_goldens_test.dart`).
- **Given** the map is in work target selection mode under `AppThemes.editorialMonocle`, **when** the selection prompt overlay banner renders, **then** the UI layer paints the banner background with `EditorialMonoclePalette.bgDeep` at alpha `kMapSelectionPromptBackgroundAlpha = 0.85` framed by a 1 px `EditorialMonoclePalette.accentDim` border, the prompt text `"Select a tile, or click cancel"` resolves its `TextStyle.color` to `EditorialMonoclePalette.fg`, the inline `cancel` action is a `CtNinePatchButton` (no Material `TextButton`) with `minHeight = kMapSelectionPromptCancelMinHeight = 34` and `padding = EdgeInsets.symmetric(horizontal: 12, vertical: 4)`, and the UI layer does not consume `Colors.black`, `Colors.white`, `Theme.of(context).colorScheme.surface`, `colorScheme.onSurface`, or `colorScheme.primary` for any of those tokens.
...
- **Given** the map widget is given **base layer display mode** `terrainAndResourcesImprovementLabels`, **when** the widget renders the base layer, **then** terrain and resource icons are drawn, and improvement marks (`{n} of {cap}` or `{n}`) are drawn only when `improvementLevel > 0` (top-left of cell); no transport sprites are drawn.
...
- **Given** the map widget uses a mode that draws improvements and/or transport sprites, **when** a tile has `improvementLevel == 0` or `roadLevel == 0`, **then** no `I0`, `0 of {cap}`, or lone `0` label is drawn and no transport sprite is drawn for that tile.
- **Given** **Show improvements** is on and the viewing player owns a revealed grain tile at improvement level 1 with `improvementTechCap` 1 and a player-visible resource, **when** the map paints that cell, **then** the UI layer shows a muted `1 of 1` (or compact `1/1` if the long form overflows) and does not paint `I1`. Pins: `packages/colonizethis_map/test/init_game_map_view_improvement_cap_test.dart`; host golden `app/test/goldens/improvement_headroom_mark_at_cap.png`.
- **Given** the same owned tile has `improvementTechCap` 2 at level 1, **when** the map paints, **then** the mark is a non-muted `1 of 2`. Golden: `app/test/goldens/improvement_headroom_mark_headroom.png`.
- **Given** a foreign revealed improved tile with level 2, **when** the map paints, **then** the UI layer shows `2` with no cap denominator. Golden: `app/test/goldens/improvement_headroom_mark_foreign.png`.
- **Given** the viewing player owns a revealed improved tile whose resource is not player-visible, **when** the map paints, **then** the UI layer shows `{level}` only with no cap denominator. Golden: `app/test/goldens/improvement_headroom_mark_hidden_resource.png`.
- **Given** an unrevealed cell in player-constrained visibility, **when** the map paints, **then** no improvement mark is drawn. Golden: `app/test/goldens/improvement_headroom_mark_unrevealed.png`.
...
- **Given** a land tile renders a resource icon and `CellViewData.resourceExtractionEffectiveUnits = E >= 0` and `CellViewData.resourceExtractionBlockedUnits = B >= 0`, **when** the map renders that tile, **then** the UI layer draws exactly `E` effective **extraction discs** (gold `0xFFFFD700`) and `B` blocked **extraction discs** (brown `0xFF5C4033`) immediately to the right of the resource icon in a left-to-right overlapping stack; each disc is a filled circle with a visible dark stroke (`0xFF1A120C`) so overlapping same-color stacks remain legible, and each disc remains visually distinct from the commodity resource sprite (Refs #4151 Phase 3).
...
- **Given** a human-owned improved land tile with visible extractable production `F > 0` that is **not** capital-connected, **when** the map renders that tile in a resource-including base mode, **then** `CellViewData.resourceExtractionEffectiveUnits = 0`, `CellViewData.resourceExtractionBlockedUnits = F`, and the UI draws **0** gold discs and **F** brown discs (Refs #4151).
- **Given** the same tile becomes capital-connected with full effective yield, **when** the map re-renders after connectivity refresh, **then** discs show `E = F` gold and `B = 0` brown unless path-capped below full.
...
- **Given** a fogged tile (`TileVisibility.fogged`) with a visible resource icon and extraction indicators, **when** the map renders the icon and indicators, **then** the indicators use the same fog modulation as the icon on that tile (fill and stroke).
...
- **Given** the viewing player owns a revealed land tile that is **not** in `ConnectivityResult.connected` and **Highlight land not bound to the capital** is ON, **when** `MAP10001` renders, **then** that tile receives the capital-link disconnected hatch and a connected owned neighbor without production does **not** (Refs #4370). Host goldens: `app/test/goldens/capital_link_hatch_mixed.png`, `capital_link_hatch_off.png`, `capital_link_hatch_fogged.png`, `capital_link_hatch_narrow.png` (`app/test/capital_link_disconnected_highlight_goldens_test.dart`). Widgetbook **Map Widget** stories: **Capital-link hatch — mixed connected/disconnected**, **highlight off**, **fogged disconnected**, **narrow (320 dp)**.
- **Given** the same disconnected tile is `TileVisibility.unrevealed` in player-constrained mode, **when** the map renders, **then** the tile stays the unrevealed treatment with no capital-link hatch (Refs #4370).
- **Given** global observe (`ShellPlayerContext.panelPlayerId == null`), **when** the map builds view data, **then** `capitalLinkDisconnected` remains false on all cells (Refs #4370). Pin: `app/test/providers/map_view_provider_extraction_test.dart` `mapViewExtractionMapsForShell` (hatch keys empty; discs unchanged).
...
- **Given** the player cycles to **terrain only**, **when** discs are not painted, **then** the UI layer hides the extraction-disc legend; cycling back to a resource-including mode with `viewingPlayerId != null` shows it again (Refs #4367). Golden: `app/test/goldens/extraction_disc_legend_hidden_terrain_only.png`.
- **Given** **global observe** (`viewingPlayerId == null`) and a resource-including base-layer mode, **when** `MAP10001` renders, **then** the UI layer omits the extraction-disc legend (Refs #4367). Golden: `app/test/goldens/extraction_disc_legend_hidden_global_observe.png`.
...
- **Given** a projected cross-region fleet marker is tapped and that marker’s fleet has `fleet.regionId` different from the destination region id, **when** the map emits `OpenNavalMissionMenuEvent`, **then** the event uses a destination-normalized `locationScopeKey` and `fleetIds` for the projected destination scope before `showNavalFleetMarkerFlow` runs.
- **Given** the only fleet at a port marker is a **non-empty** Home Fleet and the shell can mutate, **when** the player taps the marker, **then** the UI layer opens `SplitFleetDialog` for the Home Fleet and does not open `UNIT30001` as the only outcome; a confirmed split opens `DLG30001` for the new sea-going fleet from `NavalFleetsUpdatedEvent.game` (Refs #4448).
- **Given** the only fleet at a port marker is an **empty** Home Fleet, **when** the player taps the marker, **then** the UI layer emits `OpenNavalUnitsPanelEvent` with that marker’s `locationScopeKey` / `tileScopeTileKey` / Home Fleet id and does not open `DLG30001` or `DLG31001`.
- **Given** a sea-going fleet is in port at the human capital and is an eligible Home Fleet transfer source, **when** the player taps the marker (after optional `DLG31003`), **then** the UI layer opens `DLG31004` and does not open `DLG31001` (Refs #4625).
- **Given** a sea-going fleet is in port at a marker that is not an eligible capital transfer source, **when** the player taps the marker (after optional `DLG31003`), **then** the UI layer opens `DLG30001` and does not open `DLG31001` or `DLG31004`.
- **Given** a sea-going fleet is at sea at a marker, **when** the player taps the marker (after optional `DLG31003`), **then** the UI layer opens `DLG31001` with mission rows and a Sail/Move row that opens `DLG30001`.
- **Given** a human non-Home field army is stationed in province P and P’s town tile is revealed, **when** `MAP10001` renders that region, **then** the UI layer draws one army stack marker for P with a stack badge when more than one human army is in P (Home Army counts toward that badge).
- **Given** that marker includes at least one non-Home field army with cached destinations and the shell can mutate, **when** the player taps the army icon, **then** the UI layer opens `showOverlayArmyMoveFlow` / `DLG20001` (and `DLG20002` when several field armies qualify) and does **not** open `MAP20001`.
- **Given** a capital marker whose `armyIds` include the Home Army and one or more field armies with cached destinations, and the shell can mutate, **when** the player taps that marker, **then** the UI layer opens the overlay Move flow listing **only** those field armies: Home Army is absent from `DLG20002` rows and is never the `DLG20001` target. If exactly one field army is present, `DLG20002` is skipped.
- **Given** a capital marker whose `armyIds` are the Home Army plus one field army, **when** `MAP10001` renders that region, **then** the UI layer draws that marker with stack badge `2` (`stackCount == 2`).
- **Given** the only human army in P is a **non-empty** Home Army and the shell can mutate, **when** the player taps that marker, **then** the UI layer opens Split Army then `DLG20001` for the new field army and does **not** open `UNIT20001` as the only outcome.
- **Given** the only human army in the capital is the Home Army with `regimentUnitIds.length == 0`, **when** `MAP10001` renders that region, **then** the UI layer still draws the Home Army marker for that province (does not hide it).
- **Given** that empty Home Army marker and the shell can mutate, **when** the player taps it, **then** the UI layer opens `UNIT20001` and does **not** open `DLG20001`.
- **Given** a mixed capital marker (non-empty Home Army + stationed field armies with empty cached destinations) and the shell can mutate, **when** the player taps it, **then** the UI layer starts detach-then-move and does not call `showOverlayArmyMoveFlow` with those destination-less field ids.
- **Given** a civilian marker and an army marker share a capital/town cell, **when** the player taps the civilian glyph vs the army glyph, **then** each gesture reaches its own flow (civilian tile-scope panel vs army Move/roster); neither swallows the other.
- **Given** work-target selection mode is active, **when** the player taps an army marker tile, **then** the army flow does not start (existing work-target lock).
- **Given** every field army on a marker has a pending `ArmyMoveOrder`, **when** the map renders, **then** that marker is grayscale.
- **Given** observe / `canMutateViaUi == false`, **when** the player taps an army stack marker (field or Home), **then** the UI layer does not open `DLG20001`, `DLG20002`, or `UNIT20001` from that tap and stages no `ArmyMoveOrder`; the marker remains visible.
- **Given** Widgetbook / host goldens for army-marker default, stacked, grayscale, Home-only, empty-Home-Army, and mixed-Home-plus-field states under `AppThemes.editorialMonocle`, **when** `app/test/army_tile_marker_goldens_test.dart` runs, **then** committed PNGs `app/test/goldens/army_stack_marker_*.png` match (Refs #4384).
