...
| **Human fleet markers** | One icon per distinct human naval **location scope** (port or sea zone) on the region map. **In port:** anchored at the **same sea drawable cell** as the harbor icon (`TownMarkerView.portIconX` / `portIconY`), computed from `portsByProvinceSeaboard` per [town-port-icons.md](town-port-icons.md) (strict sea-only placement). **At sea:** anchored at the **sea-zone centroid** from the tile map. Icon: `ui_icon_map_fleet.png`. **Stack badge** when multiple fleets share the scope. **Grayscale** when every fleet at the marker has a pending naval move/mission draft; **Chebyshev ≤2 terrain “reveal” halo** (painting only; no stored fog change) when any fleet there has a pending naval **move** draft. **Tap** (before civilian markers when overlapping) emits **`OpenNavalUnitsPanelEvent`** with `locationScopeKey`, optional `initialSelectedFleetId`, and `tileScopeTileKey` for panel chrome. | No separate toggle; follows map visibility and draft projection from `RegionMapViewData.fleetTileMarkers`. |
| **Province and sea zone names** | **Land (province) labels:** `CellViewData.provinceDisplayName` (same string as the Political section in the province detail overlay), with fallback to local province id when missing. One label per land province per region. Centroid: average of **land** tile centers. **Sea zone labels:** one per local sea zone in the region topology; text from `RegionMapViewData.seaZoneDisplayNameByPrefixedId['regionId|localSeaZoneId']` with fallback to the local sea zone id (same resolution as app `sea_zone_name_resolver`). Centroid tile: `round(mean x)`, `round(mean y)` over all grid cells for that zone (same anchor as fleet markers). **No** capital star and **no** unit-presence icon row on sea labels. **Shared toggle:** in-game **Show province names** (`showProvinceNamesLayer`); **no** separate toggle for sea zone names. **Player-constrained visibility (sea):** draw a sea zone label only if the **centroid cell** is not `TileVisibility.unrevealed` under the same rules as terrain for the map, including fleet move-draft reveal halo (`_visibilityForTerrain` / Chebyshev ≤2). Other sea cells in the zone need not be revealed. **Sea plate style:** fill **RGB (173, 216, 230)** at **alpha 0.55**; text **black** (`#000000`). **Sea placement:** default **above** the centroid so the plate does **not** overlap the centroid cell (fleet icon cell); if that would clip the **top** of the map, place **below** the centroid instead; then apply **minimal** horizontal/vertical nudge so the **entire** plate stays inside the region world rect — no clipping. **Long names:** no ellipsis; render the full string (overlap with neighbors acceptable). Land province label rules (GP-tinted vs neutral plate, capital icon, presence row, ellipsize exceptions) are unchanged — see § Sea zone name plates and § Province label unit presence icons. Rendered after province/political border strokes; **sea zone name plates** are painted immediately **after** land province name plates and **before** capitals/towns/warp/civilian/fleet markers. | Yes. **Show province names** only; independent of province overlay, province ownership tint, and political overlay toggles. |
...
5. Province boundary strokes (when enabled), then hover glow, political borders, province names, **sea zone names**, capitals, towns/ports, warp indicators, civilian markers, fleet markers, then selection/hover chrome — unchanged except as noted elsewhere in this spec.
...
When resource icons are visible, the map may render extraction-unit discs for the same tile:

- **Scope:** Discs encode per-tile effective extraction units for the **human player only**, from `CellViewData.resourceExtractionUnits`.
- **Count rule:** Draw exactly `N` discs when `N` is an integer `>= 1`; draw none when `N` is `0` or `null`.
- **Economic alignment:** `N` follows the tile-level extraction branch used by `computeExtraction` tile `effectiveCapped` and excludes aggregate-only adjustments (for example `Game.capitalTileGrainBonusPerTurn`).
- **Placement:** Discs are anchored to the **painted** resource icon `Rect` (the quarter-size/capped destination box) and rendered immediately to the right in a horizontal right fan (`+x` step with overlap, default no vertical stagger).
- **Base-layer gating:** Discs are rendered only when resource icons are rendered (`baseLayerDisplayMode != terrainOnly`).
- **Icon visibility gating:** If resource icon visibility/prospecting rules suppress the icon on a tile, discs are also suppressed on that tile.
- **Fog parity:** On `TileVisibility.fogged`, discs use the same fog modulation treatment as resource icons.
- **Color source:** Disc fill color comes from a fixed checked-in resource palette generated by a build-time sampler from `ui_icon_com_<resource_id>.png`; runtime map rendering must not decode/summarize icon pixels for palette lookup.
...
- **Given** the map widget is given **base layer display mode** `terrainAndResources`, **when** the widget renders the base layer, **then** terrain and resource icons (64×64 source assets, on-map quarter-cell/capped display per § Resource Icons) are drawn per cell where present, and no improvement or road labels are drawn.
...
- **Given** resource icon visibility/prospecting rules suppress a tile’s resource icon, **when** the map renders that tile, **then** the UI layer draws no extraction discs on that tile.
- **Given** a fogged tile (`TileVisibility.fogged`) with a visible resource icon and `CellViewData.resourceExtractionUnits >= 1`, **when** the map renders the icon and discs, **then** the discs use the same fog modulation as the icon on that tile.
- **Given** two resource ids `A` and `B` with fixed palette entries in the checked-in build-time generated palette map, **when** extraction discs render for tiles with `resourceId = A` and `resourceId = B`, **then** the UI layer uses exactly those palette RGB values and performs no runtime PNG dominant-color sampling.
