...
- **Given** that marker includes at least one non-Home field army and the shell can mutate, **when** the player taps the army icon, **then** the UI layer opens `showOverlayArmyMoveFlow` / `DLG20001` (and `DLG20002` when several field armies qualify) and does **not** open `MAP20001`.
- **Given** a capital marker whose `armyIds` include the Home Army and one or more field armies, and the shell can mutate, **when** the player taps that marker, **then** the UI layer opens the overlay Move flow listing **only** those field armies: Home Army is absent from `DLG20002` rows and is never the `DLG20001` target. If exactly one field army is present, `DLG20002` is skipped.
...
- **Given** the only human army in P is the Home Army, **when** the player taps that marker, **then** the UI layer opens `UNIT20001` and does **not** open `DLG20001`.
