...
4. **Road/rail transport sprites** (when the mode includes roads and `roadLevel > 0`): draw on **land only** using cardinal mask bits (**N=1, E=2, S=4, W=8**) from neighboring **land** cells with `roadLevel > 0`; use road family for levels 1/2 and rail family for level 4.
...
Implementation: separate paint passes (or equivalent ordering) for transport sprites, icons, and improvement text so Z-order is guaranteed.
...
- **Given** the Great Power tint layer and a base-layer mode that includes resource icons and/or improvement labels and/or transport sprites, **when** the map renders a qualifying land tile, **then** the stack from bottom to top is: terrain → GP tint (if on) → transport sprites (if mode includes roads and level > 0) → resource icons (if mode includes resources) → improvement labels (if mode includes improvements and level > 0) → later overlays per § Layer model.
