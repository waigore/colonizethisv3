...
- **Count rule:** Draw exactly `E` gold icons and `B` grayscale icons where `E = resourceExtractionEffectiveUnits` and `B = resourceExtractionBlockedUnits`. Draw none when `E + B <= 0`.
...
- **Size:** Indicator icon edge length is `>=` resource icon display edge length for that tile (slightly larger by default, capped at source-asset native 64 px).
...
- **Given** a land tile renders a resource icon and `CellViewData.resourceExtractionEffectiveUnits = E >= 0` and `CellViewData.resourceExtractionBlockedUnits = B >= 0`, **when** the map renders that tile, **then** the UI layer draws exactly `E` gold extraction indicator icons and `B` grayscale extraction indicator icons immediately to the right of the resource icon in a left-to-right overlapping stack.
...
- **Given** extraction indicators are rendered for a tile, **when** indicator layout is computed, **then** each indicator icon edge length is greater than or equal to the resource icon display edge length for that tile and the stack advances left-to-right with overlap.
