...
- **Labour and feeding indicator (Refs #4506):** After the cargo hold indicator and before the Old World race chip, when the human player has a non-empty worker pool (`WorkerPool` full capacity > 0), the UI shows a **player-initiated** labour/feeding indicator with a worker icon (`ui_icon_worker_peasant.png`) and numeric `effectiveLabour/fullCapacity` (no spaces) from the same post-extraction preview pipeline as `GAME20001` Production (`labourReadinessForPlayer` / `forcesFeedingForPlayer` on pending build costs → Extraction → Riches-to-treasury). Hidden when the worker pool capacity is zero. Global (single total, not per-region); remains visible when switching region tabs.
- **Labour and feeding colour tiers:** When preview data is reliable, only the numeric fraction text (not the worker icon) resolves colour: **normal** (full capacity and fully fed forces) → `--muted`; **reduced** (effective labour < full capacity and labour > 0) → `--accent`; **critical** (effective labour == 0 with non-empty pool, or present armies/fleets underfed) → `--danger`. When observe mode marks treasury/cargo not-defined, or player chrome is hidden, numeric text shows the observe sentinel (`—`) and stays `--muted`.
- **Labour and feeding teaching surface:** Tapping/clicking the indicator opens a dismissible floating panel (cargo-hold family: ×, outside tap on the map scrim below shell chrome, Esc) listing `Labour this turn: N of C`, the primary shortage reason when labour < capacity (same copy as Production Available), army/navy fed status lines when forces exist, and one counsel line (`production_forcesFoodDetailsPriority`). Dismiss and reopen semantics match cargo hold. No auto-open; no deep-link buttons. Desktop hover shows `mapControls_labourFeeding_tooltip`; semantics use `mapControls_labourFeeding_semanticsLabel`.
...
- **Labour and feeding:** Worker icon (`ui_icon_worker_peasant.png`, 18×18 dp) + monospace `effective/fullCapacity`; numeric text colour follows the labour/feeding tier rules above (icon stays default). Separated from cargo by a **1 px** `--border` left rule. Tap opens the labour/feeding details popover; hidden when worker pool capacity is zero. Desktop hover shows `mapControls_labourFeeding_tooltip`.
- **Old World race chip:** Always-visible control after labour/feeding (when shown) and before the players-bar toggle. Shows the human (or observe-focus) Great Power `N / 31` Old World provinces and a rival-leader cue when another court is strictly ahead. Tap opens `GAME70001` with the same `NavigateToRouteEvent(Routes.victory, {game, humanPlayerId})` as the left-rail Victory button. Hidden when `Game.victory != null`. Independent of `showPlayersBar`. Layout, compact 320 dp copy, and ACs: [old-world-race-chip.md](components/old-world-race-chip.md).
...
- **Given** the in-game tab bar is visible and the human Great Power has worker pool capacity > 0, **when** the shell renders, **then** the UI layer renders a labour/feeding indicator after cargo showing `effectiveLabour/fullCapacity` from `labourReadinessForPlayer` (post-extraction preview).
- **Given** effective labour equals full capacity and no underfed forces, **when** the labour indicator renders, **then** only the numeric text resolves to `--muted` and the details popover (when opened) omits a shortage reason line.
- **Given** food or luxury strike reduces labour below capacity with labour > 0, **when** the labour indicator renders, **then** only the numeric text resolves to `--accent` and the details popover names the same primary reason as Production Available.
- **Given** regiments or ships exist and `previewForceFeeding` reports underfed coverage, **when** the labour indicator renders, **then** only the numeric text resolves to `--danger` and the details popover states armies and/or fleets underfed with the existing combat gist lines.
