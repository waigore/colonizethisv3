...
- **Treasury formatting modes:** The treasury numeric text has two deterministic display modes: **exact** (`12,345`, `-1,000`) and **abbreviated** (`12.3k`, `-1.0k`). Exact mode uses thousands separators and no currency symbol. Abbreviated mode uses compact decimal formatting and no currency symbol. Mode is chosen inside the treasury details popover (Exact / Compact); the chip tap does **not** toggle format by itself. Session-local `_showExactTreasury` keeps the last choice after dismiss (Refs #4560).
...
- **Treasury teaching surface (Refs #4560):** Tapping/clicking the treasury indicator (when treasury is defined) opens a dismissible floating panel (cargo-hold family: ×, map scrim below shell chrome, Esc; max-width ~280 dp). The panel shows current gold, the same signed Next-turn forecast as the chip (omitted when the chip hides delta), non-zero **committed** gold families in player language (research spend, staged market bids, grant aid, overture treasury, recruit/train workers, unit-build treasury, purchase-land treasury), Exact/Compact controls, and one muted counsel line that extraction, riches, market fills, and the rest of Next turn are included in the forecast but not itemized. Omit zero families; omit unused research seats / funding None (UXD-001); omit idle Spies (UXD-002). Do not claim listed lines sum to the net delta. Observe / `treasuryNotDefined` keeps tap disabled (no popover). No auto-open; no deep-link buttons.
...
- **Cargo hold teaching surface:** Tapping/clicking the indicator opens a small dismissible floating panel (not a full-screen dialog) anchored below the indicator via `showChromeAnchoredPopover` (`rightAlignBelow`; Refs #4582). The panel lists overseas extraction **N**, Home Fleet holds **C**, free for trade bids `max(0, C − N)`, and one fixed counsel line (`mapControls_cargoHold_details_counsel`). Dismiss via ×, outside tap on the map scrim, or Esc. Reopen on the next tap. No auto-open on load, turn end, or cargo pressure; no persistent “don’t show again” preference; no deep-link buttons. Desktop hover shows `mapControls_cargoHold_tooltip`; semantics use `mapControls_cargoHold_semanticsLabel`.
- **Labour and feeding indicator (Refs #4506):** After the cargo hold indicator and before the Old World race chip, the UI always shows a **player-initiated** labour/feeding indicator with a worker icon (`ui_icon_worker_peasant.png`) and numeric `effectiveLabour/fullCapacity` (no spaces) from the same post-extraction preview pipeline as `GAME20001` Production (`labourReadinessForPlayer` / `forcesFeedingForPlayer` on pending build costs → Extraction → Riches-to-treasury). Empty worker pool (`fullCapacity == 0`) still renders muted `0/0` (does not hide). Global (single total, not per-region); remains visible when switching region tabs.
- **Labour and feeding colour tiers:** When preview data is reliable, only the numeric fraction text (not the worker icon) resolves colour, first match wins: **`--danger`** if (`fullCapacity > 0` and `effectiveLabour == 0`) **or** present armies/fleets are underfed (`hasAnyForces && !isFullyFed`) — danger wins when labour is reduced-but-nonzero **and** forces are underfed; **`--accent`** if `0 < effectiveLabour < fullCapacity` and not danger; **`--muted`** otherwise (full capacity, including empty-pool `0/0` with no underfed forces). When observe mode marks treasury/cargo not-defined, or player chrome is hidden, numeric text shows the observe sentinel (`—`) and stays `--muted`.
- **Labour and feeding teaching surface:** Tapping/clicking the indicator opens a dismissible floating panel (cargo-hold family: ×, outside tap on the map scrim below shell chrome, Esc) listing `Labour this turn: N of C`, the primary shortage reason when labour < capacity (same copy as Production Available), or `No workers trained yet` when the pool is empty (no shortage reason), army/navy fed status lines when forces exist, and one counsel line (`production_forcesFoodDetailsPriority`). Dismiss and reopen semantics match cargo hold. No auto-open; no deep-link buttons. Desktop hover shows `mapControls_labourFeeding_tooltip`; semantics use `mapControls_labourFeeding_semanticsLabel`.
...
- **Treasury:** Coin icon (`ui_icon_treasury_coin.png`, 18×18 dp) + monospace value in `--accent-dim`; optional projected delta in `--success` (positive) or `--danger` (negative) at 10 sp. Tap opens the treasury details popover when defined; Exact/Compact live inside that panel (Refs #4560).
...
- **Labour and feeding:** Worker icon (`ui_icon_worker_peasant.png`, 18×18 dp) + monospace `effective/fullCapacity`; numeric text colour follows the labour/feeding tier rules above (icon stays default). Separated from cargo by a **1 px** `--border` left rule. Tap opens the labour/feeding details popover; always visible including empty-pool `0/0`. Desktop hover shows `mapControls_labourFeeding_tooltip`.
- **Old World race chip:** Always-visible control after labour/feeding and before the players-bar toggle. Shows the human (or observe-focus) Great Power `N / 31` Old World provinces and a rival-leader cue when another court is strictly ahead. Tap opens `GAME70001` with the same `NavigateToRouteEvent(Routes.victory, {game, humanPlayerId})` as the left-rail Victory button. Hidden when `Game.victory != null`. Independent of `showPlayersBar`. Layout, compact 320 dp copy, and ACs: [old-world-race-chip.md](components/old-world-race-chip.md).
...
- `GameMapArea`, `CtRegionMap`, map tool row, treasury/cargo/labour-feeding/Old World race indicators — see [map-widget.md](map-widget.md), [empire-buttons.md](empire-buttons.md), [old-world-race-chip.md](components/old-world-race-chip.md).
...
Folder: **Game Tab Bar** — tab-bar chrome including labour/feeding stories registered from [`_labourFeedingTabBarStories`](../../widgetbook_host/lib/catalogs/catalog_game_chrome_labour_feeding.dart) (Refs #4506) and treasury details stories (Refs #4560). Use cases: `Labour — reduced (accent numeric) (Refs #4506)`, `Labour — empty pool (muted 0/0) (Refs #4506)`, `Labour — underfed forces (danger) (Refs #4506)`, `Treasury details — committed spend (Refs #4560)`, `Treasury details — forecast only (Refs #4560)`, `Treasury details — observe disabled (Refs #4560)`, `Treasury details — mobile 360×640 (Refs #4560)`.
...
- **Given** the human player's treasury is `12345` and treasury is defined, **when** the user taps the treasury indicator, **then** the UI layer opens the treasury details popover and does not use that tap solely to toggle exact/abbreviated formatting (Refs #4560).
- **Given** the treasury details popover is open, **when** the player chooses Compact then Exact, **then** the chip and popover amounts switch formatting and the last mode persists after dismiss for the session (Refs #4560).
...
- **Given** the treasury details popover is open with a non-zero projected delta, **when** it renders, **then** it shows current treasury and the same signed Next-turn forecast as the chip (Refs #4560).
- **Given** at least one committed gold family is non-zero, **when** the treasury details popover renders, **then** those families appear as plain-language £ lines and families at 0 are omitted (Refs #4560).
- **Given** empty research seats or funding None, **when** the treasury details popover renders, **then** unused research capacity is not listed (UXD-001); idle Spies are not listed (UXD-002) (Refs #4560).
- **Given** the HUD forecast includes effects beyond listed committed lines, **when** the treasury details popover renders, **then** a muted counsel line states that extraction, riches, market fills, and the rest of Next turn are included in the forecast and are not itemized (Refs #4560).
- **Given** `treasuryNotDefined` (observe), **when** the treasury chip renders, **then** tap does not open the popover (Refs #4560).
- **Given** a 360×640 dp viewport and treasury is defined, **when** the treasury details popover opens, **then** Next turn stays tappable (map-only scrim below chrome) (Refs #4560).
- **Given** treasury details panel variants for committed spend, forecast-only (no committed lines), and `kMinViewportWidth` shell with the popover open, **when** the host golden suite in `app/test/treasury_details_goldens_test.dart` captures each keyed `RepaintBoundary`, **then** each `matchesGoldenFile` baseline under `app/test/goldens/treasury_details_*.png` matches the committed PNG (Refs #4560).
...
- **Given** the in-game tab bar is visible, **when** the shell renders, **then** the UI layer renders a labour/feeding indicator after cargo and before the Old World race chip showing `effectiveLabour/fullCapacity` from `labourReadinessForPlayer` (post-extraction preview), including empty-pool `0/0`.
- **Given** `fullCapacity == 0` and `effectiveLabour == 0` and no underfed forces, **when** the labour indicator renders, **then** the UI layer shows muted `0/0` (does not hide the chip) and the details popover (when opened) states that no workers are trained yet and omits a shortage reason.
- **Given** effective labour equals full capacity, the pool is non-empty, and no underfed forces, **when** the labour indicator renders, **then** only the numeric text resolves to `--muted` and the details popover (when opened) omits a shortage reason line.
- **Given** food or luxury strike reduces labour below capacity with labour > 0 and no underfed forces, **when** the labour indicator renders, **then** only the numeric text resolves to `--accent` and the details popover names the same primary reason as Production Available.
- **Given** `fullCapacity > 0` and `effectiveLabour == 0`, **when** the labour indicator renders, **then** only the numeric text resolves to `--danger` even if forces are fully fed.
- **Given** labour is reduced but non-zero **and** `previewForceFeeding` reports underfed coverage, **when** the labour indicator renders, **then** only the numeric text resolves to `--danger` (danger wins over accent) and the details popover states armies and/or fleets underfed with the existing combat gist lines.
- **Given** regiments or ships exist and `previewForceFeeding` reports underfed coverage (including when labour is at full capacity), **when** the labour indicator renders, **then** only the numeric text resolves to `--danger` and the details popover states armies and/or fleets underfed with the existing combat gist lines.
- **Given** the player taps the labour indicator, **when** the details popover opens, **then** it is dismissible via ×, outside tap on the map scrim, or Esc; it has no deep-link buttons; on viewports `< 600` dp **Next turn** stays tappable.
- **Given** observe mode marks treasury or cargo not-defined, **when** the labour indicator renders, **then** the numeric text shows the observe sentinel and does not open a details popover on tap.
- **Given** the in-game shell control row is visible and `Game.victory == null`, **when** the UI renders region tabs, **then** the UI layer also renders the Old World race chip after labour/feeding with the focus court’s `N / 31` per [old-world-race-chip.md](components/old-world-race-chip.md) (Refs #4451).
