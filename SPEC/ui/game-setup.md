...
| Element | Requirement |
|--------|-------------|
| Slots | Six rows: slot 0 = human (**Player 1 (You)**), slots 1–5 = AI (**Player 2–6 (AI)**). Each row: **slot label**, then the nation and leader pickers laid out per the same `kGameSetupNarrowBreakpoint` (500 dp) rule used by `CtGameSetup`. **Wide viewport (≥ 500 dp):** one horizontal row with the two pickers side-by-side (equal width, 8 dp gap) to save vertical space. **Narrow viewport (< 500 dp):** vertical stack — slot label, then nation dropdown full width, then leader dropdown full width — so the dialog honours the same `< 500 dp` stacking rule defined in [mobile-adaptation.md](mobile-adaptation.md) § 4. See [new-game-leader-selection-dialog.md](new-game-leader-selection-dialog.md) § Layout / wireframe and Acceptance Criteria for the authoritative dialog ACs. |
