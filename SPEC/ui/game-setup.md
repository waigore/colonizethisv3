...
| Actions | **Start** closes the dialog and passes **`(orderedGreatPowerIds, leaderVariantByGpId, seed, infiniteMode)`** to the shell; shell builds `GameSetupConfig` (including **`seed`** and **`infiniteMode`**) and runs [Game initializing](game-initializing.md). **Cancel** dismisses without starting setup. |
