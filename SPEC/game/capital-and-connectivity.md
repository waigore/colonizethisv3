...
At **game init**, every province has exactly one **town** tile assigned (the province's "town" for extraction). The town is: (1) the **capital tile** if the province is that faction's capital province, or (2) otherwise apply **branch eligibility first** (capital exception above; seaboard filter for sea-bound provinces; overseas port when a port exists), then **prefer plains** among remaining candidates (if any plains exist in the candidate set, rank only those; otherwise keep the full set), then rank by: (a) **minimum squared Euclidean distance** from the tile cell `(x, y)` to the **province centroid** — centroid is `round(mean x)`, `round(mean y)` over **all** tiles of that province (same formula as sea-zone fleet centroid markers in program); (b) **shortest path** to the capital tile by BFS on province tiles **in that region**, when the branch supplies a capital tile in-region (otherwise this step is skipped and all candidates tie on distance); (c) **ascending lexicographic** `townTileKey` string as final deterministic tie-break. **Neutral** provinces (no `ownerId`) use all province tiles as candidates with plains preference then steps (a)–(c) and no BFS. **Sea-bound province (non-capital):** candidate set = province tiles **4-adjacent** to a sea-zone cell for a sea zone **P–S-adjacent** to that province in topology; then plains preference and (a)–(c). **Sea-bound mismatch fallback (tolerant):** if that candidate set is empty, use **all** province tiles with plains preference and (a)–(c) and emit one `logic:` warning. **Same region (not overseas), not sea-bound:** candidate set = all province tiles; then plains preference and (a)–(c). **Overseas provinces:** if the province has a registered **port** tile, the town is that port tile (single choice); if there is **no** port, candidate set = all province tiles; then plains preference and (a)–(c) without BFS to an overseas capital. **Select-then-convert:** after the town tile is chosen, if it is not `TerrainType.plains`, The System converts that tile to plains and clears resource/extraction on it (including overseas port towns). Capital-province towns share the capital tile already converted during capital auto-choice.
