...
- **Foreign aid:** **Grant Aid** — the player supplies a **positive integer** amount in pounds (£); that amount is deducted from the granting GP’s treasury and improves relation score. **MVP** allows any valid positive integer (free entry in UI/API); there is **no** fixed list of allowed grant amounts. A **default pre-fill** (e.g. £100) in the UI or in AI order suggestions is a convenience only, not a validation rule.
