...
- Given Great Power `H` has at least one Spy remaining after spy-resolution in a province owned by faction `F` (`F ≠ H`), and `F` completed a technology and declared war this turn  
  When the system builds the last-turn intelligence digest  
  Then the digest includes one spy-report block for `F` observed by `H` covering those acts, and does not include `F`’s `hiddenAgenda`.

- Given Great Power `H` has no Spy remaining in any province owned by `F` after spy-resolution  
  When the system builds the last-turn intelligence digest  
  Then the digest includes no spy-report block for `F` observed by `H`.
