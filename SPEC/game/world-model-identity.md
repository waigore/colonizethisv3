...
**ProvinceId helper strictness:** `ProvinceId.regionIdFrom` and `ProvinceId.localIdFrom` both require a prefixed province id (`regionId|localId`) and throw for non-prefixed input. Boundary adapters that legitimately start from local ids must derive full ids explicitly (for example with `ProvinceId.full` or `ProvinceId.prefixedFrom(regionId, id)`) before game-state lookup. Code that needs the local segment from either stored shape uses `ProvinceId.localFromMaybePrefixed(id)` instead of re-inlining `isPrefixed ? localIdFrom : id`.
