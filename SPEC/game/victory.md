...
- **UI:** The app must not allow further full-turn resolution when `calendarCampaignHalted` is true (same blocking category as military victory for “Next turn”). The campaign-complete presentation is the calendar-halt variant of `OVL20001` in [victory-overlay.md](../ui/victory-overlay.md) (mounted when `calendarCampaignHalted && victory == null`; military overlay wins if `victory != null`).
...
- When `Game.victory != null`, the app shows the **victory overlay** (`VictoryOverlay` / `VictoryPanel` on `GameScreen`) in the military variant.
- When `Game.calendarCampaignHalted` and `Game.victory == null`, the same overlay mounts in the **campaign-complete** variant (see [victory-overlay.md](../ui/victory-overlay.md)).
- **Content (military):** Winner’s display name, victory type label (e.g. “Military victory”), turn number. Option to **return to main menu** or **view final state** (continue viewing the map without further turns).
- **Content (calendar):** Title “Campaign complete”; declared overall-strength winner or tie copy shared with `GAME70001`. Same Return / View Final State actions.
- No further orders or turn advancement once victory is set or the calendar campaign is halted.
