...
- **Extraction cap:** In the full model, per-resource cap is derived from unlocked gathering techs (max level per resource from the table). current product uses a single scalar cap; see [tech-and-extraction-cap.md](tech-and-extraction-cap.md).
- **Tests:** Unit tests for cap resolution (scalar current product; future per-resource); extraction pipeline respects player cap; research unlocking gathering tech increases cap.
