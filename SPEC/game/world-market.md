...
- **Riches handoff.** When the purchased tile's resource is in the riches set (`gold`, `silver`, `gems`, `diamonds`, `spices`), riches are not auto-offered on the world market (riches are excluded from trading per § Tradeable commodities). Instead, the riches yield converts to the **owning GP's** treasury during phase 3 Riches-to-treasury at `units × richesBasePrice × richesCashMultiplier` (truncated to int). The owning GP is identified via `WorldState.purchasedTilesByTileKey` and the Minor or Tribe receives no treasury (consistent with § Minor and tribe auto-sell — minors and tribes never hold treasury). Per-tile yield is `effective = min(improvementLevel, defaultExtractionCap, tileTransportLevel)` clamped to `[0, 4]`, where `tileTransportLevel = port ? 4 : roadLevel` and `defaultExtractionCap = 1` (Minors and Tribes do not research extraction tech). The mineral-prospecting filter that excludes `gold/silver/gems/diamonds` from non-Great-Power extraction is **bypassed** for purchased tiles because the owning Great Power's purchase already authorizes the extraction of riches on that tile. No further connectivity recomputation is performed in phase 3 (the merchant's purchase implies the tile had a transport path to the source faction's capital at purchase time, so the tile's own transport level is a sound conservative cap and yields a deterministic per-tile value with no Step-A-style faction-wide BFS). After phase 6 Diplomacy reassigns province ownership to a Great Power (post-conquest), the purchased-tile entry is filtered out by [PurchasedTileIndex.fromGame](../program/world-market-resolution.md), so no further riches handoff occurs from that tile.
...
- **Purchased-tile riches handoff — credit.** Given Great Power `gpA` previously executed `purchase_land` on a tile in a province owned by Minor `M1` whose tile resource is `gold` and the tile's improvement level is 1, when phase 3 Riches-to-treasury runs with `richesCashMultiplier = 1.0`, then the system credits `gpA.treasury` by exactly `1 × richesBasePrice("gold") × 1.0` and credits Minor `M1` by 0 treasury, the gold is not auto-offered on the world market in phase 13, and `gpA.stockpile.quantityOf("gold")` is unchanged.
- **Purchased-tile riches handoff — non-riches resource.** Given `gpA` purchased a tile in `M1` whose tile resource is `timber` (a non-riches commodity), when phase 3 Riches-to-treasury runs, then the system applies no treasury credit to `gpA` for that tile (the timber continues to flow through the world market as a Minor/Tribe auto-offer per § Minor and tribe auto-sell).
- **Purchased-tile riches handoff — unimproved tile.** Given `gpA` purchased a tile in `M1` whose tile resource is `silver` and the tile's improvement level is 0, when phase 3 Riches-to-treasury runs, then the system applies no treasury credit to `gpA` for that tile (effective yield is 0 by the per-tile non-Great-Power extraction formula).
- **Purchased-tile riches handoff — post-conquest filter.** Given a tile in province `P1` recorded in `WorldState.purchasedTilesByTileKey` with `gpA` as the buyer, and `P1.ownerId` is now a Great Power (the province was conquered after the original purchase), when phase 3 Riches-to-treasury runs, then the system applies no purchased-tile riches credit for that tile (the owning Great Power resumes regular phase-2 extraction onto its own stockpile via the standard GP path).
