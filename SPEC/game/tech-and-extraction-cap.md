...
- Given a player has a tech table where each tech id is either unlocked or locked and a program-level map from tech id to extraction cap level (1–4) as defined in `tech_extraction.dart`  
  When the System computes the player’s current product scalar extraction cap from the tech table  
...
- Given the implementation is running in the current product mode where a single scalar extraction cap is used per player for all resources  
