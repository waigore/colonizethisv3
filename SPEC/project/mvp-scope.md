...
| **4** | Quick Battle (GDD 06); minimal diplomacy (war/peace); AI (one personality); 7 Great Powers (configurable) | Quick Battle UI and resolution; AIPlanner; order merge; diplomacy phase | Unit tests for Quick Battle resolution, AI, diplomacy; playtest vs AI for several turns | Quick Battle, AI and diplomacy within spec | Minimal diplomacy and Quick Battle logic exist. **U2** can implement the Diplomacy panel (`03j`) for war/peace and hook combat mode selection / Quick Battle UIs to the existing resolver; these UIs are not required for Phase 4 exit but are fully authorized. |
...
| 7 Great Powers (configurable); Minor Nations and Tribes (reactive, no order phase) | Full tactical combat (manual unit movement on battlefield) |
| Full military roster (Phase 3); auto-transport; auto-resolve combat; Quick Battle (Phase 4) | Full diplomacy (overture chain, alliances) |
...
| **U2 — Core Management Panels** | UXD `03e` (Units UI), `03f` (Development UI), `03g` (Production UI), `03j` (Diplomacy UI); aligned with GDD 04, 05, 06, 07. | Implement Units, Development, Production, and minimal Diplomacy panels; map interactions in these panels to orders and state backed by colonizethis_logic (economy, movement, combat, diplomacy) and colonizethis_models. | Widget tests per panel (list + detail, filters, disabled states); integration tests ensuring orders created via UI match expected effects when TurnResolver runs; coverage of basic diplomacy interactions (war/peace) via the UI. | Phases 2–4 (economy & auto-transport, units & movement, auto-resolve/Quick Battle, minimal diplomacy, AI) provide the required models, config, and resolvers. |
...
Deliberate simplifications for MVP: single victory condition; Quick Battle (streamlined tactical, Phase 4); one AI personality for all AI powers; minimal diplomacy (war/peace); 7 Great Powers (configurable); Minor Nations and Tribes reactive; full military roster (Phase 3); two-region map with procedural world generation for new games; program-level config in scope (single source for stats/modifiers). Defer to post-MVP: per-leader personalities, portraits/dialogue, navy, full tech tree, all alternative victory types, and JSON/layered rulesets.
