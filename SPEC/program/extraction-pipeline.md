...
3. Province lookup for town development rules must be **region-scoped**: resolve province only within the tile's region ([world-model-identity.md](../game/world-model-identity.md)). Do not search regions in sequence.
4. Effective yield per [extraction-and-improvements.md](../game/extraction-and-improvements.md) § Extraction formula and town development cap: apply **capital vs non‑capital** rules, **road path from tile** vs **Town‑rule‑only** branch, and path/tile **transport** caps; **do not** use `townDevelopmentLevel` in the connectivity step.
