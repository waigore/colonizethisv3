...
1. **`_paintTiles`** — sea cells; land base tiles (without L1 interior plains resource decals); **road/rail transport sprites** (when base-layer mode includes roads, land only); **L1 interior plains resource decals** (`tile_plains_grain` / `meat` / `horses` / plantation `sugar_cane` / `tobacco` / `cotton` / `spices`, same gating as land base); L2 feature terrain overlays (forest/mountains on plains, etc.).
