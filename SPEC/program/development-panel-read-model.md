...
DevTools timeline captures remain optional owner verification for AC1 qualitative bar; timing tests below are the CI profiling anchor for AC2 (measurable µs reduction on read-model build path, not a substitute for in-app DevTools panel-open captures). **AC1 peer parity:** lazy Old World read-model open path must stay within **2×** the `ProductionScreenBody` synchronous prep surrogate on the same representative fixture (`development_panel_open_path_timing_test.dart`).
