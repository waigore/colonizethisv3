...
**SPEC/program** — Enforces GitHub [#2071](https://github.com/waigore/colonizethis/issues/2071) guidance: avoid scattered direct dual-region field access (`RegionData.provinces` / `RegionData.units`) via `worldState.oldWorld` / `worldState.newWorld` in `colonizethis_world` and thin `colonizethis_logic` production sources (post-split scan roots; Refs #3290, #4660).
...
- **Canonical region updates:** Prefer `WorldState.updateRegionById(...)` (for one-region updates) and `WorldState.mapBothRegions(...)` / `WorldState.mapBothRegionUnits(...)` (for two-region updates) over inline `if (regionId == oldWorld)` or `copyWith(oldWorld: ...)/copyWith(newWorld: ...)` branching in scanned `lib/src/**`.
- **Region-scoped lookups:** Prefer `WorldState.provincesForRegion(regionId)` (canonical helper in `world/province_lookup.dart`) over hand-rolled `if (regionId == kRegionOldWorld) return ws.oldWorld.provinces` branching in scanned `lib/src/**`.
- **Elsewhere under** `packages/colonizethis_world/lib/src/**` and `packages/colonizethis_logic/lib/src/**`: prefer `allProvinces(world)` / `allUnits(world)` / `WorldStateUnitLookup` / `tryGetProvince` so dual-region iteration stays centralized.
...
| Scan roots | `packages/colonizethis_world/lib/src/` and `packages/colonizethis_logic/lib/src/` (non-generated `.dart` only; Refs #4660) |
| Excluded files | `packages/colonizethis_world/lib/src/world/province_lookup.dart`, `packages/colonizethis_world/lib/src/world/unit_lookup.dart` (and related province lookup modules listed in the checker) |
| Budget | At most **0** physical source lines (total across both roots) outside the excluded files may contain `oldWorld.provinces`, `newWorld.provinces`, `oldWorld.units`, `newWorld.units`, `copyWith(oldWorld: ...)`, `copyWith(newWorld: ...)`, or manual `if (regionId == kRegionOldWorld)` / `else if (regionId == kRegionOldWorld)` branching. |
...
- Refs #2836 AC 5 (this PR, follow-up): introduced canonical mutable-list helpers `WorldStateProvinceLookup.mutableProvinceListsByRegion()` and `WorldStateUnitLookup.mutableUnitListsByRegion()`. Migrated the remaining 15 mutable pre-stage sites. Lowered the budget from **15 → 0**.
- Refs #4660: extended scan roots to include `packages/colonizethis_logic/lib/src` (budget remains **0**); civilian helpers use `WorldStateUnitLookup` / `tryGetProvince`.

## Acceptance criteria

- Given the repository at `dev` with `packages/colonizethis_world/lib/src/**` and `packages/colonizethis_logic/lib/src/**` sources, when CI runs `dart run tool/ct_repo_lint.dart` including rule `repo.logic_dual_region_province_field_access`, then the checker counts matching lines outside `province_lookup.dart` and `unit_lookup.dart` across both scan roots and the run passes when the count is at most 0.
- Given a contributor adds a 1st matching line outside `province_lookup.dart` and `unit_lookup.dart` (in either scan root) without updating the budget in this SPEC and the checker constant, when repo lint runs, then the run fails and lists each `path:line` hit.
