# App core services file size (repo lint)

**SPEC/program** — physical line limit for hand-written Dart sources under
`app/lib/core/services/**`. Umbrella policy: `SPEC/program/repo-lint.md`
(**no violation allowlists**). Refs #4450 Slice A / AC3–AC4.

## Source of truth

| Artifact | Role |
|----------|------|
| `tool/check_app_core_services_file_size.dart` | Walker, counter, CLI |
| `tool/ct_repo_lint_manifest.yaml` | Registers rule `repo.app_core_services_file_size` |

## Scan scope

- Recursive listing under `app/lib/core/services/` (repository-relative).
...
- Given a temporary workspace that contains
  `app/lib/core/services/game_service/too_long.dart` with **301** physical lines
  ...
  `301 physical lines > 300`.

- Given a temporary workspace whose only matching file is
  `app/lib/core/services/game_service/ok.dart` with exactly **300** physical
...
- Given a temporary workspace that does not contain the directory
  `app/lib/core/services/`, when the System runs
  `runCheckAppCoreServicesFileSize`, then the checker exits non-zero and
  reports that the core services directory was not found.

- Given the repository root as cwd, when CI runs
  `dart run tool/ct_repo_lint.dart` and rule `repo.app_core_services_file_size`
