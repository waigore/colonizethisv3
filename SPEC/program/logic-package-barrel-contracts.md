...
- The rule carries an explicit set of enforced **consumer → target** boundaries. It covers `turn → {combat, diplomacy, economy, orders, world}`, `orders → {diplomacy, economy, world}`, `combat → {world}`, `economy → {world}`, and `logic → {orders, world}` — the `combat`/`economy`/`logic` boundaries migrated by the `combat/economy/logic → sibling` slice. It grows as later Phase 1 slices migrate the remaining `diplomacy`/`setup` sibling boundaries. Boundaries not yet migrated are intentionally **not** enforced so the rule stays green while Phase 1 lands incrementally.
