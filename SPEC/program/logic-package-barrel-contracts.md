...
- Single province-lookup surface (Refs #3403 Phase 1 Step 2): the `WorldStateProvinceLookup` extension methods on `WorldState` (`world.getProvince`, `world.tryGetProvince`, `world.getProvinceByRegion`, `world.tryGetProvinceByRegion`) are the canonical province-lookup API. The matching top-level standalone wrappers in `world/province_lookup.dart` are retained only as `@Deprecated` thin delegators to those extension methods for one release cycle before removal; semantics are identical. No `colonizethis_world/lib/**` file other than the canonical `world/province_lookup.dart` calls the deprecated wrappers — enforced by the `repo.world_no_top_level_province_lookup` repo-lint rule, which flags any unqualified `getProvince` / `tryGetProvince` / `getProvinceByRegion` / `tryGetProvinceByRegion` call in the world layer. External packages may keep using the deprecated wrappers during the deprecation window; their migration and the wrappers' removal are a later slice.
