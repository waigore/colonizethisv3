# World fog/capital/connectivity/ownership/player-view/travers tests — no raw Game/WorldState (repo lint)
...
Wave 5 (#4038) widens the same gate to ownership-transfer, player-view, and
province-traversal orphans after those suites call builders/`TestFixtures`
only. This keeps concern-scoped test files from re-introducing hand-rolled
...
`fog`, `capital`, `connectivity`, `ownership`, `player_view`, or `travers`,
excluding `world_test_support/`. Naval, army, and other world tests stay out of
scope until migrated (grandfather shrink-only only if temporarily required).

## Acceptance criteria

- Given an in-scope fog, capital, connectivity, ownership, player_view, or
  travers test file that contains a call `Game(...)` or `WorldState(...)`
  outside a line comment, when the checker runs, then the checker fails and the
  violation text names that file's repo-relative path.
