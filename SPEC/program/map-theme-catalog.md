...
Non-default terrain themes ship under `assets/themes/<theme_id>/` with their own tileset config JSON, Wang/transport atlas PNG+JSON pairs, and standalone L2+ feature tiles under a theme-specific `standalone_tile_prefix` (identical stems/filenames across themes). The `default` theme keeps the historical `assets/data/map_terrain_tilesets.json`, `assets/images/terrain/tilesets/`, and `assets/images/terrain/tile_` paths (identity with pre-theme goldens).
...
- Given `ActiveMapTheme` installed with a non-default terrain `standalone_tile_prefix`, when `terrainTileAssetPath` builds paths for L2+ feature stems, then each path uses that prefix under `assets/themes/<theme_id>/` (verified by path/assert test).
