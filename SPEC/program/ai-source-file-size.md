...
file under `packages/colonizethis_ai/lib/src/` at or below **450 physical
...
modules (declare-war suppression, research funding/decision, recruitment
ledger, treasury regiment bids, economy cargo) and ratcheted the ceiling
to **450** so those siblings cannot silently re-merge toward the prior
500 band. This gate mirrors the save/data shrink-only allowlist pattern.
AI is not an extracted logic-domain package, so it is not listed under
...
After #4079 Slice C / #4104 Slice B the production allowlist is empty.

## Acceptance criteria

- Given every hand-written file under `packages/colonizethis_ai/lib/src/**` is
  at or below 450 physical lines and the grandfather allowlist is empty, when
  the System runs `runCheckAiSourceFileSize`, then the System exits `0`.
- Given a hand-written AI `lib/src` file with more than 450 physical lines and
...
- Given a generated AI `lib/src` file (for example `*.g.dart`) over 450
