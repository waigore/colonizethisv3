...
file under `packages/colonizethis_ai/lib/src/` at or below **300 physical
...
**316** physical) and ratcheted the ceiling to **300** so additive planning
work retains durable headroom. This gate mirrors the save/data shrink-only
allowlist pattern. AI is not an extracted logic-domain package, so it is
not listed under `repo.domain_package_source_file_size` (still 500
elsewhere).
...
#4365 Slice A the production allowlist is empty.

## Acceptance criteria

- Given every hand-written file under `packages/colonizethis_ai/lib/src/**` is
  at or below 300 physical lines and the grandfather allowlist is empty, when
  the System runs `runCheckAiSourceFileSize`, then the System exits `0`.
- Given a hand-written AI `lib/src` file with more than 300 physical lines and
...
- Given a generated AI `lib/src` file (for example `*.g.dart`) over 300
