...
- **Resource rules (optional).** The generator accepts optional **resource rules** (e.g. `ResourceRules` from colonizethis_data). When **resource rules are provided**, Pass 6–7 run and the result includes `terrainGrid` and `resourceGrid`; when omitted, those grids are null and the map has no terrain/resource data. **For init_game and any flow that requires terrain and resources** (e.g. geographic view, extraction), the **caller must pass resource rules** (e.g. `ResourceRules.defaultRules`) so that the generated tile map includes terrain and per-cell resources.
...
- **Pass 7 (resources):** **Multi-region resource cap fraction** (default 0.30) — maximum fraction of placed resources that may be multi-region ("both") resources per map. No new CLI flags in MVP; implementation uses the default.
...
7. **Pass 7 — Resource assignment:** For each land cell with terrain, assign at most one resource from the allowed set for **map region** and terrain; weights per TDD 04b. All resources in `ResourceRules` with region and terrain eligibility participate; see [resource-terrain-region-rules.md](../game/resource-terrain-region-rules.md). **Multi-region resource cap:** Track both-count and total resource count. When both/total >= cap, and the cell's allowed set includes both "both" and region-only resources, restrict the draw to region-only before applying weighted pick. When only "both" resources are allowed on that terrain, place as normal (necessary overage).
