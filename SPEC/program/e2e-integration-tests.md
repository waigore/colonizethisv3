...
**Shared integration helpers:** `app/integration_test/e2e_test_shared.dart` — `e2eBootstrapNewGameToMap` / `e2eWaitForMapHudAfterNewGameStart`, `e2eWaitUntilFound`, `e2ePumpFor`, `e2eCollectTextPreorder`, manifest + parallel 64px PNG verification (`e2eDiscoverRelocated64pxPngAssets`, `e2eEnsureRelocated64pxPngDecode`, `e2eDecodePngAssetPathsParallel`), transient UI + bottom sheet helpers (`e2eDismissTransientUi`, `e2eCloseBottomSheet`, `e2eExpandEachExpansionTileOnce`, `kE2eDefaultBottomSheetCloseTimeout`). Scenarios should call these instead of duplicating new-game→map polling, per-file asset preload, or copy-pasted dismiss/close/expand logic (Refs GitHub #2336).
