...
**Shared integration helpers:** `app/integration_test/e2e_test_shared.dart` — `e2eBootstrapNewGameToMap` / `e2eWaitForMapHudAfterNewGameStart`, `e2eWaitUntilFound`, `e2ePumpFor`, `e2eDismissTransientUi`, `e2eCloseBottomSheet`, `e2eExpandEachExpansionTileOnce`, `e2eCollectTextPreorder`, manifest + parallel 64px PNG verification (`e2eDiscoverRelocated64pxPngAssets`, `e2eEnsureRelocated64pxPngDecode`, `e2eDecodePngAssetPathsParallel`). Scenarios should call these instead of duplicating new-game→map polling, transient overlay dismissal, bottom-sheet close, expansion-tile expansion, or per-file asset preload (Refs GitHub #2336).
