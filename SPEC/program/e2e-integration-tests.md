...
- **Adaptive poll pacing (Refs #2336):** Long-running `integration_test` busy-wait loops (new-game bootstrap, full-turn civilian/naval/production panel open, marker-panel open, bottom-sheet close, next-turn label polls) use **`e2eAdaptivePollRampAfterIdle`** in `e2e_test_shared.dart`—`pump` steps ramp **25→50→75→100 ms** like `e2eWaitUntilFound`—so Linux headless runs advance fewer idle frames per second while preserving the same hard timeout caps above.
