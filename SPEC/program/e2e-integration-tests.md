...
- **NW fleet e2e fail-fast waits:** Per-wait UI steps in `new_game_fleet_reaches_new_world_e2e_test.dart` fail past **5s**; **Start**→map HUD is capped at **60s**; total wall-clock from the post-`bootstrapForIntegrationTest` pump past **5 minutes** at checkpoints fails the run (PR runtime rule).
- **E2E idle polling (GitHub #2336 / AC5):** Shared `e2eNextIdlePollStepMs` in `integration_test/e2e_test_shared.dart` backs off idle `WidgetTester.pump` steps (25ms → 500ms cap) in the NW fleet helpers for `_bootstrapNewGameToMap` map-wait, `_openNavalPanel` / `_pollUntilNavalPanelVisible`, and `_closeBottomSheet` so headless Linux spends fewer fixed-interval frames while preserving the same hard timeout caps. The same library exposes `e2eWaitForNewGameEntry` so fleet scenarios wait for a hit-testable **New Game** label after `bootstrapForIntegrationTest` instead of a fixed post-bootstrap pump.
