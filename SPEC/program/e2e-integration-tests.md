...
**Shared integration helpers:** `app/integration_test/e2e_helpers.dart` exposes GitHub #2336 AC1 names (`pumpFor`, `waitUntilFound`, `dismissTransientUi`, `closeBottomSheet`, `bootstrapNewGameToMap`, `collectTextPreorder`, `expandEachExpansionTileOnce`, `ensureAllRelocated64pxPngsLoad`, plus `E2ePerfLog`) as thin delegates to `e2e_*` entrypoints in `app/integration_test/e2e_test_shared.dart`. Implementations and the full helper surface live in `e2e_test_shared.dart` — `e2eBootstrapNewGameToMap` / `e2eWaitForMapHudAfterNewGameStart`, `e2eWaitForNewGameEntry`, `e2eWaitUntilFound`, `e2ePumpFor`, `e2ePumpUntil` (condition-driven settle with the same backoff cap as `e2eWaitUntilFound`), `e2ePumpUntilFinderEmpty`, `e2ePumpUntilConditionOrIdle` (best-effort condition polls using `e2eAdaptivePollRampAfterIdle` pacing, no throw on expiry), `e2eOldWorldRegionChipAppearsSelected` / `e2eNewWorldRegionChipAppearsSelected` (map HUD region-tab selection probes for adaptive post-tap settle), `e2eCollectTextPreorder`, `e2eDismissTransientUi`, `e2eCloseBottomSheet`, `e2eOpenCivilianPanel` (empire rail / first civilian marker; adaptive settle after conflicting sheet close), `kE2eDefaultBottomSheetCloseTimeout`, `e2eExpandEachExpansionTileOnce`, `e2eWaitForNextTurnLabelAdvance`, manifest + parallel 64px PNG verification (`e2eDiscoverRelocated64pxPngAssets`, `e2eEnsureRelocated64pxPngDecode`, `e2eDecodePngAssetPathsParallel`). Scenarios should call these instead of duplicating new-game→map polling, transient overlay dismissal, bottom-sheet close, expansion-tile expansion, next-turn label polls, or per-file asset preload (Refs GitHub #2336).
