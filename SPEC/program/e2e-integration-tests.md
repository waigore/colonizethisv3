...
- **Adaptive poll pacing (Refs #2336):** Long-running `integration_test` busy-wait loops (new-game bootstrap, civilian / naval / production panel open, bottom-sheet close, Explore assign sweep, split dialog dismissal, next-turn label polls) use **`e2eAdaptivePollRampAfterIdle`** in `e2e_test_shared.dart`—`pump` steps ramp **25→50→75→100 ms** like `e2eWaitUntilFound`—and evaluate success **before** the first pump where helpers do so, so already-ready widgets short-circuit. Shared helpers also include `e2ePumpUntilFinderEmpty` (same pacing until a dismiss target finder clears, bounded timeout, no throw on expiry) and `e2eWaitForNewGameEntry` (hit-testable **New Game** after `bootstrapForIntegrationTest` instead of a fixed post-bootstrap pump). Fleet bundled-explore code (`_openCivilianPanelFleetE2e`, `_anyExplorerHasEnabledExploreAssignFleetE2e` in `new_game_fleet_reaches_new_world_e2e_helpers_part2.dart`) uses the same ramp for panel-open and **Explore** row polls. Hard timeout caps above are preserved on Linux headless runs.
