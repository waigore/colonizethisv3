...
- **Next-turn UI (two shells, #2277):** The Flame-canvas path logs **`logic: next_turn_ui …`** from `game_screen.dart` (`_runFlameCanvasNextTurn`). The **map** path logs the same lifecycle with token **`logic: next_turn_ui_map …`** from `GameMapArea` (`game_map_area_turn_resolution.dart`): **`started`**, **`processing_dialog_painted`** (ms after confirm), **`session_started`**, optional **`phase`** at **debug**, **`session_done`**, **`external_result_handled`**, optional **`worker_trace_export_path`**, **`result_applied`**, **`terminal_error`** on failure, and **`cleanup_complete`**. Use these lines to attribute main-isolate time to modal paint vs spawn serialization vs decode/apply per surface.
