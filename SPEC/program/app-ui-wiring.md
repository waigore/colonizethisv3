...
- Given debug console input submits `/get_tile_basic_info`, When command parsing succeeds, Then the panel appends a read-only message derived from `mapProvincePanelProvider.selectedTileKey`, emits no `SessionCommandEvent`, and does not trigger `GameService.saveGame`.
