...
Empire-rail surfaces (`GAME20001` Production, `GAME60001` Trade, `GAME80001` Development, `GAME30001` Diplomacy, `GAME40001` Technology, `GAME70001` Victory, `UNIT*` unit sheets, `GAME90001` Counsel from rail) share the same **1.0 s open-to-interactive** contract as map-attached overlays. **First open this session:** tap → interactive chrome + primary body ≤ 1 000 ms (profile/release on binding hosts). **Reopen same turn:** session read-model cache must avoid repeating first-open work (`productionPanelOpenPathProvider`, `developmentPanelSessionCacheProvider`, `unitsPanelSessionCacheProvider`, `diplomacyPanelRowsProvider`, `victoryPanelOpenPathProvider`, `technologyPanelSlotsOpenPathProvider`, and siblings). Static `{gameId, turnNumber, worldRevision}` identity is owned by `panelStaticSessionRevision` in `panel_session_revision.dart` (Refs #4734; [app-panel-static-session-revision.md](app-panel-static-session-revision.md)). **Slice 2 (Refs #4688):** `GAME20001` defers `rankIndustryCounselRecommendations` until after first paint; `GAME60001` uses `CtTabStrip.lazyTabBodies` so Deal Book is not built on Market-first open. **Slice 3 (Refs #4688):** `UNIT20001` / `UNIT30001` reuse military/naval tree projections via `unitsPanelSessionCacheProvider`; sheet dismiss unmounts the subtree and emits `UnitsPanelClosedEvent`. **Slice 4 (Refs #4688):** CI reopen-cache profiling anchors for military/naval tree session cache (`app/test/empire_rail_panel_open_path_timing_test.dart`; complements `packages/colonizethis_economy/test/economy/empire_rail_panel_open_path_timing_test.dart` production counsel deferral anchor). **Slice 5 (Refs #4688):** `GAME30001` / `GAME70001` session caches (`diplomacyPanelSessionCacheProvider`, `victoryPanelSessionCacheProvider`) and `CtAppPerf.*.interactiveReady` markers; `GAME40001` Research Slots tab marks `technology.interactiveReady` on first paint (Tree tab deferred until selected). **Slice 6 (Refs #4688):** `UNIT10001` empire-rail reopen reuses `resolveCivilianUnitsPanelOpenPath` via `unitsPanelSessionCacheProvider`; tile-scoped and role-filter shortcut opens bypass the cache. **Slice 7 (Refs #4688):** `GAME40001` Slots-tab preview bundle (`technologyPanelSlotsOpenPathProvider`) session-cached across reopen; `clearActiveGameSession` resets `technologyPanelSessionCacheProvider`. **Slice 8 (Refs #4688):** `GAME90001` defers off-tab counsel ranking until first tab selection; `counselPanelSessionCacheProvider` reuses per-tab projections across reopen; `CtAppPerf.counsel.*` markers. **Slice 9 (Refs #4688):** consolidated representative-fixture µs ratchets in `packages/colonizethis_economy/test/economy/empire_rail_panel_open_path_timing_test.dart` (production deferred + development lazy OW ceilings; consolidated sum tied to `kUiSurfaceOpenBudgetMs`). **Dispose:** pop/dismiss unmounts controllers and panel `FlameGame`s; `clearActiveGameSession` drops turn-dependent caches. DevTools markers: [flutter-performance-tracing.md](flutter-performance-tracing.md) § Empire-rail panel open path. Profile/release wall-clock evidence on binding hosts remains a verification deliverable; µs-ratio tests are not a debug wall-clock gate.
...
| `QuickStartNewGameEvent` | `ShellScreen` Quick Start | Composition-root `handleQuickStartNewGame` → `runNewGameSetupAfterLeaderPick` (shared template, `seed == 0`) |
...
Register core (game-feature) builders in **`app/lib/core/services/app_event_handler/app_event_handler_scope.dart`**. **Feature-layer dialog builders that would otherwise force `core/services/` to import `features/`** (for example the shell `new_game_leader_selection` dialog) live in their owning feature and are injected into the scope at the **composition root** (`app/lib/main.dart`) via **`AppEventHandlerScope.extraDialogBuilders`** — a `Map<String, NavigatorKeyDialogBuilder>` (each value a `DialogBuilder Function(GlobalKey<NavigatorState>)`) merged over the core builders by `OpenDialogEvent` id. **Feature-layer `UIActionEvent` handlers** that need the navigator key (for example **`QuickStartNewGameEvent`**) are injected the same way via **`AppEventHandlerScope.extraActionHandlers`** (`Map<Type, NavigatorKeyActionHandler>`); `AppEventHandler` dispatches unmatched action types to that map. The scope resolves each factory with `appNavigatorKey` so the feature threads the navigator key **explicitly** rather than reading the global; `appNavigatorKey` access stays confined to `core/services/` + `app.dart` (enforced by `repo.app_event_bus_decoupling`). This keeps `core/services/` free of `features/shell/` imports while preserving the single choke point for `navigatorKey` / `ProviderScope.containerOf` (Refs #3546, #4416).
...
**Tile context More dialog (`MAP30002`):** `GameMapTileRadialHost` uses local `showDialog` for the overflow list plus Province details after a map secondary gesture (see [SPEC/ui/tile-more-actions-dialog.md](../ui/tile-more-actions-dialog.md); Refs #4440). Same local-by-design family as MAP20001 Tile details. Does not replace `OpenDialogEvent` for cross-panel flows.

**Local by design (no `OpenDialogEvent`):** map display options (`GameMapArea`), **`ProvinceTileDetailsDialog`** (MAP20001 Tile teaching helper opened from transport-cluster tap or named **Tile details** action via local `showDialog`; see [SPEC/ui/province-sea-zone-detail-overlay.md](../ui/province-sea-zone-detail-overlay.md); Refs #4369), tech detail (`TechTreeWidget` node tap; **`showTechDefinitionDetailDialog`** from Choose-tech **Details** and Tree surfaces — shared Tree-parity content via local `showDialog`; Refs #4222), **`TechResearchersListDialog`** (opened from tech-tree / choose-tech GP pennant rows via local `showDialog` on long-press; see [SPEC/ui/components/gp-nation-color-pennant.md](../ui/components/gp-nation-color-pennant.md) and [SPEC/ui/tech-tree-widget.md](../ui/tech-tree-widget.md); Refs #3862), civilian work-target sheet (`CivilianUnitsPanel`), next-turn confirmation (`GameMapArea`), in-game Android back exit confirmation (`GameScreen`), research tech picker (`TechnologyPanel`), **`CtDropdown`** internal picker, **new-game setup progress and error dialogs** after leader confirmation (see **SPEC/ui/game-initializing.md**), **`TransferToHomeFleetDialog`** (opened from `NavalUnitsPanel` via local `showDialog` in `naval_units_panel_support_combine_home.dart` for the regular-fleet → Home Fleet ship merge flow, and from `MAP20001` via `showOverlayTransferToHomeFleetFlow` — optional `DLG31003` then `DLG40001`; commits via **`NavalTransferShipsRequestedEvent`**; see [SPEC/ui/transfer-to-home-fleet-dialog.md](../ui/transfer-to-home-fleet-dialog.md); Refs #4625), **`ProductionCommodityBreakdownDialog`** (opened from `ProductionScreen` via local `showDialog` in `production_screen_body_panel.dart` as a read-only commodity preview; see [SPEC/ui/production-commodity-breakdown-dialog.md](../ui/production-commodity-breakdown-dialog.md)), **`ResearchFundingBreakdownDialog`** (opened from the `TechnologyPanel` slot turn-preview view via local `showDialog` as a read-only RP/treasury funding breakdown, split out of `technology_panel.dart` to keep that file under the widget file-size cap; see [SPEC/ui/technology-panel.md](../ui/technology-panel.md) § Slot turn preview).
...
**Move fleet:** `NavalUnitsPanel` uses local `showDialog` for `MoveFleetDialog` via `showMoveFleetDialogForFleet` in `naval_mission_move_dialog.dart` (see [SPEC/ui/move-fleet-dialog.md](../ui/move-fleet-dialog.md)). That helper passes **`PlayerView`** (caller-supplied or `buildPlayerView`) so sea-zone rows can show fog-respecting hostile gist (Refs #4573). On confirm, emit **`NavalMoveFleetRequestedEvent`** → `AppEventHandlerScope` updates **`currentOrdersProvider`** via **`applyNavalMoveOrderForPlayer`** (replaces any prior naval move for that fleet and removes naval mission orders for that fleet from the draft). No merge callback into the dialog.

**Naval mission (draft wiring):** Map fleet markers emit **`OpenNavalMissionMenuEvent`**; the shell runs **`showNavalFleetMarkerFlow`** (Refs #4343, #4448, #4625): after optional fleet picker (`DLG31003`), a **non-empty Home Fleet** runs **`showHomeFleetDetachThenSailFlow`** (local Split then `DLG30001` for the new fleet from `NavalFleetsUpdatedEvent.game`); an **empty Home Fleet** emits tile-scoped **`OpenNavalUnitsPanelEvent`**; **sea-going in port at the capital** that is an eligible transfer source opens **`DLG31004`** then `DLG30001` or `DLG40001`; **other sea-going in port** opens local `MoveFleetDialog` (`DLG30001`); **sea-going at sea** continues into **`showNavalMissionFlow`**. `showNavalMissionFlow` uses local `showDialog` for optional fleet picker (`DLG31003`), mission menu (`DLG31001`, including **Sail / Move** → `DLG30001`), and target picker (`DLG31002`) — same local-by-design pattern as move fleet (see [SPEC/ui/naval-mission-menu-dialog.md](../ui/naval-mission-menu-dialog.md)). The flow forwards **`PlayerView`** (caller-supplied or `buildPlayerView` from map topology) into `DLG31002` for fog-respecting Beachhead/Blockade target intel (Refs #4340). `NavalUnitsPanel` opens `showNavalMissionFlow` from the Mission row. **`MAP20001` overlay Blockade / Beachhead (Refs #4413)** calls the same flow with `initialMission` + `initialTargetProvinceId` so `DLG31001` is skipped and `DLG31002` preselects the viewed province (multi-fleet → `DLG31003` of eligible fleets only). **`MAP20001` sea-zone overlay Patrol / Defend (Refs #4605)** calls the same flow with `initialMission` patrol or defend and no target id so `DLG31001` and `DLG31002` are skipped and the flow emits **`NavalMissionRequestedEvent`** (multi-fleet → `DLG31003` of eligible in-zone fleets only). Picker and target `showDialog` helpers live in `naval_mission_flow_support.dart`. Marker and panel Mission callers omit those args. On confirm, emit **`NavalMissionRequestedEvent`** → `AppEventHandlerScope` updates **`currentOrdersProvider`** via **`applyNavalMissionOrderForPlayer`** (replaces any prior naval mission for that fleet and removes naval move orders for that fleet from the draft). **Cancel pending** emits **`NavalMissionCancelRequestedEvent`** → **`removeNavalMissionOrderForPlayer`**. Human v1 missions: Patrol, Blockade, Beachhead, Defend (not Join Home Fleet UI). Availability and target gating use **`navalMissionAvailabilityForFleet`** in `colonizethis_orders` (Refs #4213).

**Army map markers:** Map army stack markers emit **`OpenArmyStackMarkerEvent`**. The shell mirrors fleet-marker observe gating: if `canMutateViaUi == false`, snackbar + return. Otherwise, field armies with cached destinations run **`showOverlayArmyMoveFlow`** with those ids only (never raw `armyIds` / never the Home Army id). A non-empty Home Army **in that stack** with no such field armies runs **`showHomeArmyDetachThenMoveFlow`** (local `showDialog` for Split then `DLG20001` for the new field army from `LandArmiesUpdatedEvent.game`; Refs #4407). Empty Home-Army-only emits **`OpenMilitaryUnitsPanelEvent`**. A dest-less field army away from the Home Army still uses overlay Move. Does not open `MAP20001`.

**Land armies (`MilitaryUnitsPanel`):** **Move army** uses local `showDialog` like naval move (see [SPEC/ui/move-army-dialog.md](../ui/move-army-dialog.md)) and confirms via **`ArmyMoveRequestedEvent`**. The dialog lists **only** destinations returned by **`colonizethis_logic`** as **order-engine-valid** for the current draft (visibility, adjacency, ownership, war/declare-war rules). The dropdown is **grouped by owning faction** with **player-owned** provinces first; **invasion** into another GP / Minor / Tribe without war uses a **second confirm** and sets optional **`declareWarTargetFactionId`** on the event so the shell merges **`declareWar`** and **`ArmyMoveOrder`** atomically. **`AppEventHandler`** opens the panel with **`ref.watch(currentOrdersProvider)`** and passes **`draftOrders`** into **`MilitaryUnitsPanel`** (naval parity for pending move lines). The scope handler applies the move (and declare war when present), runs **full draft validation** for that player; on failure it **does not** update `currentOrders`, logs **`error`**, emits **`ShowSnackBarEvent`**, and **asserts** in debug (per [orders.md](orders.md) shell contract). **Split** / **combine** use **`ArmySplitRequestedEvent`** / **`ArmyCombineRequestedEvent`** → handler mutates `Game`; panel refreshes from updated state. Same decoupling pattern as naval split (`NavalSplitFleetRequestedEvent`). **Home Army detach-then-move** (`showHomeArmyDetachThenMoveFlow`, overlay Military Move/Invade and army-stack marker; Refs #4407) uses the same local `showDialog` carve-out for Split then `DLG20001`.
...
- Given `MAP20001` Military **Train** is shown on the human capital, When the player taps it, Then the system emits `OpenDialogEvent(trainMilitaryDialogId)` with no params, does not emit `ClosePanelEvent` or `OpenMilitaryUnitsPanelEvent`, and the overlay stays mounted (Refs #4769).
...
- Given `MAP20001` Naval **Train** is shown on the human capital, When the player taps it, Then the system emits `OpenDialogEvent(trainNavalDialogId)` with no params, does not emit `ClosePanelEvent` or `OpenNavalUnitsPanelEvent`, and the overlay stays mounted (Refs #4776).
...
- Given `ShellScreen` emits `QuickStartNewGameEvent`, When `AppEventHandler` is bound with a composition-root `extraActionHandlers` entry for that type, Then the injected handler receives the same `navigatorKey` used for dialogs and `core/services/` does not import `features/shell/`.
