...
**Army map markers:** Map army stack markers emit **`OpenArmyStackMarkerEvent`**. The shell mirrors fleet-marker observe gating: if `canMutateViaUi == false`, snackbar + return. Otherwise, non-empty `fieldArmyIds` runs **`showOverlayArmyMoveFlow`** with those ids only (never raw `armyIds`); Home-Army-only emits **`OpenMilitaryUnitsPanelEvent`**. Does not open `MAP20001`.
