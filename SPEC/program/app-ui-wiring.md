...
**Naval mission (draft wiring):** `showNavalMissionFlow` uses local `showDialog` for optional fleet picker (`DLG31003`), mission menu (`DLG31001`), and target picker (`DLG31002`) — same local-by-design pattern as move fleet (see [SPEC/ui/naval-mission-menu-dialog.md](../ui/naval-mission-menu-dialog.md)). Map marker emits **`OpenNavalMissionMenuEvent`**; `NavalUnitsPanel` opens the same flow from the Mission row. The flow forwards **`PlayerView`** (caller-supplied or `buildPlayerView` from map topology) into `DLG31002` for fog-respecting Beachhead/Blockade target intel (Refs #4340). On confirm, emit **`NavalMissionRequestedEvent`** → `AppEventHandlerScope` updates **`currentOrdersProvider`** via **`applyNavalMissionOrderForPlayer`** (replaces any prior naval mission for that fleet and removes naval move orders for that fleet from the draft). **Cancel pending** emits **`NavalMissionCancelRequestedEvent`** → **`removeNavalMissionOrderForPlayer`**. Human v1 missions: Patrol, Blockade, Beachhead, Defend (not Join Home Fleet UI). Availability and target gating use **`navalMissionAvailabilityForFleet`** in `colonizethis_orders` (Refs #4213).
