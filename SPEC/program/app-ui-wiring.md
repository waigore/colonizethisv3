...
Empire-rail surfaces (`GAME20001` Production, `GAME60001` Trade, `GAME80001` Development, `GAME30001` Diplomacy, `GAME40001` Technology, `GAME70001` Victory, `UNIT*` unit sheets, `GAME90001` Counsel from rail) share the same **1.0 s open-to-interactive** contract as map-attached overlays. **First open this session:** tap → interactive chrome + primary body ≤ 1 000 ms (profile/release on binding hosts). **Reopen same turn:** session read-model cache must avoid repeating first-open work (`productionPanelOpenPathProvider`, `developmentPanelSessionCacheProvider`, and siblings). **Slice 2 (Refs #4688):** `GAME20001` defers `rankIndustryCounselRecommendations` until after first paint; `GAME60001` uses `CtTabStrip.lazyTabBodies` so Deal Book is not built on Market-first open. **Dispose:** pop/dismiss unmounts controllers and panel `FlameGame`s; `clearActiveGameSession` drops turn-dependent caches. DevTools markers: [flutter-performance-tracing.md](flutter-performance-tracing.md) § Empire-rail panel open path. CI profiling anchors: `empire_rail_panel_open_path_timing_test.dart` (µs ratios; not a debug wall-clock gate).
