# Models package — no `part` / `part of` directives (repo lint)

**SPEC/program** — repository lint gate that forbids Dart `part` / `part of`
file-splitting in `packages/colonizethis_models/lib/`.

## Motivation

`packages/colonizethis_models` previously kept size under the 500 non-comment
line gate via `part` / `part of` for `orders`, `world_state`, `world_market`,
and `app_events` (Refs #3393 Phase 5). Per #4068 Slice C those fragments were
converted into proper libraries with explicit imports. The `part of` pattern
couples every fragment to a single library's implicit namespace and hides
cross-file symbol visibility. This gate keeps any new sub-file in the models
package a proper library with explicit imports, matching the equivalent gates
for `colonizethis_world` (`repo.world_no_part_directives`),
`colonizethis_turn` (`repo.turn_no_part_directives`),
`colonizethis_diplomacy` (`repo.diplomacy_no_part_of`),
`colonizethis_orders` (`repo.orders_no_part_directives`), and
`colonizethis_economy` (`repo.economy_no_part_directives`).

## Source of truth

| Artifact | Role |
|----------|------|
| `tool/check_models_no_part_directives.dart` | Checker and CLI entrypoint |
| `tool/ct_repo_lint_manifest.yaml` (`repo.models_no_part_directives`) | Rule registration |
...
`packages/colonizethis_models/lib/`. Generated Dart and test files stay
...
- Given a Dart file under `packages/colonizethis_models/lib/` that contains a
...
- Given a Dart file under `packages/colonizethis_models/lib/` that contains a
  ...
  the violation text names that file's repo-relative path and the 1-based line
  number of the directive.
- Given a Dart file under `packages/colonizethis_models/lib/` that contains no
  `part` / `part of` directive, when the checker runs, then the checker does
  not fail for that file.
- Given a Dart file outside `packages/colonizethis_models/lib/` that contains a
