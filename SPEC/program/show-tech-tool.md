...
- After editing `packages/colonizethis_data/lib/src/data/tech_effect_summary.yaml`, run `dart tool/generate_tech_effect_l10n.dart` from the repo root, then `cd app && flutter gen-l10n`, so `app/lib/l10n/arb/app_en.arb`, `tech_effect_summary_lookup.dart`, and its generated `tech_effect_summary_lookup_entries_*.dart` parts stay aligned with the YAML.
