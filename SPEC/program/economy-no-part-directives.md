# Economy package — no `part` / `part of` directives (repo lint)

**SPEC/program** — repository lint gate that forbids Dart `part` / `part of`
file-splitting in `packages/colonizethis_economy/lib/`.

## Motivation

`packages/colonizethis_economy` previously split the world-market
`DealMatcher` into `part of` fragments (`deal_matcher_matching.dart`,
`deal_matcher_indexing.dart`). Per #3979 those fragments were converted into
proper libraries with explicit imports and a shared `DealMatchSession` bag.
...
sub-file in the economy package a proper library with explicit imports,
matching the equivalent gates for `colonizethis_world`
(`repo.world_no_part_directives`), `colonizethis_turn`
(`repo.turn_no_part_directives`), `colonizethis_diplomacy`
(`repo.diplomacy_no_part_of`), and `colonizethis_orders`
(`repo.orders_no_part_directives`).

## Source of truth

| Artifact | Role |
|----------|------|
| `tool/check_economy_no_part_directives.dart` | Checker and CLI entrypoint |
| `tool/ct_repo_lint_manifest.yaml` (`repo.economy_no_part_directives`) | Rule registration |
...
`packages/colonizethis_economy/lib/`. Generated Dart and test files stay
...
- Given a Dart file under `packages/colonizethis_economy/lib/` that contains a
...
- Given a Dart file under `packages/colonizethis_economy/lib/` that contains a
  ...
  the violation text names that file's repo-relative path and the 1-based line
  number of the directive.
- Given a Dart file under `packages/colonizethis_economy/lib/` that contains no
  `part` / `part of` directive, when the checker runs, then the checker does
  not fail for that file.
- Given a Dart file outside `packages/colonizethis_economy/lib/` that contains a
