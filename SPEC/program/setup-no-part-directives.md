# Setup package — no `part` / `part of` directives (repo lint)

**SPEC/program** — repository lint gate that forbids Dart `part` / `part of`
file-splitting in `packages/colonizethis_setup/lib/`.

## Motivation

`packages/colonizethis_setup` previously split several libraries into
`part of` fragments: ownership paint/packing/remainder, create ownership /
initial-game / capitals, capital-choice tile scan / port-road geometry,
GP Old World resource redistribution tile scans / quota placement, and the
locked province assigner graph / engine. Per #4086 those fragments were
converted into proper libraries with explicit imports. The `part of` pattern
couples every fragment to a single library's implicit namespace, hides
cross-file symbol visibility, and makes individual helpers harder to test in
isolation. This gate keeps any new sub-file in the setup package a proper
library with explicit imports, matching the equivalent gates for
...
| Artifact | Role |
|----------|------|
| `tool/check_setup_no_part_directives.dart` | Checker and CLI entrypoint |
| `tool/ct_repo_lint_manifest.yaml` (`repo.setup_no_part_directives`) | Rule registration |
...
`packages/colonizethis_setup/lib/`. Generated Dart (for example `*.g.dart`)
...
- Given a Dart file under `packages/colonizethis_setup/lib/` that contains a
...
- Given a Dart file under `packages/colonizethis_setup/lib/` that contains a
  ...
  the violation text names that file's repo-relative path and the 1-based line
  number of the directive.
- Given a Dart file under `packages/colonizethis_setup/lib/` that contains no
  `part` / `part of` directive, when the checker runs, then the checker does
  not fail for that file.
- Given a Dart file outside `packages/colonizethis_setup/lib/` that contains a
