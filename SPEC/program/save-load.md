...
- **Round-trip map view state fields.** Given a Game with `mapViewState.zoomMultiplier`, `mapViewState.showProvinceOverlay`, `mapViewState.showProvinceOwnershipTint`, `mapViewState.showProvinceNamesLayer`, `mapViewState.showCapitalLinkDisconnectedHighlight`, `mapViewState.showPlayerTurnEventsFeed`, `mapViewState.showPlayersBar`, `mapViewState.showMapResources`, `mapViewState.showMapImprovements`, and `mapViewState.showMapRoads` set to any values (including model defaults), when the system saves and then loads by the same game id, then the loaded Game preserves those map view fields exactly. `Game.toJson()` always writes `mapViewState` (including when equal to `MapViewState.defaults`) so preferences such as `showPlayersBar` are explicit on disk.
- **Legacy save default map view state.** Given a current-envelope save payload where `mapViewState` is absent (legacy save), when the system loads the game, then the loaded Game uses model default map view state values (`zoomMultiplier = 1.0`, `showProvinceOverlay = true`, `showProvinceOwnershipTint = false`, `showProvinceNamesLayer = true`, `showCapitalLinkDisconnectedHighlight = true`, `showPlayerTurnEventsFeed = false`, `showPlayersBar = true`, `showMapResources = true`, `showMapImprovements = true`, `showMapRoads = true`) and gameplay remains functional. These model defaults are the legacy/missing-field fallback only; **new campaigns** created through standard game setup initialize `showPlayersBar = false` (see `SPEC/ui/empire-overview.md` § Players bar) while keeping the same legacy load fallback. Missing information-layer fields inside an otherwise present `mapViewState` also load as all-on. Missing `showCapitalLinkDisconnectedHighlight` inside a present `mapViewState` loads as **ON** (Refs #4370).
