...
- Given `app/lib/features/**/*.dart` files (excluding generated suffixes, `_test.dart`, paths under `test/`, the Ct-* catalog directory `app/lib/features/game/widgets/chrome/**`, and the canonical Flame-renderer / palette-data / dev-tooling file allowlist documented in this rule's table row), when `dart run tool/ct_repo_lint.dart` runs rule `repo.app_editorial_monocle_colors`, then no scanned line outside `//` comments contains a banned Material color literal `Colors.{black,white,red,redAccent,green,greenAccent,grey,gray}` (with any opacity / grade suffix) or a `const Color(0x...)` hex literal, and the run fails listing every offending `file:line: literal` when any line does (Refs #2914). The checker uses only whole-file scope exclusions; it does not load keyed waiver data.
