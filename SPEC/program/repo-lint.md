...
| `tool/check_dart_file_non_comment_line_size.dart` | Enforce repository-wide Dart files at **1000 non-comment lines or fewer** (fail when strictly greater), excluding generated suffixes (`.g.dart`, `.freezed.dart`, `.mocks.dart`, `.gen.dart`) and the tracked l10n abstract API `app/lib/l10n/app_localizations_contract.dart` (Refs #2021 split); rule `repo.dart_file_non_comment_line_size` |
...
- Given repository Dart files (excluding generated suffixes `.g.dart`, `.freezed.dart`, `.mocks.dart`, `.gen.dart`, and excluding `app/lib/l10n/app_localizations_contract.dart` per `tool/check_dart_file_non_comment_line_size.dart`), when `dart run tool/ct_repo_lint.dart` runs rule `repo.dart_file_non_comment_line_size`, then each scanned file has at most 1000 non-comment lines and the run fails while listing every violating file when any file is strictly greater than 1000.
