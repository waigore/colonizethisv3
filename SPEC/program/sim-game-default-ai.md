...
`defaultSimGameAi(game, player, topology, baseSeed) -> Orders`

- Input: current `game`, one GP `player`, `topology`, and `baseSeed` fallback when `aiSeedByGpId[player.id]` is missing.
...
- Build [PlayerView](player-view.md) and call the order suggestion API for candidate move/work/build/research orders.
- Apply category order: moves → work → build → research; use seeded choice within category.
...
- Category order and diplomacy post-filter are applied; full province ids are used for owner lookup so moves to at-peace GPs or unknown-relation minors are dropped.
