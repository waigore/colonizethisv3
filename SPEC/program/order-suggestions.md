...
- **Suggested diplomatic orders per target:** For each known target **T**, the implementation walks candidates in fixed priority order: offerPeace → alliance → establishOverture → **grantAid** → **setSubsidy** → declareWar. For each candidate in that order, if appending it to the **working** order list (starting from `currentOrders`, then including every suggestion already accepted earlier in this same invocation) passes the order engine, the candidate is appended to **L** and to the working list; otherwise it is skipped. **Grant before subsidy** when both are valid: grant appears earlier in the priority list, so both may appear toward the same **T** when rules allow. Multiple entries in **L** may share the same `targetFactionId` only as allowed by [orders.md](orders.md) (e.g. one grantAid + one setSubsidy toward **T**).
- **Draft list (`currentOrders`):** The suggester uses `currentOrders` as the initial working list. It does **not** skip a target **T** solely because **T** already has a pending economic order; it may still propose the complementary economic type when valid. Pending non-economic orders toward **T** block further suggestions toward **T** because validation fails.
...
- Given `currentOrders` includes only a `grantAid` from P to T (and no non-economic order toward T), when `suggestDiplomaticOrders` runs, then L **may** include a `setSubsidy` toward T when valid under the order engine.
