...
- **Selected-unit availability (`getAvailableWorkTargetsForUnit`, `AvailableWorkTargetsForUnit`):** Human-shell paths that only need **one unit’s** assignable work targets (civilian units panel, explorer/builder shortcut assign enablement, unit-scoped pickers) **must** use `getAvailableWorkTargetsForUnit` (or providers derived from it). They **must not** call broad `suggestWorkOrders` for that use case. The function returns `assignable`, optional `blockedReason`, and `validTileKeysByTarget` keyed by work target id; when the unit has **any** pending draft `WorkOrder` for the player, it returns `assignable=false` with `blockedReason=pending_draft_work` and performs **no** per-tile order-engine probing for that unit (Refs #2133).
- **`getValidWorkOrderTileKeysWithVisibility` with pending draft:** If the unit already has a pending draft `WorkOrder` whose `target` differs from the requested `workTarget`, the helper returns an empty set **without** running order-engine acceptance probes for candidate tiles (Refs #2133). When the pending order’s `target` equals the requested `workTarget`, enumeration proceeds so the same unit may amend the pending tile choice while preserving dev-exclusive reservation semantics (own pending omitted from the reserved set per § Dev-exclusive tile reservations).
...
- **Relationship to `availableWorkTargetsProvider`:** That provider aggregates **`suggestWorkOrders`** (multi-unit suggestions). Shortcut **enablement** does **not** reimplement exclusivity rules; it aligns with **per-unit** valid tile keys from **`getValidWorkOrderTileKeysWithVisibility`**, the same entrypoint used when the shell enters tile-target selection after choosing an order. Full rule coverage stays in **`colonizethis_logic`** tests (e.g. `order_engine_validate_work_build_improvement_test.dart`, `work_order_target_prechecks_test.dart`, `order_suggestion_valid_work_tiles_test.dart`).
