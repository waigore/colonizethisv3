...
- **Selected-unit availability (`getAvailableWorkTargetsForUnit`, `AvailableWorkTargetsForUnit`):** Human-shell paths that only need **one unit’s** assignable work targets **must** use `getAvailableWorkTargetsForUnit` (or providers derived from it), not broad `suggestWorkOrders`. When the unit has **any** pending draft `WorkOrder` for the player, the API returns `assignable=false` with `blockedReason=pending_draft_work_order` and performs **no** per-tile order-engine probing for that unit (Refs #2133).
- **`getValidWorkOrderTileKeys` / `getValidWorkOrderTileKeysWithVisibility` with pending draft:** If the unit already has a pending draft `WorkOrder` whose `target` differs from the requested `workTarget`, the helper returns an empty set **without** running order-engine acceptance probes for candidate tiles. When the pending order’s `target` equals the requested `workTarget`, enumeration proceeds so the same unit may amend the pending tile choice while preserving dev-exclusive reservation semantics (own pending omitted from the reserved set per § Dev-exclusive tile reservations) (Refs #2133).
...
- Given a civilian unit with a pending draft `WorkOrder` for that `unitId` on the current turn, when `getAvailableWorkTargetsForUnit` runs for that unit, then the system returns not assignable with empty `validTileKeysByTarget` and performs no order-engine candidate-tile probing attributable to that unit for that call.
- Given the same unit has pending draft work whose `target` differs from `workTarget`, when `getValidWorkOrderTileKeysWithVisibility` runs for that `unitId` and `workTarget`, then the system returns an empty tile set without order-engine candidate-tile probing attributable to that unit for that call.
