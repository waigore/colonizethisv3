...
- **Primary vs economic suggestions per target:** For each `targetFactionId`, candidates are ordered as in `_diplomaticCandidatesForTargetOrdered` (roughly: `offerPeace`, `alliance`, `establishOverture`, then `grantAid`, `setSubsidy`, then `declareWar`). The implementation uses **two passes** on that list: (1) walk in order considering **only** non-economic types; append the **first** that passes the order engine against a **trial** `Orders` (starting from `currentOrders`) and stop the primary pass. (2) Walk again in order considering **only** `grantAid` and `setSubsidy`; append each that passes against the trial list after step (1), updating the trial after each acceptance. This keeps primary ordering ahead of economic in the candidate list (e.g. `declareWar` before `grantAid` in the template list) while still allowing both economic orders when the engine allows (same cap exception as [orders.md](orders.md) § diplomatic orders).
- **Draft list (`currentOrders`):** Each candidate is validated with the order engine against the **current** merged `currentOrders` (and the trial list built while adding multiple suggestions for the same target as above). Pending orders in `currentOrders` constrain what can be suggested; removing a pending order restores eligibility per engine validation.

**Acceptance criteria (diplomatic suggestions)**

- Given fixed `Game`, `MapTopology`, `PlayerView`, and `Orders` for player P, when `suggestDiplomaticOrders` returns a list L, then for every order o in L, when o is appended to P’s diplomatic orders in `currentOrders` in the **same order as L** (and prior suggestions from L for the same target already appended when required by the implementation), running `validatePlayerOrdersWithContext` for P yields **accepted** for the full merged diplomatic list.
...
- Given `currentOrders` includes no diplomatic order to T, when a prior call returned a suggestion toward T and the player removed all diplomatic orders to T from the draft, when `suggestDiplomaticOrders` runs again with the updated `currentOrders`, then the system **may** again include valid suggestions toward T subject to game rules and engine validation.
