...
- **`features/game/flame/map_state/`** — `GameMapArea` widget stack, selection, province actions, turn-feed hooks, and draft projections. `game_map_area.dart` library parts: `game_map_area_build_overlays.dart` (map canvas stack + in-map overlay chrome), `game_map_area_build.dart` (controls bar + play-area shell). Public barrel: `map_state/map_state.dart`.
