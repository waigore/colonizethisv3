...
- **`features/game/screens/trade/`** — World Market trade screen library ([TradeScreen](../../app/lib/features/game/screens/trade/trade_screen.dart) with `part` files for widget-test contract keys/copy (`trade_screen_contract.dart`), Market tab body/handlers (`trade_screen_market_tab.dart`, `trade_screen_market_tab_handlers.dart`), Market row chrome (`trade_screen_market_row.dart`), and Deal Book ledger (`trade_screen_deal_book.dart`)), plus shared Market-tab section callback factory ([trade_section_handlers.dart](../../app/lib/features/game/screens/trade/trade_section_handlers.dart)). Public imports use full package paths; no barrel re-export shim.
