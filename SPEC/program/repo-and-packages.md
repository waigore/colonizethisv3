...
- **`TurnResolutionRunner`** (`app/lib/core/services/turn_resolution_runner.dart`) runs Full AI + merge + resolver in a worker isolate. Library parts: `turn_resolution_runner_types.dart` (progress/terminal event types and session handle) and `turn_resolution_runner_isolate_listener.dart` (SendPort message decode and lifecycle teardown).
