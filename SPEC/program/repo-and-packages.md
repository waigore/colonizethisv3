...
- **`features/game/widgets/panels/`** — in-game order panels: civilian, military, and naval unit panels (with `part` support files), pause menu, observe-mode sentinel, and shared [GamePanelMixin](../../app/lib/features/game/widgets/panels/game_panel_contract.dart) contract. Diplomacy panel remains at `features/game/widgets/diplomacy_panel.dart` (imports `panels/game_panel_contract.dart`). Public imports use full package paths; no barrel re-export shim.
