...
- **`features/game/flame/region_map/`** — `RegionMapComponent` Flame render stack and viewport helpers. `region_map_component.dart` library parts split shared constants/plate layout (`region_map_component_shared.dart`) from visibility, fog, label, and overlay helpers (`region_map_component_shared_visibility*.dart`); unit marker paint (`region_map_component_render_markers_units.dart`); capital/town/port paint (`region_map_component_render_markers_settlements_towns.dart`) from warp-zone edges (`region_map_component_render_markers_settlements_warp.dart`); province label compute vs paint (`region_map_component_render_political_labels_province_compute.dart`, `region_map_component_render_political_labels_province_paint.dart`). Public barrel: `region_map/region_map.dart`.
