...
4. **Minor/tribe sell-priority relation tiebreaker (#3753 R7.3).** For an offer whose seller is a **Minor or Tribe**, the buyer bids competing for that offer within the same integer priority tier are ordered by the buyer's **relation score with that selling Minor/Tribe**, highest first. The tiebreaker applies only to buyers that hold a **Consulate (or higher) overture** with the seller; consulate-less buyers keep the default ordering and sort **after** every consulate-holding buyer. Relation-score ties (R9, one decimal place) break by ascending `submitterId` (then ascending order id). This tiebreaker applies **only** to Minor/Tribe sellers; **GP sellers keep the existing ordering**. It sits below the integer priority tiers (priority is absolute) and never overrides first right of refusal (item 1).
5. **Submitter id (deterministic).** Equal-priority, equal-FTP, equal-relation entries sort by ascending `submitterId` then by ascending order id.
...
#### Subsidy price adjustment (#3753 R3.4)

`applyDealsToPlayers` accepts an optional `subsidyPercentByPayerTargetKey` map keyed by the **directed** subsidy pair `'<payerId>>'<targetId>'` (built by the phase handler from `Game.subsidyStates`; `payerId` is always a Great Power and `targetId` a Minor/Tribe per [diplomacy.md](../game/diplomacy.md) § Diplomatic Order Types). For each `FilledDeal`, the settled per-unit price for the **Great-Power side's** transfer is adjusted by the subsidy percentage `p` when the deal is between the subsidising GP and the subsidised Minor/Tribe (empty map = legacy behaviour, no adjustment):

- **Surcharge (R3.4b):** when key `'<buyerId>>'<sellerId>'` is present (the GP buyer subsidises the Minor/Tribe seller), the GP buyer's treasury **debit** becomes `round(quantity × pricePerUnit × (1 + p/100))`.
- **Discount (R3.4a):** when key `'<sellerId>>'<buyerId>'` is present (the GP seller subsidises the Minor/Tribe buyer), the GP seller's treasury **credit** becomes `round(quantity × pricePerUnit × (1 − p/100))`.

The adjustment is applied to the GP-side transfer only and is **independent of** the overseas profit-share credits (R8), which always use the unadjusted `pricePerUnit`. The surcharge difference for an R3.4b deal leaves the Great-Power pool through the Minor/Tribe treasury sink (no faction is credited the surcharge), preserving the [treasury conservation invariant](#treasury-conservation-invariant-refs-2924).
...
- **Tile-owner full share (#3753 R8.2):** if the deal is attributed to a purchased tile and the buyer is not the tile owner, credit the owning GP by `quantity × pricePerUnit × (relationScore / 100)` (full relation-linear share, no 40% cap) and deduct only this amount from the seller's proceeds.
- **Embassy kickback (#3753 R8.3):** additionally credit every GP that holds an Embassy with the selling minor/tribe and does not own the sourcing tile by `quantity × pricePerUnit × (relationScore / 100) × 0.10`; these kickbacks are funded from the sink remainder (not deducted from seller proceeds).
- Remove the remaining sale value from the economy. No other faction is credited.
...
typedef DealMatchInputs = ({
  ...
  // relation score with that seller. Empty map disables the tiebreaker
...
4. Inside each tier, run two passes:
   ...
   - **Pass 2 — Any.** Iterate the remaining offers and bids (those with `remaining > 0`) and attempt matches regardless of FTP. For an offer whose `sellerFactionId` appears in `sellPriorityRelationByMinorTribeSeller`, the bid iteration order for that offer is rebuilt per the #3753 R7.3 sell-priority relation tiebreaker: consulate-holding buyers (those present in the seller's relation map) are placed first, sorted by descending relation score (ties by ascending `buyerFactionId` then ascending faction-local index), followed by the remaining bids in their default order. Offers from sellers absent from the map (all GP sellers and minor/tribe sellers with no consulate-holding buyer) keep the default `(buyerFactionId, faction-local index)` order.
...
- **Subsidy surcharge on GP buyer (#3753 R3.4b).** Given an active `SubsidyState { payerId: 'gpA', targetId: 'm1', percent: 10 }`, a `FilledDeal(seller: 'm1', buyer: 'gpA', quantity: 10, pricePerUnit: 20.0)`, and `subsidyPercentByPayerTargetKey` carrying `'gpA>m1': 10`, when `applyDealsToPlayers` runs, then `gpA`'s treasury is debited by exactly `round(10 × 20 × 1.10) = 220` (base `200` plus the `20` surcharge), the seller `m1` is credited nothing (treasury sink), and the surcharge is not credited to any faction.
- **Subsidy discount on GP seller (#3753 R3.4a).** Given an active `SubsidyState { payerId: 'gpA', targetId: 'm1', percent: 20 }`, a `FilledDeal(seller: 'gpA', buyer: 'm1', quantity: 10, pricePerUnit: 20.0)` (the Minor buys from the subsidising GP), and `subsidyPercentByPayerTargetKey` carrying `'gpA>m1': 20`, when `applyDealsToPlayers` runs, then `gpA`'s treasury is credited by exactly `round(10 × 20 × 0.80) = 160` rather than the full `200`.
- **Subsidy adjustment scoped to subsidised pair (#3753 R3.4).** Given `subsidyPercentByPayerTargetKey` carrying `'gpA>m1': 10` and a `FilledDeal(seller: 'm2', buyer: 'gpA', quantity: 10, pricePerUnit: 20.0)` where `m2 != m1`, when `applyDealsToPlayers` runs, then `gpA`'s treasury is debited by exactly `200` (the unadjusted notional — no subsidy applies to a deal with a non-subsidised counterparty).
