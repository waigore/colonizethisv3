...
- **primaryType**: enum describing the main event (e.g. `declareWar`, `peace`, `allianceFormed`, `allianceBroken`, `overtureAccepted`, `overtureRejected`, `joinEmpireResolved`, `grantAidApplied`, `subsidySet`, `subsidyUpdated`, `subsidyCancelled`, `boycottSet`, `boycottRevoked`, `interventionIntervene`, `interventionProtest`, `interventionDoNothing`, `agreementsClearedOnWar`, `callToArmsAccepted`, `callToArmsRefused`).
...
3b. **Process boycotts and revocations (Refs #3753 R6)** — Runs after Join Empire/Colony so colonies resolved this turn are visible. For each `boycott` order whose issuer holds at least one `ColonyState` (`colonyOfGpId == issuer`), whose target is **another** Great Power at peace with the issuer, and for which no `(issuer, target)` `BoycottState` already exists: record a `BoycottState { gpId, targetGpId, sinceTurn }` on `Game.boycottStates`, append a `boycottSet` event, and **cancel any active `SubsidyState` from the target GP to a Tribe that is a colony of the issuer** (append `subsidyCancelled` for each). For each `revokeBoycott` order whose issuer holds an active `(issuer, target)` `BoycottState`: remove it and append a `boycottRevoked` event. Orders that no longer satisfy preconditions at resolution are ignored (validation already rejects them).
...
6. **Terminate agreements on war** — Clear overtures between warring pairs; **auto-cancel any `BoycottState` whose `(gpId, targetGpId)` pair is now `AT_WAR`** (append `boycottRevoked`; the war rules already block trade, Refs #3753 R6.4); further steps per implementation (subsidies, grants, decay) follow the live resolver order in `diplomacy_resolver.dart`.
...
7b. **Apply trade-deal relation boost** — Before decay, every canonical pair key on `Game.worldMarketState.completedTradePairKeys` (set by the previous turn's World Market phase, Step F) that is **not** `AT_WAR` gains **`+2.0`** plus **`+0.4`** when an Embassy is in effect between the parties, applied **once per pair**, clamped 0–100. Because this runs after the phase-start snapshot, boosted pairs are event-modified and skip decay this turn. See [diplomacy.md](../game/diplomacy.md) § Relation Model — Trade-deal relation boost (Refs #3753 R10).
...
- When a **subsidy** is created, updated, or cancelled (including automatic cancellation on war or insufficient funds, or cancellation of a target GP's subsidy to a boycotted colony, Refs #3753 R6.2), events with primaryType `subsidySet`, `subsidyUpdated`, or `subsidyCancelled` are appended.
- When a **boycott** is set or removed (voluntary `revokeBoycott` or auto-cancel on war, Refs #3753 R6), events with primaryType `boycottSet` or `boycottRevoked` are appended.
