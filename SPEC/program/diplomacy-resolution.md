...
7. **Apply relation modifiers** — From trade, grants (GrantAid), war, broken treaties per game rules. **SetSubsidy (Refs #3753 R3 — percentage model):** valid only GP→Minor/Tribe with an **Embassy** (R2 — Consulate alone is insufficient) and a valid percentage (5–20, step 5, carried in `amount`). The resolver stores/updates `SubsidyState { payerId, targetId, percent }` and appends `subsidySet`/`subsidyUpdated`; it deducts **no** treasury (no upfront or per-turn payment). The subsidy's effects are the world-market price discount/surcharge and the scaled trade-deal relation boost (R10). Ongoing subsidies are cleared (with `subsidyCancelled`) when the pair goes `AT_WAR` or the payer loses the Embassy with that Minor/Tribe (R3.5).
...
- When a **subsidy** is created, updated, or cancelled (including automatic cancellation on war or loss of the Embassy with that Minor/Tribe, Refs #3753 R3.5), events with primaryType `subsidySet`, `subsidyUpdated`, or `subsidyCancelled` are appended.
...
**Economy:** GrantAid deducts payer treasury (one-time transfer). SetSubsidy charges **no** treasury (percentage model, Refs #3753 R3); it records the percent on `SubsidyState`. **`tradeSlotsForGp`** returns commodity capacity for trade agreements: **0** without embassy toward the target, **3** with embassy (baseline), **6** with embassy when the ordering GP has **`trade_fairs`** unlocked. War terminates trade agreements with target. See [tech-tree-labour-economy.md](../game/tech-tree-labour-economy.md) § trade_fairs.
...
- **GrantAid / SetSubsidy:** GrantAid requires Embassy and deducts payer treasury. SetSubsidy (Refs #3753 R3) is GP→Minor/Tribe only, requires an **Embassy**, takes a percentage (5–20, step 5), and deducts **no** treasury; it persists `SubsidyState.percent` and is cleared on war or Embassy loss.
