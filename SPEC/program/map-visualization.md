...
For interactive **human army** map icons (province town tiles), `RegionMapViewData` carries `armyTileMarkers[]` distinct from ctdev `unitMarkers`. Each marker is one human stack at a stationed province: `tileKey`, `x`, `y`, prefixed `provinceId`, `armyIds` (Home Army included when present, including zero-regiment Home Army), `fieldArmyIds` (non-Home only), `stackCount == armyIds.length` (presence cardinality, not Move-picker row count), `hasHomeArmy`, and `renderGrayscale` (true when there is at least one field army and every field army has a pending `ArmyMoveOrder`). Built from `WorldState.armies` + town markers; consumed by the Flame map as a player-app tap payload only.
