# Combat package — no `part` / `part of` directives (repo lint)

**SPEC/program** — repository lint gate that forbids Dart `part` / `part of`
file-splitting in `packages/colonizethis_combat/lib/`.

## Motivation

`packages/colonizethis_combat` previously split land auto-resolve into
`combat_resolver.dart` plus `part of` fragments for engagement outcome,
post-battle, and support helpers. Per #3983 those fragments become proper
libraries with explicit imports. The `part of` pattern couples every fragment
to a single library's implicit namespace and hides cross-file symbol
visibility. This gate keeps any new sub-file in the combat package a proper
...
`colonizethis_world` (`repo.world_no_part_directives`).

## Source of truth

| Artifact | Role |
|----------|------|
| `tool/check_combat_no_part_directives.dart` | Checker and CLI entrypoint |
| `tool/ct_repo_lint_manifest.yaml` (`repo.combat_no_part_directives`) | Rule registration |
...
`packages/colonizethis_combat/lib/`. Generated Dart and test files stay excluded
...
- Given a Dart file under `packages/colonizethis_combat/lib/` that contains a
...
- Given a Dart file under `packages/colonizethis_combat/lib/` that contains a
  ...
  the violation text names that file's repo-relative path and the 1-based line
  number of the directive.
- Given a Dart file under `packages/colonizethis_combat/lib/` that contains no
  `part` / `part of` directive, when the checker runs, then the checker does
  not fail for that file.
- Given a Dart file outside `packages/colonizethis_combat/lib/` that contains a
