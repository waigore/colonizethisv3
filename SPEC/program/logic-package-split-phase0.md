...
`lib/src/constants.dart` re-exports order/work constants via `package:colonizethis_orders/colonizethis_orders.dart` (Refs #4660 barrel SoT; the orders barrel already publishes `order_work_constants.dart`), so existing `package:colonizethis_logic/src/constants.dart` and `package:colonizethis_logic` barrel consumers keep their import paths and symbols unchanged (back-compat preserved). The world/models convenience re-exports (`GamePlayerLookup`, `kRegion*`, `kGridNeighborsCardinal4`, `kUnitType*`) stay in `constants.dart` as a neutral re-export shim.
...
The script also enforces an **≥80% line coverage** gate for the non-logic-domain support packages `colonizethis_data`, `colonizethis_models`, and `colonizethis_save` when each is included in `PACKAGES_TO_TEST` / the CI shard plan. These packages hold shared data models and persistence adapters rather than split logic domains, so they are held to the project-wide 80% baseline (`.cursor/rules/colonizethis-testing.mdc`) rather than the 90% logic/ai/map threshold.
...
- **Given** `colonizethis_world` is included in `PACKAGES_TO_TEST`, **when** `tool/run_package_tests.sh` builds its 90% coverage-gate target list, **then** `packages/colonizethis_world` is included in that list (the post-uplift world coverage gate is enabled, no deferral remains).
- **Given** the `run_package_tests.sh` 90% coverage-gate target list, **when** it is enumerated, **then** it contains all eight split domain packages (`colonizethis_world`, `colonizethis_combat`, `colonizethis_economy`, `colonizethis_diplomacy`, `colonizethis_setup`, `colonizethis_orders`, `colonizethis_turn`, `colonizethis_ai_contracts`) and does **not** contain `colonizethis_data`, `colonizethis_models`, or `colonizethis_save` (which are gated at the 80% threshold instead).
- **Given** `PACKAGES_TO_TEST=colonizethis_models` and a fresh `packages/colonizethis_models/coverage/lcov.info` from `dart test --coverage`, **when** `tool/run_package_tests.sh` completes, **then** it invokes `tool/check_coverage_threshold.sh 80 packages/colonizethis_models` and fails when line coverage is below 80%.
...
- **Given** a consumer importing `package:colonizethis_logic/colonizethis_logic.dart` or `package:colonizethis_logic/src/constants.dart`, **when** it references `kWorkTargetExplore` or `kMineralResourceIds`, **then** the symbols resolve unchanged because `lib/src/constants.dart` re-exports them via `package:colonizethis_orders/colonizethis_orders.dart` (`show` combinator; Refs #4660 barrel SoT) (`identical(constants.kMineralResourceIds, order_work_constants.kMineralResourceIds) == true`).
