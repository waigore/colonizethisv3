...
**Wrong:** `orders` → `turn` (1 file remaining)

| Source | Import | Symbols | Status / proposed destination |
|--------|--------|---------|---------------------|
| `orders/orders_application.dart` | `lib/src/trace/turn_trace_runtime.dart` | `TurnTraceRuntime` | Fixed — trace runtime hoisted to neutral `lib/src/trace/` |
| `orders/orders_application_context.dart` | same | same | Fixed — same |
| `orders/order_projections.dart` | `turn/turn_resolver.dart` | `resolveTurnForGame`, `requireTurnResolutionComplete` | Deferred — invert (callback injection) or move projections to `turn/` |
