...
| `OpenCivilianUnitsPanelEvent` | `GameSideMenu`, province Tile/Political/Civilian inline shortcuts | `CivilianUnitsPanel` (+ Riverpod game/orders, `AppEventBus`). Optional shortcut fields (at most one non-null): `exploreShortcutTargetTileKey` (`explorerOnly`), `prospectShortcutTargetTileKey`, `buildImprovementShortcutTargetTileKey` (`builderOnly`), `buildRoadShortcutTargetTileKey` (`engineerOnly`), `buildFortShortcutTargetTileKey` (`engineerOnly`), `buildPortShortcutTargetTileKey` (`engineerOnly`), `buildRailShortcutTargetTileKey` (`railBuilderOnly`), `purchaseLandShortcutTargetTileKey` (`merchantOnly`), `upgradeTownShortcutTargetTileKey` (`builderOnly`), `relocateShortcutTargetTileKey` (`spyOnly`), `counterSpyShortcutTargetTileKey` (`spyOnly`) — each opens the panel in the matching filtered shortcut mode for direct assign (or Spy Relocate / Spy `counter_spy`) on that tile key. |
| `OpenMilitaryUnitsPanelEvent` | `GameSideMenu`; Home-Army-only map marker tap | `MilitaryUnitsPanel` |
| `OpenArmyStackMarkerEvent` | Map army stack marker tap | Field armies with cached destinations: `showOverlayArmyMoveFlow` (field ids only → optional `DLG20002` then `DLG20001`); non-empty Home Army in that stack otherwise: Split then `DLG20001`; empty Home-only: `OpenMilitaryUnitsPanelEvent`. Observe: snackbar + return. |
...
| `OpenNavalMissionMenuEvent` | Map fleet marker tap | `showNavalFleetMarkerFlow` (non-empty Home Fleet → Split then `DLG30001` for the new fleet; empty Home Fleet → tile-scoped `OpenNavalUnitsPanelEvent`; capital in-port eligible transfer → `DLG31004` then `DLG30001` or `DLG40001`; other in-port → `DLG30001`; at sea → `showNavalMissionFlow` / `DLG31001` + Sail → `DLG30001`); mission confirm emits `NavalMissionRequestedEvent` / `NavalMissionCancelRequestedEvent` |

**Civilian / naval work and fleets:** `CivilianUnitsPanel` emits `StartCivilianWorkTargetSelectionEvent`, **`StartCivilianRelocateSelectionEvent`** (Spy only, Refs #4219), `LocateMapTileEvent`, `RemovePendingWorkOrderRequestedEvent`, **`RemovePendingCivilianMoveRequestedEvent`**, and `CancelInProgressCivilianWorkRequestedEvent`; `GameMapArea` listens for **`StartCivilianRelocateSelectionEvent`**, enters relocate selection mode, and on valid tile pick may emit **`ConfirmDialogEvent`** (leave-intel soft warn) then **`CivilianMoveRequestedEvent`**; `AppEventHandlerScope` validates and stages draft civilian `MoveOrder`s via `applyCivilianMoveOrderForPlayer`. `MilitaryUnitsPanel` / `NavalUnitsPanel` emit `LocateMapTileEvent`; `MilitaryUnitsPanel` emits **`ArmyMoveRequestedEvent`**, **`ArmySplitRequestedEvent`**, **`ArmyCombineRequestedEvent`**, and **`LandArmiesUpdatedEvent`** when army state changes; `NavalUnitsPanel` emits `NavalFleetsUpdatedEvent` after combine; split uses `SplitFleetDialog` → **`NavalSplitFleetRequestedEvent`** → scope applies and emits **`NavalFleetsUpdatedEvent`**. Train dialogs emit **`TrainCivilianBuildOrdersCommittedEvent`** / **`TrainMilitaryBuildOrdersCommittedEvent`** on close; scope merges orders. `AppEventHandlerScope` subscribes and updates `currentOrdersProvider` / `currentGameProvider` using `colonizethis_logic` where applicable (`removePendingWorkOrderAt`, `clearUnitCurrentWork`). Panels do not receive Riverpod `ref` for those mutations.
...
- Given **`AppEventHandler`** is bound with **`extraActionHandlers`** mapping **`QuickStartNewGameEvent`**, When the system emits **`QuickStartNewGameEvent`**, Then that handler is invoked with the handler’s **`navigatorKey`** and no dialog is opened.
