...
- **App widget tests:** The app package has an **80% line-coverage target** for **widget-unit code only**. The gate applies only to **`lib/widgets/`** (e.g. `CtMainMenu`, `CtGameSetup`). Code outside that scope is excluded from the app coverage percentage so the target reflects individual widget correctness, not entry points or glue.
- **Excluded from app coverage gate:** `main.dart`, `app.dart`, `lib/config/`, `lib/core/`, `lib/providers/`, `lib/features/` (routing, themes, services, Riverpod, Flame/screens). These are not widget units and are not required to meet the 80% widget target.
- When you run **`tool/run_quality_gate_tests.sh`**, the script runs `flutter test --coverage` in `app/` and then **`tool/check_coverage_threshold.sh 80 app`**, which uses `lcov --extract` to keep only `lib/widgets/*` before computing the percentage. To verify app widget coverage after `tool/test_coverage.py`: `tool/check_coverage_threshold.sh 80 app`.
- **Improving app widget coverage:** Add widget tests in `app/test/` that pump `CtMainMenu` and `CtGameSetup` (and any new `lib/widgets/` widgets). Cover uncovered branches (e.g. `MainMenuVariant.pixelArt`, `MainMenuState.afterVictory`, narrow layout in Game Setup, dropdown and slot logic). See `app/test/screen_spec_acceptance_test.dart` and SPEC/ui/main-menu.md, game-setup.md for acceptance criteria.
