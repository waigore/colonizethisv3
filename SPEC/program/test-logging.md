...
- When you run **`tool/run_quality_gate_tests.sh`**, the script runs `flutter test --coverage` in `app/` and then **`tool/check_coverage_threshold.sh 85 app`**, which enforces that overall `app/lib/` coverage is at or above 85%. To verify app coverage after `tool/test_coverage.py`: `tool/check_coverage_threshold.sh 85 app`.
