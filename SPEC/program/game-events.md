...
| `order_rejected`     | On validation failure          | playerId, **orderKind** (discriminator for the [Orders] family), orderSummaryOrId, reasonCode (e.g. insufficient_treasury, invalid_destination). |
| `work_order_completed` | When a civilian work target resolves during Build/Work phase | playerId, unitId, workTarget, targetTileKey, provinceId (prefixed), turnNumber, optional **revealedResourceId** (commodity id when `workTarget` is `prospect` and `resourceByTileKey` is a prospect-required mineral; omitted/null for no deposit and for non-Prospect work), optional ids-only payoff snapshot captured from `stateAfter` at emission (Refs #4778): **payoffKind** token (`fully_visible`, `yield_raise`, `road_limit`, `town_limit`, `unbound`, `binds_capital`, `port_present`, `railroad_present`, `workshops_start`, `workshops_pause`, `workshops_resume`, `fort_open_field`, `fort_wood`, `fort_stone`, `fort_modern`, `purchase_tradeable`, `purchase_riches`), **payoffCommodityId** (commodity id for improve / purchase / yield clauses; never a UI string), **payoffLevel** (post-work town development or fort level). Same-turn-assigned **1-turn** Prospect and **Purchase land** are not visible on the pre-Build/Work `currentWork` snapshot; emission must detect those completions (newly prospected tiles + this turn's prospect orders; newly recorded `purchasedTilesByTileKey` + this turn's `purchase_land` orders), not only `remainingTurns <= 1` on the before snapshot. |
...
- **Emission — work_order_completed (same-turn 1-turn Prospect).** Given a player assigns Explorer **Prospect** this turn with no pre-existing `currentWork` and Build/Work completes that 1-turn survey (tile added to `playerProspectedTiles`), when the Build/Work phase finishes, then the system emits exactly one `work_order_completed` for that unit/tile with `workTarget` `prospect`. Given the surveyed tile's `resourceByTileKey` is a prospect-required mineral, when that event is emitted, then `revealedResourceId` is that commodity id. Given the tile is mineral-eligible with no prospect-required resource, when that event is emitted, then `revealedResourceId` is null. Given Explore or other non-Prospect civilian work completes, when `work_order_completed` is emitted, then `revealedResourceId` is null.
- **Emission — work_order_completed (same-turn 1-turn Purchase land).** Given a player assigns Merchant **Purchase land** this turn with no pre-existing `currentWork` and Build/Work records that tile in `purchasedTilesByTileKey` for that player, when the Build/Work phase finishes, then the system emits exactly one `work_order_completed` for that unit/tile with `workTarget` `purchase_land`. Given the purchased tile's resource is a riches commodity, when that event is emitted, then `payoffKind` is `purchase_riches` and `payoffCommodityId` is that commodity id. Given the resource is tradeable (not riches), when that event is emitted, then `payoffKind` is `purchase_tradeable` and `payoffCommodityId` is that commodity id.
- **Emission — work_order_completed payoff snapshot.** Given Explore completes, when `work_order_completed` is emitted, then `payoffKind` is `fully_visible`. Given Upgrade town completes, when the event is emitted, then `payoffLevel` is the post-work `townDevelopmentLevel` and `payoffKind` is `workshops_start` (level 1), `workshops_pause` (level 2), or `workshops_resume` (level ≥ 3). Given Build fort completes, when the event is emitted, then `payoffLevel` is the post-work `fortLevel` and `payoffKind` is `fort_open_field` (0), `fort_wood` (1), `fort_stone` (2), or `fort_modern` (≥ 3). Given Build improvement completes, when the event is emitted, then `payoffCommodityId` is the tile resource id when present and `payoffKind` is computed from post-work capital-link connectivity on `stateAfter`: `unbound` when the tile is not capital-connected, `road_limit` when post-work production exceeds the path transport cap, `town_limit` when the town-development cap binds after path, otherwise `yield_raise`. When tile maps are omitted, the system falls back to `yield_raise`. Given Build port / railroad / road completes, when the event is emitted, then `payoffKind` is `port_present`, `railroad_present`, or `binds_capital` respectively (yield-family tokens may replace `binds_capital` when the snapshot records a yield clause). Payloads remain ids-only; the UI layer formats display names.
