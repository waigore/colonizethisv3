...
| `work_order_completed` | When a civilian work target resolves during Build/Work phase | playerId, unitId, workTarget, targetTileKey, provinceId (prefixed), turnNumber, optional **revealedResourceId** (commodity id when `workTarget` is `prospect` and `resourceByTileKey` is a prospect-required mineral; omitted/null for no deposit and for non-Prospect work). Same-turn-assigned **1-turn** Prospect is not visible on the pre-Build/Work `currentWork` snapshot; emission must detect that completion (completion path or post-phase newly-prospected tiles + this turn's prospect work orders), not only `remainingTurns <= 1` on the before snapshot. |
