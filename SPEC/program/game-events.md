...
Events are a union or sealed type (e.g. `GameEvent`) with variants. Payloads use keys/ids only; no UI strings. Province ids in any payload are **prefixed** (`regionId|localId`) per [world-model-identity.md](../game/world-model-identity.md). Implementation keeps sealed variants in one Dart library so pattern-switch exhaustiveness stays sound; multi-file splits would need `part` directives (disallowed under world package lint) or a non-sealed base (Refs #4330). That library (`packages/colonizethis_world/lib/src/game_events.dart`) has a dedicated **400** physical-line ceiling; other world lib files are gated at **300** (`repo.colonizethis_world_lib_file_size`, Refs #4515).
...
- **Sealed single-library size.** Given `GameEvent` remains `sealed` in `packages/colonizethis_world/lib/src/game_events.dart`, when `repo.colonizethis_world_lib_file_size` runs, then that file is allowed up to 400 physical lines and every other non-generated world lib file is gated at 300 (Refs #4515).
