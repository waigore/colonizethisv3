...
2. **Generate OW map** — Call colonizethis_map with locked profile (`provinceCount=60`, `continentCount=4`, region `oldWorld`). Validate topology contains exactly 4 P–P continents (order-insensitive by size). Lake-fill pass must eliminate enclosed inland seas by subsuming inland sea cells into bordering landmass tiles. On partition mismatch, retry generation with deterministic seed offsets up to 5 retries (6 attempts total). On exhaustion, fail setup with `old_world_partition_retry_exhausted`.
