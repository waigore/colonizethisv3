...
8. **Build state:**
  ...
  - **7c. Province and sea-zone naming** — Apply naming from active ruleset per [naming.md](../game/naming.md). Applies to all provinces owned at setup and all sea-zone nodes in each region topology; provinces acquired later retain existing display name. Sea-zone names are generated deterministically from setup seed and stored by prefixed sea-zone id for UI display.
