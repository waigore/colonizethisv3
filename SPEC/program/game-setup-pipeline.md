...
6. **New World tribe assignment** — **`_assignFactionsOnRemainderAuto`:** single-component remainders use **`assignTerritoriesLockedOnLandmass`**; multi-component remainders use **bounded DFS** over tribe-to-component packings (`_tryPackFactionsOntoPpComponentsDfs`), then locked growth per component. Failures surface as **`SetupTopologyDataException`** with **`faction_component_bin_pack_failed`** or **`assignment_remainder_not_connected`**; `runInitGame` may **regenerate maps** and retry (step 3 pipeline retries), not an in-place BFS paint of the whole NW. **Tribe sea-bound ownership gate (locked full-init profile):** after NW ownership is painted, The System verifies every Tribe owns **≥1 sea-bound (P–S) province** ([game-setup.md](../game/game-setup.md) § Tribe Assignment). When any Tribe owns zero sea-bound provinces, The System throws **`SetupTopologyDataException`** with retriable code **`tribe_missing_sea_bound_province`**, and the step-3 regen-until-pass loop regenerates the tile-map pair with a bumped seed and retries. The gate is enforced only under the locked full-init profile; freeform / injected-generator configs are unaffected.
7. **Build state:**
...
- Given `createGameFromGeneratedMaps` runs with an Old World `TileMapResult` that includes both `terrainGrid` and `resourceGrid`  
  When The System completes ownership and before §7b capitals, and later completes §7d.strip before `applyGreatPowerStartingGrainBootstrap`  
  Then The System runs **§7d.terrain** before capitals/towns, then **§7d.redist** after §7d.strip, and both run **before** `applyInitTownRoadsToCapitals`, with **no** `GameSetupConfig` field or external flag disabling either step when applicable.
...
- Given `runInitGame` is invoked with a locked full-init `GameSetupConfig` and the default tile-map generator  
  When initialization completes successfully  
  Then in `result.game.worldState.newWorld.provinces` every Tribe owner has at least one owned province that is sea-bound (P–S) in `result.topologyByRegion['newWorld']` (`colonizethis_setup` `init_game_orchestrator` tests).

- Given `createGameFromGeneratedMaps` paints New World ownership for a locked full-init profile and the painting leaves at least one Tribe with zero sea-bound provinces  
  When New World ownership validation runs  
  Then The System throws `SetupTopologyDataException` with stable code `tribe_missing_sea_bound_province`, and `isRetriableInitTopologyCode('tribe_missing_sea_bound_province')` is `true` so the bounded regen loop retries with a bumped map seed.
