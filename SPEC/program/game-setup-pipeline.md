...
6b. **GP land connectivity repair (OW)** — **Only if `config.enforceFairGpOldWorldAssignment` is true.** Run deterministic DFS search over ownership swap states; each edge is one legal same-landmass GP↔(GP/minor) 1:1 province swap. Goal state: every GP is P–P connected while preserving one-landmass-per-GP and ≥1 sea-bound province per GP. If search fails within configured limits, fail setup with `gp_land_connectivity_exhausted`. If **false**, skip 6b entirely.
