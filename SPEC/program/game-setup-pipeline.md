...
6b. **Fair-assignment connectivity repair (OW + NW)** — **Only if `config.enforceFairAssignment` is true.** Run deterministic DFS search over ownership swap states; each edge is one legal same-landmass 1:1 province swap and the search tracks visited ownership states to avoid revisits. OW target set: all GPs and minors, while preserving GP hard rules (one-landmass-per-GP and ≥1 sea-bound province per GP). NW target set: all tribes. A swap is legal only if it does not make a previously contiguous swap-partner disconnected. If search fails within configured limits, fail setup with `fair_assignment_connectivity_exhausted`. If **false**, skip 6b entirely.
7. **Tribe assignment** — Assign NW provinces to tribes via BFS per [game-setup.md](../game/game-setup.md) § Tribe Assignment before 6b is evaluated.
8. **Build state:**
