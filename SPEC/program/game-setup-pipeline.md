...
**Advanced-start bootstrap (when `config.advancedStart ≠ none`):** After `createGameFromGeneratedMaps` and **before** map-view build, `runInitGame` calls `applyAdvancedStartBootstrap(game, config)` in `colonizethis_setup`. Mutates turn number, GP techs, treasury, and workforce (foundation slice); remaining steps per [advanced-starts.md](../game/advanced-starts.md). Skipped with a logged warning when `isLockedFullInitProfile` is false.
