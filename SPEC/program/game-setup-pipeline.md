...
- **GameSetupConfig:** seed, selectedGreatPowerIds, continent count, minor/tribe counts, target province counts, min provinces per minor, **`enforceFairAssignment`** (bool, default **false**), **`initTownRoadWiringRegionIds`** (`Set<String>`, default **`{oldWorld}`**), and optional **`preferredInitialMapZoomMultiplier`** (`double?`, fit-relative `m`). For map-generating clients, setup orchestration applies a locked profile before generation/assignment: `greatPowerCount=6`, `minorNationCount=6`, `numProvincesOldWorld=60`, `tribeCount=10`, `numProvincesNewWorld=30`, `continentCount=4`. In this locked path, `enforceFairAssignment` is ignored and setup uses one assignment pass without post-assignment repair. For fresh campaign setup, if `preferredInitialMapZoomMultiplier` is null the setup seeds `Game.mapViewState.zoomMultiplier = 4.0`; if set, setup uses that preferred value. In both cases setup clamps to the shared fit-relative band `[0.5, 8.0]`.
...
2. **Generate OW map** — Call colonizethis_map with locked profile (`provinceCount=60`, `continentCount=4`, region `oldWorld`). Validate topology contains exactly 4 P–P continents and sorted continent sizes equal `13, 13, 17, 17` (order-insensitive; equivalent `17/17/13/13`) and passes locked-role feasibility for contiguous assignment (`7+7+3` on each 17-continent, `7+3+3` on each 13-continent, with GP groups sea-bound-capable). Lake-fill pass must eliminate enclosed inland seas by subsuming inland sea cells into bordering landmass tiles. On partition or feasibility mismatch, retry generation with deterministic seed offsets up to 50 retries (51 attempts total). On exhaustion, fail setup with `old_world_partition_retry_exhausted`.
3. **Generate NW map** — Call colonizethis_map with locked profile (`provinceCount=30`, `continentCount=4`, region `newWorld`) and deterministic seed offset (`effectiveSeed + 1`). Validate topology contains exactly 4 P–P continents and sorted continent sizes equal `6, 6, 9, 9` (order-insensitive). On partition mismatch, retry generation with deterministic seed offsets up to 50 retries (51 attempts total). On exhaustion, fail setup with `new_world_partition_retry_exhausted`.
...
5. **GP assignment** — From OW **P–P** edges, derive continent ids (land subgraph components, [map-topology.md](../game/map-topology.md) § Continents). **Hard quotas:** each GP target is fixed to 7 provinces (total 42). **Before** BFS: assign each GP to **one** continent with capacity checks (targets summed per continent ≤ its province count; GPs per continent ≤ **sea-bound** provinces there for seeds). Multi-source BFS with strict per-GP continent map (`factionLandmassIds`); infeasible → clear error. [game-setup.md](../game/game-setup.md) § GP Assignment (one continent per GP).
6. **Minor Nation assignment** — Assign remaining OW provinces to minors via BFS per [game-setup.md](../game/game-setup.md) § Minor Nation Assignment. **Hard quotas:** each minor target is fixed to 3 provinces (total 18). Combined OW ownership after steps 5+6 must be exactly 60.
7. **Tribe assignment** — Assign NW provinces to tribes via BFS with fixed locked targets: 10 tribes, 3 provinces each (total 30), with fixed continent-role split `3+3+2+2` over the sorted locked `9,9,6,6` landmasses.
8. **Build state:**
