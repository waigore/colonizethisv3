...
3. **Generate NW map** — Same for `newWorld`; use deterministic seed offset (e.g. `effectiveSeed + 1`) and same resource rules. **Locked full-init (#1830):** target OW/NW P–P multisets (`[13,13,17,17]` / `[6,6,9,9]`) are **not** enforced by `runInitGame` today; bounded regen/rejection until multisets match is specified for **colonizethis_map** (see [locked-province-assigner.md](locked-province-assigner.md)). Until that generator integration ships, `runInitGame` keeps **one** pair of generation calls per init; setup then chooses locked vs BFS painting from the inferred topology.
...
5. **Old World ownership** — Reserve `minorCount × minProvincesPerMinor` for minors, derive landmasses from OW **P–P** components, then assign GPs and minor nations per [game-setup.md](../game/game-setup.md) and [locked-province-assigner.md](locked-province-assigner.md). **Per landmass:** use **`assignTerritoriesLockedOnLandmass`** only when `GameSetupConfig.isLockedFullInitProfile` **and** OW matches the locked multiset **and** `lockedOldWorldRoleFeasibilityHolds`; otherwise use **`assignTerritoriesByBfsGrowth`** with the same seeds and fair targets. Locked assigner exhaustion → **`assigner_exhausted`** (wrapped as `SetupTopologyDataException` where applicable). **No** GP connectivity repair pass.
6. **New World tribe assignment** — Prefer locked remainder assignment; if greedy multi-component packing or locked search fails, fall back to **`assignTerritoriesByBfsGrowth`** on the full NW province set per [locked-province-assigner.md](locked-province-assigner.md).
7. **Build state:**
