...
2. **Generate OW map** — Call colonizethis_map with locked profile (`provinceCount=60`, `continentCount=3`, region `oldWorld`). Validate topology partition equals `21/21/18` (order-insensitive). On mismatch, retry generation with deterministic seed offsets up to 5 retries (6 attempts total). On exhaustion, fail setup with `old_world_partition_retry_exhausted`.
