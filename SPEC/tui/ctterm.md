...
- **Political layer glyphs:** The in-game shell’s political map layer uses a **single character per tile** drawn from the precomputed mapping `Game.politicalGlyphByPlayerId` so that the map viewport remains the same shape as terrain/resources/units layers. Great Powers use unique **uppercase letters** derived from their display names; Minor Nations and Tribes use **digits `1`–`9` then lowercase letters `a`–`z`**; unclaimed tiles use `·`. The mapping is computed once during game setup and persisted in the save so glyphs are stable across reloads.
...
- **Logging:** Use multiple log prefixes for easy identification: `tui:`, `tui:menu:`, `tui:save:`, `tui:nav:` (and later `tui:map:`, `tui:event:`). Use Dart `logger`; no `print` for operational output. See [SPEC/program/logging/logging.md](../program/logging/logging.md) for level and prefix conventions; [SPEC/program/ctdev-logging.md](../program/ctdev-logging.md) for ctdev file/Sim Log only.
