...
List<FrrCreditsScenario> frrCreditsDefensiveScenarios() => [
  ...
  frrEmptyCreditsRow(label: 'negative — deal with null sellerOriginTileKey is skipped', filledDeals: [deal(buyer: 'gpB', quantity: 10, pricePerUnit: 20.0)], constantRelation: 100),
  frrEmptyCreditsRow(label: 'negative — deal with unmapped tile key is skipped (no attribution)', filledDeals: [dealOn('unmapped', buyer: 'gpB')], constantRelation: 100),
  frrEmptyCreditsRow(label: 'negative — zero quantity or zero price deals are skipped', filledDeals: [dealOn('k1', buyer: 'gpB', quantity: 0, pricePerUnit: 20.0), dealOn('k1', buyer: 'gpB', quantity: 10, pricePerUnit: 0.0)], constantRelation: 100),
];
// dart format on
...
FrrCreditsScenario _frrCreditsDeterminismScenario() => frrCreditsRow(
  ...
  filledDeals: [dealOn('k2', buyer: 'gpC', quantity: 1, pricePerUnit: 5.0), dealOn('k1', buyer: 'gpC', quantity: 2, pricePerUnit: 5.0), dealOn('k2', buyer: 'gpC', quantity: 3, pricePerUnit: 5.0)],
...
List<FrrCreditsScenario> frrCreditsAggregationScenarios() => [
  frrCreditsRow(
    ...
    filledDeals: [dealOn('k1', buyer: 'gpC', quantity: 10, pricePerUnit: 10.0), dealOn('k2', buyer: 'gpC', quantity: 4, pricePerUnit: 5.0), dealOn('k3', buyer: 'gpC', quantity: 2, pricePerUnit: 3.0)],
    ...
    relationScoreFor: frrRelationTable(const {'gpA': {'M1': 100, 'M2': 25}, 'gpB': {'M1': 50}}),
  ...
  frrCreditsRow(
    ...
    filledDeals: [dealOn('k1', buyer: 'gpA', isFirstRightOfRefusalMatch: true, quantity: 4, pricePerUnit: 10.0), dealOn('k1', buyer: 'gpB', quantity: 6, pricePerUnit: 10.0)],
...
List<FrrCreditsScenario> frrCreditsKickbackScenarios() => [
  frrCreditsRow(
    ...
    relationScoreFor: frrRelationTable(const {'gpA': {'M1': 100}}),
  ...
  frrCreditsRow(label: 'R8.6 — kickback applies on Minor/Tribe sale with no purchased tile', filledDeals: [deal(buyer: 'gpB', quantity: 10, pricePerUnit: 20.0)], purchasedTileIndex: idx(const []), embassyGpRelationsFor: frrEmbassyForM1(const {'gpC': 50}), expect: frrKickbackExpect(treasuryCreditEmpty: true, embassyKickbackCloseTo: const {'gpC': 10.0}), refs: '#3753'),
  frrCreditsRow(
  ...
  frrCreditsRow(label: 'no embassy holders → no kickbacks (empty callback result)', constantRelation: 100, embassyGpRelationsFor: (_) => const {}, expect: frrKickbackExpect(embassyKickbackEmpty: true, totalEmbassyKickback: 0.0, treasuryCreditCloseTo: const {'gpA': 200.0}), refs: '#3753'),
  frrCreditsRow(label: 'relation-0 embassy holder records no kickback', filledDeals: [deal(buyer: 'gpB', quantity: 10, pricePerUnit: 20.0)], purchasedTileIndex: idx(const []), embassyGpRelationsFor: frrEmbassyForM1(const {'gpC': 0}), expect: frrKickbackExpect(sameAsEmpty: true), refs: '#3753'),
];
// dart format on
...
List<FrrCreditsScenario> frrIssueAcD5CreditsScenarios() => [
  frrD5CreditsRow(
    ...
    relationScoreFor: frrRelationTable(const {kFrrIssueAcD5GpA: {kFrrIssueAcD5MinorM1: 75}}),
  ...
  frrD5CreditsRow(
    label: 'negative — buyer == owning GP (D2 FRR-match path) excluded from D4 aggregation: no double-credit when gpA wins the offer itself',
    filledDeal: frrD5FrrMatchDeal(),
    constantRelation: 100,
    expect: frrTreasuryCloseTo(const {}, creditedDealsEmpty: true, treasuryCreditEmpty: true, totalProfitTreasury: 0.0),
    refs: '#2992 D5 AC4',
  ),
  frrD5CreditsRow(
    ...
    relationScoreFor: frrRelationTable(const {kFrrIssueAcD5GpA: {kFrrIssueAcD5MinorM1: 100}, kFrrIssueAcD5GpB: {kFrrIssueAcD5MinorM1: 50}}),
  ...
  frrD5CreditsRow(
    ...
    relationScoreFor: frrRelationTable(const {kFrrIssueAcD5GpA: {kFrrIssueAcD5MinorM1: 100, kFrrIssueAcD5MinorM2: 25}}),
