...
void runCanAffordRecruitWorkerExpectation(CanAffordRecruitWorkerPins pins) {
  ...
  final result = canAffordRecruitWorker(
    _playerWithTech(pins.tech),
    RecruitWorkerOrder(targetTier: pins.targetTier),
    pins.workers,
    stockpile,
    pins.treasury,
  );
...
WorkerActionCostScenario canAffordRecruitWorkerScenario({
...
typedef ApplyRecruitWorkerCostPins = ({
  WorkerTier targetTier,
  WorkerPool initialWorkers,
  Map<String, int> stockpileDeltas,
  int treasury,
  WorkerPool expectedWorkers,
  Map<String, int> expectedStockpileQuantities,
...
WorkerActionCostScenario applyRecruitWorkerCostScenario({
  required String label,
