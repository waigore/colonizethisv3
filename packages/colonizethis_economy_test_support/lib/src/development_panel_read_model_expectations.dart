// dart format off
// Development panel read-model assertions (Refs #4175, #4014).
...
import 'development_panel_read_model_scenarios.dart';
import 'extraction_fixture_support.dart';

const _kWorkTargetBuildImprovement = 'build_improvement';
  ...
  const orders = Orders(
    workOrdersByPlayerId: {
      'pl1': [
        WorkOrder(
          unitId: 'b1',
          target: _kWorkTargetBuildImprovement,
...
void runDevelopmentPanelReadModelExpectation(
  DevelopmentPanelReadModelTarget target,
) {
  switch (target) {
    case DevelopmentPanelReadModelTarget.ownedProvinceListsImprovableGrain:
    ...
    case DevelopmentPanelReadModelTarget.modelBuildsWithPendingWorkOrders:
      expect(
        _modelWithPendingWorkOrders().oldWorld.idleBuilderCount,
        greaterThanOrEqualTo(0),
      );
    case DevelopmentPanelReadModelTarget.playerViewFiltersUnrevealedImprovableTiles:
      final map = tileMapFromGrids(
        grid: const [
          ['p1', 'p1'],
        ],
    ...
    case DevelopmentPanelReadModelTarget.assignedCiviliansIncludePendingBuilder:
      final unit = Unit(
        id: 'b1',
