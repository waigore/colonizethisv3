...
ResourceExtractorScenario extractionScenario({
  ...
  final resolvedExpect =
      expect ??
      ResourceExtractorExpectation(
  ...
  return (
    ...
    refs: refs,
    verify: pinOnly
        ? _noopResourceExtractorVerify
        : (result) =>
              assertResourceExtractorExpectation(result, resolvedExpect),
  );
}
...
List<ResourceExtractorScenario> resourceExtractorConnectivityCapScenarios({
  ...
  extractionScenario(label: 'stub connectivity: land totals and tech cap applied', grid: _p1Grid2x2, resourceGrid: const [[Resource.grain, Resource.timber], [Resource.iron, null]], improvements: [owP1Imp(3, 2), const TileImprovementSpec('oldWorld|p1|1|0', 2, 1), const TileImprovementSpec('oldWorld|p1|0|1', 4)], connected: {kOwP1Tile00, 'oldWorld|p1|1|0', 'oldWorld|p1|0|1'}, expectLand: const {'grain': 2, 'timber': 1}, landAbsent: const ['iron'], expectOverseasEmpty: true),
...
List<ResourceExtractorScenario> resourceExtractorMineralTownDevScenarios({
  ...
  extractionScenario(label: 'extracts wool and copper when present on tile map', grid: _p1Grid2x2, resourceGrid: const [[Resource.wool, Resource.copper], [Resource.timber, Resource.iron]], improvements: tileImps(_p1FourTiles), connected: _p1FourTiles, playerProspectedTiles: {'pl1': _p1FourTiles}, expectLand: const {'wool': 1, 'copper': 1, 'timber': 1, 'iron': 1}),
  ...
  extractionScenario(label: 'mineral from prospected tile counts in land', tileMap: ironTileMap, improvements: [owP1Imp(2, 2)], connected: {kOwP1Tile00}, playerProspectedTiles: {'pl1': {kOwP1Tile00}}, expectLand: const {'iron': 2}),
...
List<ResourceExtractorScenario> resourceExtractorEmptyConnectivityScenarios() =>
    [extractionScenario(label: 'returns empty ExtractionTotals when player has no connected tiles', tileMapByRegion: const {}, expectLandEmpty: true, expectOverseasEmpty: true)];
// dart format on
...
// dart format off
List<ResourceExtractorScenario> resourceExtractorSpecialCaseScenarios({
  required TileMapResult grainTileMap,
}) {
  final blockade = blockadedOverseasExtractionFixture();
  return [
    extractionScenario(label: 'overseas totals when connected tile in different region', useOverseasGame: true, tileMapByRegion: {'oldWorld': singleTileMap(null), 'newWorld': singleTileMap(Resource.sugarCane, province: 'n1')}, improvements: const [TileImprovementSpec('newWorld|n1|0|0', 1, 1)], connected: {'newWorld|n1|0|0'}, expect: const ResourceExtractorExpectation(overseas: {'sugarCane': 1}, landEmpty: true)),
    ...
    _townRuleScenario(label: 'town-rule-only + port: townDevelopmentLevel DOES cap yield', grid: const [['p1', 'p1'], ['p1', 'p2']], resourceGrid: const [[null, null], [null, Resource.grain]], p2TownTileKey: 'oldWorld|p2|0|1', portsByProvinceSeaboard: {'oldWorld|p2|sea1': 'oldWorld|p2|0|1'}, expectLand: const {'grain': 2}),
    _townRuleScenario(label: 'town-rule-only + non-port: townDevelopmentLevel does NOT cap yield', grid: const [['p1', 'p1', 'p1'], ['p1', 'p2', 'p2'], ['p1', 'p2', 'p2']], resourceGrid: const [[null, null, null], [null, Resource.grain, null], [null, null, null]], p2TownTileKey: 'oldWorld|p2|1|0', expectLand: const {'grain': 4}),
    extractionScenario(label: 'effective extraction capped by player tech cap when improvement and transport are high', tileMap: grainTileMap, improvements: [owP1Imp(4, 4)], connected: {kOwP1Tile00}, techCapComparisonPin: const TechCapComparisonPin(capsAndExpectedGrain: [(2, 2), (3, 3)])),
    extractionScenario(label: 'capital tile grain bonus is unconditional on connectivity', tileMapByRegion: const {}, connected: const {}, gameOverride: resourceExtractorGame(tileState: TileMapState(), capitalTileGrainBonusPerTurn: 5), connectivityByPlayer: connectivityFor(const {}), expect: const ResourceExtractorExpectation(land: {'grain': 5}, overseasEmpty: true)),
    extractionScenario(label: 'blockaded overseas port: connectivity excludes tile so overseas extraction zero', gameOverride: blockade.game, tileMapByRegion: blockade.tileMapByRegion, blockadedOverseasPin: BlockadedOverseasPin(topology: blockade.topology, blockadedPortProvincesByPlayerId: {'pl1': {'newWorld|n1'}}), refs: '#3939'),
    extractionScenario(label: 'skips connected tile and logs when province missing from region (world-model)', tileMap: grainTileMap, improvements: [owP1Imp(2, 2)], connected: {kOwP1Tile00}, gameForTileState: (tileState) => provinceMissingExtractorGame(tileState: tileState), expectLogMessageContains: 'extraction province missing', expect: const ResourceExtractorExpectation(landAbsent: ['grain'], landEmpty: true, overseasEmpty: true), refs: '#3939'),
  ];
}
// dart format on

final _townRuleTileSpecs = [
  owP1Imp(0, 1),
  const TileImprovementSpec('oldWorld|p2|1|1', 4),
];

ResourceExtractorScenario _townRuleScenario({
  required String label,
  required List<List<String>> grid,
  required List<List<Resource?>> resourceGrid,
  required String p2TownTileKey,
  required Map<String, int> expectLand,
