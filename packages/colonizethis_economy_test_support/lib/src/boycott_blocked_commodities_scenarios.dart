...
typedef BoycottBlockedCommoditiesScenario = ({String label, Game Function() buildGame, String buyerPlayerId, bool useDefaultTileMaps, bool useDefaultTopology, Map<String, ConnectivityResult>? connectivityByFactionId, Map<String, List<TradeOrder>>? autoOffersByFactionId, void Function(Set<CommodityId> blocked) verify, String? refs});
...
List<BoycottBlockedCommoditiesScenario> boycottBlockedCommoditiesScenarios() => [
  _boycottColonyRow(
    ...
    expect: const BoycottBlockedCommoditiesExpectation(blockedCommodityIds: {'furs'}),
    refs: '#3758',
  ),
  _boycottColonyRow(
  ...
  _boycottColonyRow(label: 'empty for a buyer that is not the boycott target', buyerPlayerId: 'gpA', expect: const BoycottBlockedCommoditiesExpectation(isEmpty: true), refs: '#3758'),
  _boycottColonyRow(label: 'empty when no boycott is active', boycottStates: const [], expect: const BoycottBlockedCommoditiesExpectation(isEmpty: true), refs: '#3758'),
  _boycottColonyRow(label: 'empty when the boycotting GP holds no colony', colonyStates: const [], expect: const BoycottBlockedCommoditiesExpectation(isEmpty: true), refs: '#3758'),
  _boycottColonyRow(label: 'empty when tile maps are omitted (unit-test path unaffected)', useDefaultTileMaps: false, expect: const BoycottBlockedCommoditiesExpectation(isEmpty: true), refs: '#3758'),
  _boycottColonyRow(label: 'empty when topology is absent', useDefaultTopology: false, expect: const BoycottBlockedCommoditiesExpectation(isEmpty: true), refs: '#3758'),
];
/// Runs a boycott-blocked-commodities scenario row.
void runBoycottBlockedCommoditiesScenario({required BoycottBlockedCommoditiesScenario scenario, required Map<String, TileMapResult> defaultTileMaps, required MapTopology defaultTopology}) {
  final blocked = boycottedColonySellableCommodityIds(game: scenario.buildGame(), buyerPlayerId: scenario.buyerPlayerId, tileMapByRegion: scenario.useDefaultTileMaps ? defaultTileMaps : const {}, topology: scenario.useDefaultTopology ? defaultTopology : null, connectivityByFactionId: scenario.connectivityByFactionId, autoOffersByFactionId: scenario.autoOffersByFactionId);
