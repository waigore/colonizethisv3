...
typedef WorkMaterialAffordPins = ({
...
void runWorkMaterialAffordExpectation(WorkMaterialAffordPins pins) {
  ...
  expect(
    ProjectedCostEngine.canAffordWorkMaterialCost(stockpile, pins.cost),
    pins.expectedAfford,
  );
}
...
const _delegationBuildOrder = BuildUnitOrder(
...
void runBuildAffordDelegationExpectation() {
  final viaEngine = ProjectedCostEngine.canAffordBuildOrder(
    _delegationPlayer,
    _delegationBuildOrder,
    _delegationWorkers,
    _delegationStockpile,
    _delegationTreasury,
  );
  final direct = canAffordBuild(
    _delegationPlayer,
    _delegationBuildOrder,
    _delegationWorkers,
    _delegationStockpile,
...
const _applyDeductionOrder = BuildUnitOrder(
...
const _applyDeductionStockpile = Stockpile(
  quantities: {'paper': 50, 'cast_iron': 50, 'lumber': 50},
);
...
void runBuildApplyDeductionDelegationExpectation() {
  final viaEngine = ProjectedCostEngine.applyBuildOrderCostDeduction(
    _applyDeductionPlayer,
    _applyDeductionOrder,
    _applyDeductionWorkers,
    _applyDeductionStockpile,
    _applyDeductionTreasury,
  );
