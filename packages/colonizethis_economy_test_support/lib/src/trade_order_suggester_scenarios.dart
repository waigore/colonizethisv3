...
typedef TradeOrderSuggesterScenario = ({
  String label,
  TradeSuggestionContext Function() buildContext,
  void Function(TradeSuggestionContext context, TradeSuggestionResult result)
  verify,
  String? refs,
});
...
List<TradeOrderSuggesterScenario> tradeOrderSuggesterEmptyDefensiveScenarios() => [
  ...
  suggesterRow(label: 'negative entries in available/need maps are silently dropped (no throw)', availableStockpileByCommodityId: const {'timber': -5, 'iron': 10}, commodityNeedByCommodityId: const {'coal': -3, 'wool': 4}, expect: const SuggesterExpectation(offersLength: 1, bidsLength: 1, offerCommodityIds: ['iron'], bidCommodityIds: ['wool'])),
];
// dart format on
...
List<TradeOrderSuggesterScenario> tradeOrderSuggesterSurplusOfferScenarios() => [
  ...
  suggesterRow(label: 'riches commodities are excluded from offers', availableStockpileByCommodityId: const {'spices': 100, 'gold': 50, 'timber': 10}, expect: const SuggesterExpectation(offersLength: 1, offerCommodityIds: ['timber'])),
  suggesterRow(label: 'offers iterate in alphabetical commodity id order (determinism)', availableStockpileByCommodityId: const {'wool': 4, 'coal': 2, 'iron': 3, 'timber': 1}, expect: const SuggesterExpectation(offerCommodityIds: ['coal', 'iron', 'timber', 'wool'])),
];
// dart format on
...
List<TradeOrderSuggesterScenario> tradeOrderSuggesterBidTypeCapScenarios() => [
  ...
  suggesterRow(label: 'bidTypeCap=3 admits the first three alphabetical commodities only', commodityNeedByCommodityId: const {'wool': 5, 'coal': 5, 'timber': 5, 'iron': 5}, expect: const SuggesterExpectation(bidCommodityIds: ['coal', 'iron', 'timber']), refs: '#2989'),
  suggesterRow(label: 'bidTypeCap=6 admits up to six distinct commodities', bidTypeCap: 6, commodityNeedByCommodityId: const {'cattle': 1, 'coal': 1, 'cotton': 1, 'grain': 1, 'hides': 1, 'iron': 1, 'timber': 1}, expect: const SuggesterExpectation(bidsLength: 6, bidCommodityIds: ['cattle', 'coal', 'cotton', 'grain', 'hides', 'iron']), refs: '#2989'),
];
// dart format on
...
List<TradeOrderSuggesterScenario> tradeOrderSuggesterCargoCapScenarios() => [
  suggesterRow(label: 'cargo budget is consumed across distinct bids (per-buyer total)', tradeCargoCapacity: 6, commodityNeedByCommodityId: const {'coal': 4, 'iron': 5}, expect: const SuggesterExpectation(bidQuantities: [(commodityId: 'coal', quantity: 4), (commodityId: 'iron', quantity: 2)]), refs: '#2989'),
...
List<TradeOrderSuggesterScenario> tradeOrderSuggesterValidatorCleanScenarios() => [
  ...
  suggesterRow(label: 'mixed surplus/deficit submission produces no commodity in both lists', bidTypeCap: 6, availableStockpileByCommodityId: const {'timber': 50, 'wool': 5}, commodityNeedByCommodityId: const {'wool': 8, 'iron': 4}, expect: const SuggesterExpectation(offerBidDisjoint: true, offersContain: {'timber'}, bidsContainAll: {'wool', 'iron'})),
];
// dart format on
...
List<TradeOrderSuggesterScenario> tradeOrderSuggesterTreasuryCapScenarios() => [
  suggesterRow(label: 'treasury budget is consumed across distinct bids in id order', bidTypeCap: 6, treasuryBudgetForBids: 90, worldMarketState: const WorldMarketState(prices: {'iron': 30, 'timber': 30}), commodityNeedByCommodityId: const {'iron': 5, 'timber': 5}, expect: suggesterSingleBidExpect('iron', 3), refs: '#3123'),
  suggesterRow(label: 'single bid is partial-capped by treasury', treasuryBudgetForBids: 90, worldMarketState: const WorldMarketState(prices: {'timber': 30}), commodityNeedByCommodityId: const {'timber': 5}, expect: suggesterSingleBidExpect('timber', 3, bidsLength: null), refs: '#3123'),
  suggesterRow(label: 'zero treasury budget suppresses bids entirely', treasuryBudgetForBids: 0, worldMarketState: const WorldMarketState(prices: {'timber': 30}), commodityNeedByCommodityId: const {'timber': 5}, expect: const SuggesterExpectation(bidsEmpty: true), refs: '#3123'),
];
// dart format on
