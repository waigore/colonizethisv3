...
final List<MaxAffordableBidQuantityScenario> maxAffordableBidQuantityScenarios = [
  maxAffordableBidQtyRow(label: 'floor(treasury / price) when price is positive', expected: 3, refs: '#3115'),
  maxAffordableBidQtyRow(label: 'zero treasury budget yields zero', remainingTreasuryBudget: 0, expected: 0, refs: '#3115'),
  maxAffordableBidQtyRow(label: 'missing-price free-fill returns bid remaining', bidRemaining: 8, pricePerUnit: 0.0, remainingTreasuryBudget: 10, expected: 8, refs: '#3115'),
  maxAffordableBidQtyRow(label: 'negative price preserves free-fill contract', bidRemaining: 5, pricePerUnit: -1.0, remainingTreasuryBudget: 0, expected: 5, refs: '#3115'),
];
// dart format on
...
final List<DecrementTreasuryForFillScenario> decrementTreasuryForFillScenarios = [
  decrementTreasuryFillRow(label: 'decrements running treasury tally after a priced fill', matchQty: 3, pricePerUnit: 30.0, expectedTreasury: 10, refs: '#3856'),
  decrementTreasuryFillRow(label: 'skips decrement on missing-price free-fill path', matchQty: 5, pricePerUnit: 0.0, expectedTreasury: 100, refs: '#3856'),
];
// dart format on
...
List<GpTreasuryCreditScenario<int>> gpTreasuryCreditIntScenarios() => [
  gpTreasuryCreditScenarioExpect<int>(label: 'starts empty with a zero total', expect: const GpTreasuryCreditExpectation<int>(isEmpty: true, total: 0, view: {})),
  gpTreasuryCreditScenarioExpect<int>(label: 'add creates and accumulates entries, total stays incremental', setup: (acc) => acc..add('gpA', 10)..add('gpB', 5)..add('gpA', 3), expect: const GpTreasuryCreditExpectation<int>(view: {'gpA': 13, 'gpB': 5}, total: 18, isEmpty: false)),
  gpTreasuryCreditScenarioExpect<int>(label: 'view preserves first-seen insertion order', setup: (acc) => acc..add('gpC', 1)..add('gpA', 1)..add('gpB', 1)..add('gpA', 1), expect: const GpTreasuryCreditExpectation<int>(viewKeyOrder: ['gpC', 'gpA', 'gpB'])),
  ...
  gpTreasuryCreditScenarioExpect<int>(label: 'incremental total equals the naive re-summed view total', setup: (acc) => acc..add('gpA', 7)..add('gpB', 11)..add('gpA', 2), expect: const GpTreasuryCreditExpectation<int>(totalEqualsNaiveViewSum: true)),
];

List<GpTreasuryCreditScenario<double>> gpTreasuryCreditDoubleScenarios() => [
  gpTreasuryCreditScenarioExpect<double>(label: 'ensure records a zero entry without changing the total', setup: (acc) => acc..add('gpA', 40.0)..ensure('gpB'), expect: const GpTreasuryCreditExpectation<double>(view: {'gpA': 40.0, 'gpB': 0.0}, total: 40.0)),
  gpTreasuryCreditScenarioExpect<double>(label: 'ensure is a no-op when the key already has a credit', setup: (acc) => acc..add('gpA', 12.5)..ensure('gpA'), expect: const GpTreasuryCreditExpectation<double>(view: {'gpA': 12.5}, total: 12.5)),
  gpTreasuryCreditScenarioExpect<double>(label: 'total matches naive re-sum including a zero-profit entry', setup: (acc) => acc..add('gpA', 4.6)..add('gpB', 40.0)..ensure('gpC')..add('gpA', 0.4), expect: const GpTreasuryCreditExpectation<double>(totalEqualsNaiveViewSum: true, viewCloseTo: {'gpC': 0.0})),
];
// dart format on
