...
class TechCapComparisonPin {
  const TechCapComparisonPin({
    required this.capsAndExpectedGrain,
    this.playerId = 'pl1',
  });
...
class BlockadedOverseasPin {
  const BlockadedOverseasPin({
    required this.topology,
    required this.blockadedPortProvincesByPlayerId,
    this.openOverseasCommodity = 'sugarCane',
    this.playerId = 'pl1',
  });
...
void assertBlockadedOverseasPin({
  required Game game,
  required Map<String, TileMapResult> tileMapByRegion,
  required BlockadedOverseasPin pin,
}) {
  final connectivityBlockaded = resolveConnectivity(
    game: game,
    tileMapByRegion: tileMapByRegion,
    topology: pin.topology,
    blockadedPortProvincesByPlayerId: pin.blockadedPortProvincesByPlayerId,
  );
  ...
  expect(
    resultOpen[pin.playerId]!.overseas[pin.openOverseasCommodity] ?? 0,
    greaterThan(0),
  );
}
