...
typedef CommoditiesWithBidAndOfferScenario = ({
  String label,
  List<TradeOrder> proposedOrders,
  Set<CommodityId> expected,
  String? refs,
});
...
List<CommoditiesWithBidAndOfferScenario> commoditiesWithBidAndOfferScenarios() => [
  ...
  (
    label: 'handles multiple overlapping commodities',
    proposedOrders: [
      testBid('timber', 5),
      testOffer('timber', 5),
      testBid('iron', 5),
...
typedef AdmittedBidCommodityIdsScenario = ({
  String label,
  List<TradeOrder> proposedOrders,
  int bidTypeCap,
  Set<CommodityId> mutuallyExcludedCommodityIds,
  Set<CommodityId> expected,
  String? refs,
});
...
List<AdmittedBidCommodityIdsScenario> admittedBidCommodityIdsScenarios() => [
  (
    label: 'admits distinct bid commodities in submission order up to the cap',
    proposedOrders: [
      testBid('wool', 1),
      testBid('iron', 1),
      testBid('coal', 1),
    ...
    expected: {'wool', 'iron', 'coal'},
    refs: '#3615',
  ),
  (
    label: 'a repeat bid on an admitted commodity does not consume a slot',
    proposedOrders: [
      testBid('wool', 1),
      testBid('wool', 2),
      testBid('iron', 1),
  ...
  (
    label: 'skips non-positive quantities, riches, and mutually-excluded ids',
    proposedOrders: [
      testBid('timber', 0),
      testBid('gold', 5),
      testBid('iron', 5),
    ...
    bidTypeCap: 6,
    mutuallyExcludedCommodityIds: {'iron'},
    expected: {'wool', 'coal'},
    refs: '#3615',
  ),
  (
    label: 'returns const empty set when bidTypeCap <= 0',
...
void verifyAdmittedBidCommodityIdsScenario(AdmittedBidCommodityIdsScenario scenario) {
  final admitted = admittedBidCommodityIdsInSubmissionOrder(
    ...
    mutuallyExcludedCommodityIds: scenario.mutuallyExcludedCommodityIds,
  );
