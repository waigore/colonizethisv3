...
List<ApplyExtractionToStockpileScenario> applyExtractionToStockpileScenarios() => [
  ...
  applyExtractionToStockpileScenario(label: 'accumulates on existing stockpile', initialDeltas: {'grain': 3, 'meat': 1}, extracted: {'grain': 4, 'meat': 2}, expectedQuantities: {'grain': 7, 'meat': 3}),
  applyExtractionToStockpileScenario(label: 'ignores negative values in extracted', initialDeltas: {'grain': 10}, extracted: {'grain': -2, 'meat': 3}, expectedQuantities: {'grain': 10, 'meat': 3}),
  applyExtractionToStockpileScenario(label: 'zero quantities do not change stockpile', initialDeltas: {'grain': 5}, extracted: {'grain': 0, 'iron': 0}, expectedQuantities: {'grain': 5}),
...
List<ApplyExtractionForPlayersScenario> applyExtractionForPlayersScenarios() => [
  applyExtractionForPlayersScenario(
    ...
    stockpilePins: [(playerIndex: 0, commodityId: 'grain', quantity: 3), (playerIndex: 0, commodityId: 'iron', quantity: 1), (playerIndex: 1, commodityId: 'iron', quantity: 2), (playerIndex: 1, commodityId: 'coal', quantity: 4)],
  ),
  applyExtractionForPlayersScenario(
    label: 'players with no extraction keep existing stockpile',
    game: minimalTwoPlayerGame(
      players: [
        const Player(id: 'p1', displayName: 'A', isHuman: true).copyWith(stockpile: const Stockpile().applyDelta('grain', 7)),
    ...
    stockpilePins: [(playerIndex: 0, commodityId: 'grain', quantity: 7), (playerIndex: 1, commodityId: 'iron', quantity: 2)],
  ),
  applyExtractionForPlayersScenario(
