...
List<TradeOrderValidatorScenario> tradeOrderValidatorTreasuryCapScenarios() => [
  validatorTreasuryTimberIronBids(label: 'rejects bid when cumulative spend exceeds treasuryBudgetForBids', treasuryBudgetForBids: 60, proposedOrders: [validatorBid('timber', 1), validatorBid('iron', 2)], expect: ValidatorExpectation(outcomes: [(accepted: true, reason: null), (accepted: false, reason: TradeOrderRejectionReasons.bidExceedsTreasuryBudget)]), refs: '#3093'),
  ...
  validatorTreasuryTimberBid(label: 'treasury cap takes precedence over bidExceedsCargoCapacity (rule 5 before rule 6)', treasuryBudgetForBids: 10, tradeCargoCapacity: 100, bidQty: 5, expect: ValidatorExpectation(singleRejectedWithReason: TradeOrderRejectionReasons.bidExceedsTreasuryBudget), refs: '#3093'),
  ...
  validatorTreasuryTimberIronBids(label: 'rejected bid does not consume the running spend budget — greedy continuation admits a later smaller bid that fits (Refs #3123)', treasuryBudgetForBids: 100, timberPrice: 30, ironPrice: 10, proposedOrders: [validatorBid('timber', 4), validatorBid('iron', 1)], expect: ValidatorExpectation(outcomes: [(accepted: false, reason: TradeOrderRejectionReasons.bidExceedsTreasuryBudget), (accepted: true, reason: null)], orderAcceptedPin: (index: 1, accepted: true, reason: 'greedy continuation: rejected bid must not consume the running spend budget so a later bid that fits the remaining budget is admitted')), refs: '#3123'),
];

List<TradeOrderValidatorScenario>
tradeOrderValidatorTreasuryCatalogScenarios() => [
  validatorTreasuryTimberBids(label: 'treasuryBudgetForBids == 0 rejects every priced bid (Refs #3123)', treasuryBudgetForBids: 0, proposedOrders: [validatorBid('timber', 1), validatorBid('timber', 5)], expect: const ValidatorExpectation(allRejectedWithReason: TradeOrderRejectionReasons.bidExceedsTreasuryBudget), refs: '#3123'),
