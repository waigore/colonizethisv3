...
import 'support/split_home_fleet_harness.dart';

void main() {
  suppressLogsForTests();

  testWidgets('taps Split inside naval panel, performs exactly one nudge, '
      'and confirms when one nudge enables the confirm button', (
    WidgetTester tester,
  ) async {
    final l10n = lookupAppLocalizations(const Locale('en'));
    final controller = SplitHarnessController(nudgesUntilConfirm: 1);
    await pumpSplitHarness(tester, controller: controller, l10n: l10n);

    await e2eSplitHomeFleetOnce(tester, l10n, navalPanelAlreadyOpen: true);
  ...
  testWidgets('falls back to the single-nudge `ctTransfer.left.>` button '
      'when the move-all `>>` button is absent', (WidgetTester tester) async {
    final l10n = lookupAppLocalizations(const Locale('en'));
    final controller = SplitHarnessController(
  ...
  testWidgets('iterates the bounded nudge loop until `splitFleet_confirm` '
      'becomes enabled when several nudges are required', (
    WidgetTester tester,
  ) async {
    final l10n = lookupAppLocalizations(const Locale('en'));
    final controller = SplitHarnessController(nudgesUntilConfirm: 3);
    await pumpSplitHarness(tester, controller: controller, l10n: l10n);
  ...
  testWidgets('short-circuits the nudge loop when `splitFleet_confirm` '
      'is already enabled on dialog entry', (WidgetTester tester) async {
    final l10n = lookupAppLocalizations(const Locale('en'));
    final controller = SplitHarnessController(nudgesUntilConfirm: 0);
    await pumpSplitHarness(tester, controller: controller, l10n: l10n);
  ...
  testWidgets('expands every collapsed `ExpansionTile` left in the naval panel '
      'after the split dialog dismisses', (WidgetTester tester) async {
    final l10n = lookupAppLocalizations(const Locale('en'));
    final controller = SplitHarnessController(nudgesUntilConfirm: 1);
    await pumpSplitHarness(
      tester,
    ...
    expect(
      ...
      isFalse,
      reason:
          'Expansion tile must start collapsed for the trailing '
