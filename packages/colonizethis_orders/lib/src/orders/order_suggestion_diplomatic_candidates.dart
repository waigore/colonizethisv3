...
import 'order_suggestion_diplomatic_target_ids.dart';
import 'validators/diplomatic/diplomatic_sub_validator.dart';

export 'order_suggestion_diplomatic_target_ids.dart';
...
DiplomaticOrder? offerPeaceSuggestionCandidate(
  ...
  if (!target.knownTargetIds.contains(target.targetId) || rel?.atWar != true) {
    return null;
  }
...
List<DiplomaticOrder> setSubsidySuggestionCandidates(
  ...
  final overtureRow = target.playerOverturesByTargetId[target.targetId];
  if (!targetIsMinorOrTribe || overtureRow == null || !overtureRow.hasEmbassy) {
    return const [];
  }
  return [
    DiplomaticOrder(
