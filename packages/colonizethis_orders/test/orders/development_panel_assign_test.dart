...
void main() {
  runLabeledScenarioGroup('development panel assign (Refs #4175 Slice B)', [
    rs(
      'compareDevelopmentImproveTilePriority prefers connected then lower level',
      () {
    ...
    rs('idleBuildersForDevelopmentAssign uses stable unit id order', () {
    ...
    rs('idleBuildersForDevelopmentAssign excludes pending work units', () {
      final setup = OscDualBuilderGrainTiles();
      final idle = idleBuildersForDevelopmentAssign(
        game: setup.game(),
        playerId: OscIds.playerId,
    ...
    rs(
      'resolveDevelopmentAssignRowState disables when materials insufficient',
      () {
        final setup = OscDualBuilderGrainTiles();
    ...
    rs(
      'resolveDevelopmentAssignRowState enables with materials and connectivity',
      () {
        final setup = OscDualBuilderGrainTiles();
        final state = resolveDevelopmentAssignRowState(
          game: setup.game(),
    ...
    rs('developmentPanelMaterialShortageCommodityIds flags blocked rows', () {
      final setup = OscDualBuilderGrainTiles();
      final brokePlayer = setup.player.copyWith(
        stockpile: const Stockpile(),
      );
