// Table-driven civilian work affordance scenarios (Refs #4262).
...
import 'suggestion/order_suggestion_core_fixtures.dart';

// dart format off

void cwaRunPreviewShortfallAfterPending() {
...
void cwaRunPendingSecondOrderUnaffordable() {
  final s = OscDualBuilderGrainTiles();
  final lowStockGame = s.game().copyWith(
    players: [
      s.player.copyWith(stockpile: oscLumberCastIronStockpile(amount: 1)),
...
void cwaRunFreeTargetsSkipCostPreview() {
  final s = OscDualBuilderGrainTiles();
  final preview = previewWorkOrderAffordAtTile(
    game: s.game(),
    playerId: OscIds.playerId,
...
List<RunnableScenario> civilianWorkAffordanceScenarios() => [
  rs(
    'previewWorkOrderAffordAtTile reports shortfall after pending work',
    cwaRunPreviewShortfallAfterPending,
    '#4262',
  ),
  rs(
    'previewPendingWorkOrderAfford marks second order unaffordable',
    cwaRunPendingSecondOrderUnaffordable,
    '#4262',
  ),
  rs('free targets skip cost preview', cwaRunFreeTargetsSkipCostPreview, '#4262'),
];
