// Compact orders-domain logging assertions (Refs #3949 wave 3).
...
/// Pins for [ordersLoggingScenarios] rows.
enum OrdersLoggingTarget {
  ordersLogIsCtLogger,
  ordersLogIsSharedInstance,
  ordersApplicationLogAlias,
  orderSuggestionLogPrefix,
  ordersLogEmitsPrefixedMessages,
  noLibOrdersSourceConsumesLogicLog,
}

void runOrdersLoggingExpectation(OrdersLoggingTarget target) {
  switch (target) {
    case OrdersLoggingTarget.ordersLogIsCtLogger:
      expect(ordersLog, isA<CtLogger>());
      expect(ordersLog.prefix, equals('orders'));

    case OrdersLoggingTarget.ordersLogIsSharedInstance:
      expect(identical(ordersLog, ordersLog), isTrue);

    case OrdersLoggingTarget.ordersApplicationLogAlias:
      expect(identical(ordersApplicationLog, ordersLog), isTrue);

    case OrdersLoggingTarget.orderSuggestionLogPrefix:
      expect(orderSuggestionLog, isA<CtLogger>());
      expect(orderSuggestionLog.prefix, equals('orders.order_suggestion'));

    case OrdersLoggingTarget.ordersLogEmitsPrefixedMessages:
    ...
    case OrdersLoggingTarget.noLibOrdersSourceConsumesLogicLog:
