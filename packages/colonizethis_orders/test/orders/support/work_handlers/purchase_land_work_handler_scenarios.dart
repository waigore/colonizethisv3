...
void plwhRunTryApplyWithoutTreasuryDeduction() {final game = purchaseLandTryApplyGame(); final merchant = game.worldState.oldWorld.units.single; final work = WorkOrderState( unitsById: (oldWorld: {merchant.id: merchant}, newWorld: const {}), tileState: game.worldState.tileState, visibilityByTile: const {}, portsByProvinceSeaboard: const {}, purchasedTilesByTileKey: const {}, oldProvinces: List<Province>.from(game.worldState.oldWorld.provinces), newProvinces: const [], ); var state = BuildWorkState( game: game, buildOrders: const {}, workOrders: const {}, work: work, ); final player = game.players.single; final context = WorkOrderExecutionContext(state: state, player: player); final handler = purchaseLandWorkOrderHandler; const order = WorkOrder( unitId: 'merchant1', target: kWorkTargetPurchaseLand, targetTileKey: purchaseLandTileKey, ); final applied = handler.tryApply( context, order, merchant, purchaseLandTileKey, true, ); expect(applied, isTrue); expect(context.treasury, 500); expect( context.purchasedTilesByTileKey.containsKey(purchaseLandTileKey), isFalse, ); final updatedMerchant = context.state.work.unitById('merchant1'); expect(updatedMerchant, isNotNull); expect(updatedMerchant!.status, UnitStatus.working); expect(updatedMerchant.tileKey, purchaseLandTileKey); expect(updatedMerchant.currentWork?.workTarget, kWorkTargetPurchaseLand); expect(updatedMerchant.currentWork?.remainingTurns, 1);}
