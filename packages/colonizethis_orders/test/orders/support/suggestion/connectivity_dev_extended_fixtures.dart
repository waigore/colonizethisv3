...
Map<String, TileMapResult> tileMapByRegionFromGame(Game game) => {for (final e in game.worldState.tileKeysByRegionAndProvince.entries) e.key: tileMapFromProvinceTileKeys(e.value)};

/// AC-A5: foreign-province barrier blocks owned-land extension toward resource.
({Game game, MapTopology topology, Map<String, TileMapResult> tileMapByRegion, String capTile}) connectivityDevForeignBarrierCase() {
  const ow = connectivityDevOw, pCapital = '$ow|pCap', pForeign = '$ow|pFor', pResource = '$ow|pRes', capTile = '$pCapital|2|0', resourceTile = '$pResource|0|2';
  final tileMap = TileMapResult(width: 3, height: 3, grid: const [['pCap', 'pCap', 'pCap'], ['pFor', 'pFor', 'pFor'], ['pRes', 'pRes', 'pRes']]);
  final topology = MapTopology(nodes: [TopologyNode(id: pCapital, regionId: ow, type: TopologyNodeType.province), TopologyNode(id: pForeign, regionId: ow, type: TopologyNodeType.province), TopologyNode(id: pResource, regionId: ow, type: TopologyNodeType.province)], edges: [TopologyEdge(id1: pCapital, id2: pForeign), TopologyEdge(id1: pForeign, id2: pResource)]);
  final capTiles = [for (var x = 0; x < 3; x++) '$pCapital|$x|0'], resTiles = [for (var x = 0; x < 3; x++) '$pResource|$x|2'];
  final game = ordersOwRegionGame(players: [Player(id: 'gp1', displayName: 'GP', isHuman: false, capitalProvinceId: pCapital, capitalTile: CapitalTile(regionId: ow, provinceId: pCapital, x: 2, y: 0))], oldWorld: RegionData(provinces: [Province(id: pCapital, regionId: ow, ownerId: 'gp1'), Province(id: pForeign, regionId: ow, ownerId: 'minor1'), Province(id: pResource, regionId: ow, ownerId: 'gp1')]), tileKeysByRegionAndProvince: {ow: {'pCap': capTiles, 'pRes': resTiles}}, resourceByTileKey: {resourceTile: 'iron'}, tileState: const TileMapState(improvementByTile: {resourceTile: 1}));
  return (game: game, topology: topology, tileMapByRegion: {ow: tileMap}, capTile: capTile);
}

/// AC-A7: frontier tile wins lex-smaller non-frontier under road probe cap.
({ConnectivityDevSnapshot snapshot, String frontierTile, List<String> lexSorted}) connectivityDevFrontierRoadLexCase() {
  const frontierTile = 'oldWorld|p1|2|4';
  final lexSorted = <String>[for (var y = 0; y < 5; y++) for (var x = 0; x < 5; x++) 'oldWorld|p1|$x|$y'];
  final snapshot = ConnectivityDevSnapshot(connected: {'oldWorld|p1|4|4', 'oldWorld|p1|3|4', 'oldWorld|p1|4|3'}, pathTransportCap: const {}, extensionDistanceByTile: {frontierTile: 6, 'oldWorld|p1|3|3': 6, 'oldWorld|p1|4|2': 6}, seaZonesReachableFromCapital: const {}, provincesWithUnconnectedDevTargets: {'oldWorld|p1'}, hasUnconnectedDevTargets: true, frontierExtensionTiles: {frontierTile, 'oldWorld|p1|3|3', 'oldWorld|p1|4|2'}, bottleneckRailTiles: const {}, adjacentToConnectedTiles: {frontierTile, 'oldWorld|p1|3|3', 'oldWorld|p1|4|2'});
...
({ConnectivityDevSnapshot snapshot, String townResourceTile, String frontierTile}) connectivityDevTownRuleFrontierCase() {
  const townResourceTile = 'oldWorld|p1|2|2', frontierTile = 'oldWorld|p1|3|2';
  final snapshot = ConnectivityDevSnapshot(connected: {townResourceTile, 'oldWorld|p1|0|0'}, pathTransportCap: const {}, extensionDistanceByTile: const {frontierTile: 1}, seaZonesReachableFromCapital: const {}, provincesWithUnconnectedDevTargets: const {'oldWorld|p1'}, hasUnconnectedDevTargets: true, frontierExtensionTiles: {frontierTile}, bottleneckRailTiles: const {}, adjacentToConnectedTiles: {frontierTile});
  return (snapshot: snapshot, townResourceTile: townResourceTile, frontierTile: frontierTile);
}

/// AC-F6 human suggestion parity for build_improvement connectivity ordering.
({Game game, MapTopology topology, Map<String, TileMapResult> tileMapByRegion, String connectedGrain, String farGrain, ConnectivityDevSnapshot snapshot}) connectivityDevSuggestionParityCase() {
  const ow = 'oldWorld', p1 = '$ow|p1', playerId = 'gp1', capTile = '$p1|2|2', connectedGrain = '$p1|0|0', farGrain = '$p1|4|0';
  final tileMap = TileMapResult(width: 5, height: 5, grid: List.generate(5, (_) => List.filled(5, 'p1')));
  const topology = MapTopology(nodes: [TopologyNode(id: p1, regionId: ow, type: TopologyNodeType.province)], edges: []);
  final tileState = TileMapState(improvementByTile: const {connectedGrain: 0, farGrain: 0}, roadLevelByTile: {capTile: 1, '$p1|2|1': 1, '$p1|1|1': 1, '$p1|1|0': 1, connectedGrain: 1});
  final tiles = [for (var y = 0; y < 5; y++) for (var x = 0; x < 5; x++) '$p1|$x|$y'];
  final game = ordersOwRegionGame(players: [Player(id: playerId, displayName: 'Human', isHuman: true, capitalProvinceId: p1, capitalTile: CapitalTile(regionId: ow, provinceId: p1, x: 2, y: 2), stockpile: const Stockpile(quantities: {'lumber': 20, 'castIron': 20}))], oldWorld: RegionData(provinces: [Province(id: p1, regionId: ow, ownerId: playerId)], units: [Unit(id: 'b1', type: kUnitTypeBuilder, ownerId: playerId, locationProvinceId: p1, tileKey: capTile)]), tileKeysByRegionAndProvince: {ow: {p1: tiles}}, resourceByTileKey: {connectedGrain: 'grain', farGrain: 'grain'}, tileState: tileState, playerVisibilityByTile: {playerId: {for (final t in tiles) t: 'fullyVisible'}});
  final tileMapByRegion = {ow: tileMap};
  final snapshot = buildConnectivityDevSnapshot(game: game, playerId: playerId, topology: topology, tileMapByRegion: tileMapByRegion)!;
  return (game: game, topology: topology, tileMapByRegion: tileMapByRegion, connectedGrain: connectedGrain, farGrain: farGrain, snapshot: snapshot);
}
