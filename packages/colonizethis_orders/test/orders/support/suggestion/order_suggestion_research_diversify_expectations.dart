// Compact research diversification assertions (Refs #3949 wave 3).
...
/// Pins for [orderSuggestionResearchDiversifyScenarios] rows.
enum OrderSuggestionResearchDiversifyTarget {
  slot1TakesHighestWeightUnrepresentedBucket,
  weightZeroMatchesGreedyDefault,
}

void runOrderSuggestionResearchDiversifyExpectation(
  OrderSuggestionResearchDiversifyTarget target,
) {
  switch (target) {
    case OrderSuggestionResearchDiversifyTarget
        .slot1TakesHighestWeightUnrepresentedBucket:
      final pool = researchDiversifyGreedyPool();
      final slot0Bucket =
          researchCategoryBucketOf(pool.first.category);

      final altTech = pool.skip(1).firstWhere(
    ...
    case OrderSuggestionResearchDiversifyTarget.weightZeroMatchesGreedyDefault:
      final player = const Player(
        id: researchDiversifyPlayerId,
        displayName: 'GP',
        isHuman: false,
