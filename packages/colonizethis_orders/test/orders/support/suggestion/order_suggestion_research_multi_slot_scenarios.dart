...
void osrmsRunFillsEveryEmptySlotWithDistinctResearchableTech() {
  const player = Player(id: researchDiversifyPlayerId, displayName: 'GP', isHuman: false, treasury: 1000, researchSlots: 3);
  final game = researchDiversifyGameFor(player);
  final view = buildPlayerView(game, researchDiversifyTopology, researchDiversifyPlayerId);
  final suggestions = suggestResearchOrders(view, game, researchDiversifyTopology, const Orders());
  expect(suggestions.length, greaterThanOrEqualTo(2), reason: 'multiple era-1 techs are researchable from an empty state');
  expect(suggestions.length, lessThanOrEqualTo(3), reason: 'never exceeds the player research slot count');
  final slotIndices = suggestions.map((o) => o.slotIndex).toSet();
  expect(slotIndices.length, suggestions.length, reason: 'each suggestion targets a distinct slot');
  for (final slot in slotIndices) {expect(slot, inInclusiveRange(0, 2));}
  final techIds = suggestions.map((o) => o.techId).toSet();
  expect(techIds.length, suggestions.length, reason: 'each suggestion targets a distinct tech');
}
...
void osrmsRunDoesNotReSuggestTechAlreadyAssignedByPendingOrder() {
  final researchable = researchableTechIds(const <String, bool>{}).toList()..sort();
  expect(researchable.length, greaterThanOrEqualTo(2));
  final pendingTechId = researchable.first;
  const player = Player(id: researchDiversifyPlayerId, displayName: 'GP', isHuman: false, treasury: 1000, researchSlots: 3);
  final game = researchDiversifyGameFor(player);
  final view = buildPlayerView(game, researchDiversifyTopology, researchDiversifyPlayerId);
  final pending = Orders(researchOrdersByPlayerId: {researchDiversifyPlayerId: [ResearchOrder(slotIndex: 0, techId: pendingTechId, funding: ResearchFundingLevel.medium)]});
  final suggestions = suggestResearchOrders(view, game, researchDiversifyTopology, pending);
  expect(suggestions.map((o) => o.techId), isNot(contains(pendingTechId)), reason: 'a tech already taken by a pending order is not re-suggested');
  expect(suggestions.map((o) => o.slotIndex), isNot(contains(0)), reason: 'slot 0 is already taken by the pending order');
}
