...
void osaiRunSuggestMoveOrdersReturnsList() {final game = apiImplDefaultGame(); final view = apiImplViewFor(game,apiImplBaseTopology); final list = _api.suggestMoveOrders(view,game,apiImplBaseTopology,apiImplEmptyOrders,); expect(list,isA<List<MoveOrder>>());}

void osaiRunSuggestWorkOrdersReturnsList() {final game = apiImplDefaultGame(); final view = apiImplViewFor(game,apiImplBaseTopology); final list = _api.suggestWorkOrders(view,game,apiImplBaseTopology,apiImplEmptyOrders,); expect(list,isA<List<WorkOrder>>());}

void osaiRunSuggestBuildOrdersReturnsList() {final game = apiImplDefaultGame(); final view = apiImplViewFor(game,apiImplBaseTopology); final list = _api.suggestBuildOrders(view,game,apiImplBaseTopology,apiImplEmptyOrders,); expect(list,isA<List<BuildUnitOrder>>());}

void osaiRunSuggestBuildOrdersIncludesShipWhenAffordable() {final game = apiImplAffordableShipGame(); final view = apiImplViewFor(game,apiImplSingleProvinceTopology); final list = _api.suggestBuildOrders(view,game,apiImplSingleProvinceTopology,apiImplEmptyOrders,); final shipBuilds = list.where((o) => ShipEconomyCatalog.byId.containsKey(o.unitType)).toList(); expect(shipBuilds,isNotEmpty,reason: 'API should suggest ship builds when affordable',);}

void osaiRunSuggestResearchOrdersReturnsList() {final game = apiImplDefaultGame(); final view = apiImplViewFor(game,apiImplBaseTopology); final list = _api.suggestResearchOrders(view,game,apiImplBaseTopology,apiImplEmptyOrders,); expect(list,isA<List<ResearchOrder>>());}

void osaiRunSuggestNavalMoveOrdersReturnsList() {final game = apiImplDefaultGame(); final view = apiImplViewFor(game,apiImplBaseTopology); final list = _api.suggestNavalMoveOrders(view,game,apiImplBaseTopology,apiImplEmptyOrders,); expect(list,isA<List<NavalMoveOrder>>());}
...
void osaiRunSuggestRecruitWorkerOrdersReturnsList() {final game = apiImplDefaultGame(); final view = apiImplViewFor(game,apiImplBaseTopology); final list = _api.suggestRecruitWorkerOrders(view,game,apiImplBaseTopology,apiImplEmptyOrders,); expect(list,isA<List<RecruitWorkerOrder>>());}

void osaiRunSuggestRecruitWorkerOrdersIncludesPeasantWhenFabricAffordable() {final game = apiImplFabricRecruitGame(); final view = apiImplViewFor(game,apiImplSingleProvinceTopology); final list = _api.suggestRecruitWorkerOrders(view,game,apiImplSingleProvinceTopology,apiImplEmptyOrders,); expect(list.any((o) => o.targetTier == WorkerTier.peasant),isTrue,reason: 'API impl must surface peasant recruit when 2 fabric affords ' 'the cost row',);}

List<RunnableScenario> orderSuggestionApiImplScenarios() => [
