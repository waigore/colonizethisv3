// Scenario run tear-offs for order_suggestion_work_logging (Refs #3949 wave 3).
...
void oswlRunLoggerLinesNeverEmitUnboundedFullListPayload() {
  withWspLogCapture((events) {
...
void oswlRunMultipleProspectTilesEmitIncludedCount() {
  withWspLogCapture((events) {
    ...
    expect(
      ...
      reason: 'fixture must surface multiple prospect rows to assert summary',
    );

    final prospectLines = wspSuggestWorkLines(
      events,
...
void oswlRunPendingTargetsPreserveDuplicateCheckAndLogOrdering() {
  withWspLogCapture((events) {
    ...
    final explorerLines = wspSuggestWorkLines(
      events,
