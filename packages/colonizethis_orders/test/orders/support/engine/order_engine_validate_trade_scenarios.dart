...
void vetRunAcceptsValidOfferWhenStockpileCoversQuantity() {final game = vetGameWith(player: vetGp1(stockpile: Stockpile(quantities: {CommodityCatalog.timber.id: 10}),),); final engine = vetTradeEngine()..addTradeOrderWithContext(game,vetTopology,'gp1',validatorOffer(CommodityCatalog.timber.id,5),); final results = vetValidate(game,engine); expect(results,hasLength(1)); expect(results.single.isAccepted,isTrue);}

void vetRunRejectsMutualExclusionWhenBidAndOfferShareACommodity() {final game = vetGameWith(player: vetGp1(stockpile: Stockpile(quantities: {CommodityCatalog.timber.id: 20}),),overtures: vetEmbassyOverture,); final engine = vetTradeEngine()..addTradeOrderWithContext(game,vetTopology,'gp1',validatorOffer(CommodityCatalog.timber.id,5),)..addTradeOrderWithContext(game,vetTopology,'gp1',validatorBid(CommodityCatalog.timber.id,3),); final results = vetValidate(game,engine); expect(results.every((r) => !r.isAccepted),isTrue); expect(results.map((r) => r.reason).toSet(),{TradeOrderRejectionReasons.mutualExclusion,});}
