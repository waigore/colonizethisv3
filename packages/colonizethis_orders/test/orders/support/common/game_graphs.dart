...
//
// Parameterized builders for high-frequency empty-region two-GP and
// owned-province shapes. Prefer [TestFixtures.minimalGame] when isomorphic.
...
const Player ordersCommonGp1 = Player(id: 'gp1', displayName: 'GP1', isHuman: true);

/// Default GP2 used by many diplomatic / suggestion fixtures.
const Player ordersCommonGp2 = Player(id: 'gp2', displayName: 'GP2', isHuman: false);
...
DiplomacyRelation ordersGpRelation({
  ...
  num score = 50,
}) => DiplomacyRelation(factionId1: factionId1, factionId2: factionId2, state: state, level: level, formalAlliance: formalAlliance, score: score);

/// Province with optional prefixed id (`regionId|localId`).
Province ordersProvince({required String localId, String regionId = 'oldWorld', String? ownerId, bool prefixed = true}) =>
    Province(id: prefixed ? '$regionId|$localId' : localId, regionId: regionId, ownerId: ownerId);
...
Game ordersTwoGpEmptyGame({
  ...
  diplomacyRelations: diplomacyRelations ?? [ordersGpRelation(state: state, level: level, formalAlliance: formalAlliance, score: score)],
);
...
MapTopology ordersProvinceTopology(Iterable<Province> provinces, {String regionId = 'oldWorld', List<TopologyEdge> edges = const []}) => MapTopology(
  nodes: [for (final p in provinces) TopologyNode(id: p.id.contains('|') ? p.id.split('|').last : p.id, regionId: regionId, type: TopologyNodeType.province)],
  edges: edges,
);
// dart format on
