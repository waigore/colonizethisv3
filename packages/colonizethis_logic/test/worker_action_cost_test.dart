...
void main() {
  Player playerWithTech(Map<String, bool>? tech) => Player(
        id: 'p1',
        displayName: 'P1',
        isHuman: true,
        techUnlocked: tech,
  ...
  group('canAffordRecruitWorker', () {
    test('peasant recruit needs only fabric (no tech, no peasant, no treasury)',
        () {
      final stockpile =
          const Stockpile().applyDelta(CommodityCatalog.fabric.id, 2);
    ...
    test('peasant recruit rejected when fabric below cost', () {
      final stockpile =
          const Stockpile().applyDelta(CommodityCatalog.fabric.id, 1);
    ...
    test('apprentice rejected with tech-locked when required tech missing', () {
      final stockpile =
          const Stockpile().applyDelta(CommodityCatalog.paper.id, 10);
    ...
    test('apprentice rejected when materials below cost', () {
      final stockpile =
          const Stockpile().applyDelta(CommodityCatalog.paper.id, 1);
    ...
    test('apprentice allowed when tech, peasant, treasury, and materials meet '
      ...
      final stockpile =
          const Stockpile().applyDelta(CommodityCatalog.paper.id, 2);
  ...
  group('applyRecruitWorkerCostDeduction', () {
    test('peasant recruit adds a peasant and deducts only fabric', () {
      final stockpile =
          const Stockpile().applyDelta(CommodityCatalog.fabric.id, 5);
    ...
    test('apprentice train consumes a peasant, paper, and treasury', () {
      final stockpile =
          const Stockpile().applyDelta(CommodityCatalog.paper.id, 5);
    ...
    test('master train consumes a peasant, paper, and treasury', () {
      final stockpile =
          const Stockpile().applyDelta(CommodityCatalog.paper.id, 12);
