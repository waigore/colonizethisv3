...
void main() {
  ...
  group('applyExtractionForPlayers', () {
    test('applies per-player extraction to player stockpiles', () {
      ...
      expect(updated.players[0].stockpile.quantityOf(CommodityCatalog.grain.id), 3);
      expect(updated.players[0].stockpile.quantityOf(CommodityCatalog.iron.id), 1);
      expect(updated.players[1].stockpile.quantityOf(CommodityCatalog.iron.id), 2);
      expect(updated.players[1].stockpile.quantityOf(CommodityCatalog.coal.id), 4);
    });

    test('players with no extraction keep existing stockpile', () {
      var p1 = const Player(id: 'p1', displayName: 'A', isHuman: true)
          .copyWith(stockpile: const Stockpile().applyDelta(CommodityCatalog.grain.id, 7));
      final game = Game(
        id: 'g1',
        worldState: WorldState(
          turnState: const TurnState(phase: TurnPhase.orders, turnNumber: 0),
        ...
        players: [p1, const Player(id: 'p2', displayName: 'B', isHuman: false)],
      );
      final extractedByPlayerId = {
      ...
      expect(updated.players[0].stockpile.quantityOf(CommodityCatalog.grain.id), 7);
      expect(updated.players[1].stockpile.quantityOf(CommodityCatalog.iron.id), 2);
    });
