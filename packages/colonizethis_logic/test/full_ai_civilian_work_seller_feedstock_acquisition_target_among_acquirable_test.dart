...
// intersection. Reuses the below-quota zero-NW lock-recovery seller fixture from
// the selection test (gp1 owns an unimproved `wool` regiment-build-input
// feedstock tile, so the improvement-input gate is active and it needs
// `lumber` / `castIron`), then exercises the intersection of the topology-free
// feedstock candidate list with a caller-supplied acquirable province set.
const _sellerId = 'gp1';
...
void main() {
  group(
    ...
    '(Refs #2847 H8-extraction seller feedstock-tile acquisition intersection)',
    () {
      test(
        'returns only the feedstock candidates present in the acquirable set, '
        'in ascending province-id order',
        () {
          ...
          // Precondition: all three feedstock provinces are candidates.
          expect(
            sellerFeedstockTileAcquisitionTargetProvinceIdsSorted(
              game,
              _sellerId,
            ),
            equals(<String>['oldWorld|t1', 'oldWorld|t2', 'oldWorld|t3']),
          );
          expect(
            sellerFeedstockTileAcquisitionTargetsAmongAcquirable(
              game,
      ...
      test(
        'preserves ascending province-id order regardless of acquirable-set '
        'iteration order',
        () {
          final game = _flaggedSellerGame(
            resourceByTileKey: const {
      ...
      test('returns empty when the acquirable set is disjoint from candidates', () {
        final game = _flaggedSellerGame(
          resourceByTileKey: const {
            _grainTile: 'grain',
            _woolTile: 'wool',
        ...
        expect(
          sellerFeedstockTileAcquisitionTargetsAmongAcquirable(
            game,
            _sellerId,
            const {'oldWorld|t9', 'oldWorld|t8'},
          ),
          isEmpty,
        );
      });

      test('returns empty when the acquirable set is empty', () {
        final game = _flaggedSellerGame(
          resourceByTileKey: const {
            _grainTile: 'grain',
            _woolTile: 'wool',
      ...
      test(
        'returns empty when the acquisition residual is inactive even though '
      ...
      test('evaluation is deterministic', () {
        ...
        final a = sellerFeedstockTileAcquisitionTargetsAmongAcquirable(
          game,
          _sellerId,
          acquirable,
        );
        final b = sellerFeedstockTileAcquisitionTargetsAmongAcquirable(
          game,
          _sellerId,
          acquirable,
        );
