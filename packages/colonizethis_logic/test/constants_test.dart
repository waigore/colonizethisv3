...
void main() {
  ...
  group('GamePlayerLookup.fleetById', () {
    // Fleets live on WorldState; the cache key is Game so a `copyWith` that
    // replaces worldState (the canonical fleet mutation pattern) invalidates
    // implicitly per Refs #2575 Phase 4.
    Game gameWithFleets(List<Fleet> fleets) => Game(
      id: 'g-fleets',
      worldState: WorldState(
        turnState: TurnState(phase: TurnPhase.orders, turnNumber: 1),
        oldWorld: const RegionData(),
      ...
      players: const [Player(id: 'gp1', displayName: 'France', isHuman: true)],
    );

    test('returns fleet when id matches', () {
      final g = gameWithFleets([
        Fleet(id: 'f1', ownerId: 'gp1', regionId: 'oldWorld'),
        Fleet(id: 'f2', ownerId: 'gp1', regionId: 'oldWorld'),
      ]);
      expect(g.fleetById('f2')?.id, 'f2');
    });
    ...
    test('handles empty fleet list', () {
      final g = gameWithFleets(const []);
      expect(g.fleetById('anything'), isNull);
    });
    ...
    test('copyWith new worldState invalidates lookup for new instance', () {
      ...
      final updated = g.copyWith(
        worldState: g.worldState.copyWith(
          fleets: [
            Fleet(id: 'f1', ownerId: 'gp1', regionId: 'oldWorld'),
            Fleet(id: 'f2', ownerId: 'gp1', regionId: 'oldWorld'),
          ],
    ...
    test('repeated lookups reuse cached map (same Game instance)', () {
      final g = gameWithFleets([
        Fleet(id: 'f1', ownerId: 'gp1', regionId: 'oldWorld'),
        Fleet(id: 'f2', ownerId: 'gp1', regionId: 'newWorld'),
      ]);
      // First call seeds the Expando cache; second uses it. Both must hit.
      expect(g.fleetById('f1')?.regionId, 'oldWorld');
      expect(g.fleetById('f2')?.regionId, 'newWorld');
      expect(g.fleetById('missing'), isNull);
    });

    test('lookup scales across many distinct fleet ids', () {
      const count = 80;
      final fleets = List<Fleet>.generate(
        count,
        (i) => Fleet(
          id: 'fleet_$i',
          ownerId: 'gp1',
