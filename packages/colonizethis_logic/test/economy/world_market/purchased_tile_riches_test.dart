...
import 'package:colonizethis_logic/src/economy/world_market/purchased_tile_riches.dart';
  ...
  group('computePurchasedTileRichesCredits — riches handoff per #2991 C5', () {
    test(
      ...
      () {
        ...
        expect(result.treasuryCreditByGpId, equals({'gpA': richesBasePrice('gold')}));
    ...
    test('multi-tile aggregation — distinct GPs each accrue their own credits', () {
      const ow = 'oldWorld';
      ...
      expect(result.treasuryCreditByGpId.keys.toSet(), equals({'gpA', 'gpB'}));
      expect(
    ...
    test('determinism — two calls with the same inputs return equal credits', () {
      ...
      expect(
        r1.treasuryCreditByGpId,
        equals(r2.treasuryCreditByGpId),
      );
    });
  });
}
