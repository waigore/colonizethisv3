...
/// complement to [_ownsUnprospectedOldWorldMineralFeedstockTile]
...
bool ownsProspectedOldWorldMineralFeedstockTile(
  ...
  final ws = game.worldState;
  final prospected =
      ws.playerProspectedTiles[playerId] ?? const <String>{};
...
/// localization).
///
/// The Old World feedstock unit reservation
/// ([_resolveOwFeedstockReservation]) only holds back an **idle** Explorer; a
/// supplier whose Explorers are all busy (for example dispatched to multi-turn
/// New World exploration) has none to reserve, so its Old World `iron` mineral
/// tile is never prospected and `castIron` never becomes feasible. A near-zero
/// count on gate-active turns therefore localizes the residual to Explorer
/// availability rather than to candidate generation or improvement. Read-only
/// scan over the player's units; deterministic over `(game, playerId)`.
