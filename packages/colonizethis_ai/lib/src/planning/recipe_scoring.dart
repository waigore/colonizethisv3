// Recipe scoring helpers for the economy planner. Pure deterministic
// functions extracted from `economy_planner.dart` so the planner stays
// focused on labour allocation and cargo preference, while feasibility
// checks and recipe scoring live alongside related ProductionRecipe logic.
...
/// Sentinel for "no input bound" when computing feasible recipe runs.
const int kVeryLargeRuns = 999999;

/// Scored recipe candidate: pairs a recipe with its computed score so the
/// economy planner can rank candidates deterministically before allocating
/// labour. SPEC/ai/economy-planner.md § Allocation.
class ScoredRecipe {
  ScoredRecipe({required this.recipe, required this.score});

  final ProductionRecipe recipe;
  final double score;
}

/// Maximum number of full runs of [recipe] producible against the current
/// [stockpile] and [remainingLabour] budget. Returns 0 when no full run
/// is possible. SPEC/ai/economy-planner.md § Feasibility.
int feasibleRuns({
  ...
  int maxByInputs = kVeryLargeRuns;
...
/// Deterministic score for [recipe] combining shortage, chain value, and
/// agenda bias. Higher is more desirable. SPEC/ai/economy-planner.md
/// § Scoring.
double scoreRecipe({
  required ProductionRecipe recipe,
  required Stockpile stockpile,
...
/// Agenda: warmonger favours military-related; industrial_trader / merchant
/// favour trade goods.
