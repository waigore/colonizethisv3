...
// and `colonial_pressure_quota_met_futile_below_quota_gp_peace_branches_test.dart`
...
//      sorted ascending.
//   5. The delegating stubs in `colonial_pressure.dart` return the same
...
//      fixtures and the consumer paths agree on the deciders.
...
void main() {
  ...
  group('colonial_pressure delegation contract (Refs #2509 S1)', () {
    test('colonial_pressure.quotaMetBelowQuotaAtWarPeaceTargets mirrors the '
        'canonical helper across the branch table', () {
      ...
      // stub in a future slice cannot silently shift the quota-met
      // below-quota peace decider boundary.
      final cases = <(Game, AIWorldSnapshot, List<String>, String)>[
        // below-quota outer guard
        (
          _buildGame(
            provincesByOwner: const {_gpOwn: 9, _gpPartner: 5},
            players: const [
              Player(id: _gpOwn, displayName: 'GP_OWN', isHuman: false),
              Player(id: _gpPartner, displayName: 'P', isHuman: false),
            ],
        ...
        (
          ...
          const [_gpPartner],
          'own == quota, below-quota enemy → [enemy]',
        ),
        // at-war minor filtered
        (
          _buildGame(
            provincesByOwner: {
              _gpOwn: kObserverConquestMinOwProvincesPerGp + 2,
        ...
        (
          ...
          const [_gpThird],
          'quota-met enemy filtered → [below-quota enemy]',
        ),
        // sort determinism
        (
          _buildGame(
          ...
          const [_gpPartner, _gpThird],
          'reverse at-war order → sorted ascending',
        ),
      ];
      for (final (game, snapshot, expected, label) in cases) {
        final canonical = quotaMetBelowQuotaAtWarPeaceTargets(
          game: game,
          snapshot: snapshot,
        );
        final delegated = colonial_pressure.quotaMetBelowQuotaAtWarPeaceTargets(
          game: game,
          snapshot: snapshot,
        );
        expect(
          ...
          canonical,
          reason:
              '`colonial_pressure.quotaMetBelowQuotaAtWarPeaceTargets` '
              'is a thin delegating stub for legacy callers; it must '
              'mirror the canonical helper exactly so the legacy '
              'branch-table fixture and the consumer paths agree on '
              'the quota-met futile-bullying peace decision '
              '(fixture: $label).',
        );
        expect(
          canonical,
    ...
    test('colonial_pressure.quotaMetFutileBelowQuotaGpPeaceTargets mirrors the '
        'canonical helper across the branch table', () {
      final cases = <(Game, AIWorldSnapshot, List<String>, String)>[
        // below-quota outer guard
        (
        ...
        // at-quota fire path with minor-owned frontier
        (
          _buildGame(
            provincesByOwner: {
              _gpOwn: kObserverConquestMinOwProvincesPerGp,
          ...
          const [_gpPartner],
          'own == quota, below-quota off-frontier enemy → [enemy]',
        ),
        // frontier-owner skip
        (
          _buildGame(
          ...
          const [_gpThird],
          'frontier-owner skipped → [off-frontier enemy]',
        ),
        // sort determinism (multiple non-blocker enemies)
        (
          _buildGame(
            provincesByOwner: {
              _gpOwn: kObserverConquestMinOwProvincesPerGp + 2,
      ...
      for (final (game, snapshot, expected, label) in cases) {
        final canonical = quotaMetFutileBelowQuotaGpPeaceTargets(
          game: game,
          snapshot: snapshot,
        );
        ...
        expect(
          ...
          canonical,
          reason:
              '`colonial_pressure.quotaMetFutileBelowQuotaGpPeaceTargets` '
              'is a thin delegating stub for legacy callers; it must '
              'mirror the canonical helper exactly so the legacy '
              'branch-table fixture and the consumer paths agree on '
              'the narrower quota-met futile peace decision '
              '(fixture: $label).',
        );
        expect(
          canonical,
