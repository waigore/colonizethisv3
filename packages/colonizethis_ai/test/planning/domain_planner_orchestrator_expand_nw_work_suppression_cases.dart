  ...
  group('runDomainPlanners EXPAND-phase NW work filter', () {
    test(
      ...
      () {
        ...
        final view = buildPlayerView(game, topology, expandNwWorkSuppressionNationId);
        final snapshot = AIWorldSnapshot(
          ...
          colonial: const ColonialSummary(
            invadableNewWorldProvinceIdsSorted: [expandNwWorkSuppressionNwTribeProvince],
        ...
        final work = orders.workOrdersByPlayerId[expandNwWorkSuppressionNationId] ?? const [];
        expect(
          work.any(
            (w) =>
      ...
      () {
        ...
        final snapshot = AIWorldSnapshot(
          ...
          conquest: const ConquestSummary(
            ...
            invadableProvinceIdsSorted: [expandNwWorkSuppressionOwMinorProvince],
          ),
          colonial: const ColonialSummary(
            invadableNewWorldProvinceIdsSorted: [expandNwWorkSuppressionNwTribeProvince],
            adjacentNewWorldOwnerFactionIdsSorted: ['tribe1'],
        ...
        final armyMoves =
            orders.armyMoveOrdersByPlayerId[expandNwWorkSuppressionNationId] ?? const [];
        expect(armyMoves, isNotEmpty);
        expect(
          armyMoves.any(
            (m) => m.destinationProvinceId == expandNwWorkSuppressionNwTribeProvince,
          ),
          isFalse,
          reason:
        ...
        expect(
          armyMoves.any(
            (m) => m.destinationProvinceId == expandNwWorkSuppressionOwMinorProvince,
          ),
          isTrue,
    ...
    test('EXPAND filter outcome is deterministic for identical inputs', () {
      ...
      List<String> describe(Orders orders) =>
          (orders.workOrdersByPlayerId[expandNwWorkSuppressionNationId] ?? const [])
