...
// implementations live in `expand_phase_planner.dart`; `colonial_pressure.dart`
// previously retained thin delegating stubs for legacy callers (the
// `colonial_pressure_unwinnable_sole_gp_branches_test.dart` and
// `colonial_pressure_consolidate_gains_sole_gp_peace_branches_test.dart`
// fixtures and the `diplomacy_planner_peace_targets.dart` /
...
//   4. The delegating stubs in `colonial_pressure.dart` return the same
//      value as the canonical helpers for every relevant input — required
//      so the legacy `colonial_pressure_unwinnable_sole_gp_branches_test.dart`
//      and `colonial_pressure_consolidate_gains_sole_gp_peace_branches_test.dart`
//      fixtures and the in-file consumer paths agree on the deciders.
...
void main() {
  ...
  group('colonial_pressure delegation contract (Refs #2509 S1)', () {
    test(
      'colonial_pressure.unwinnableSoleGpFrontierPeaceTarget mirrors the canonical helper',
      () {
        // Pins the S1 delegation contract: the legacy public symbol
        // exported from `colonial_pressure.dart` must mirror the
        // canonical helper for every relevant fixture so removing the
        // stub in a future slice cannot silently shift the below-quota
        // outgunned peace decider boundary.
        final cases = <(Game, AIWorldSnapshot, String?, String)>[
          // sole-GP-null branch
          (
            _ownVsPartnerGame(
              ownProvinces: 6,
          ...
          (
            ...
            null,
            'at-quota short-circuit → null',
          ),
          // pivot-guard short-circuit (GP-only frontier, no minors)
          (
            _ownVsPartnerGame(ownProvinces: 6, partnerProvinces: 12),
            _ownSnapshot(
              oldWorldProvincesOwned: 6,
              atWarWith: const [_gpPartner],
              invadableProvinceIdsSorted: const ['oldWorld|${_gpPartner}_1'],
            ...
            null,
            'pivot-guard false (no minors anywhere) → null',
          ),
          // deficit-band: 9 OW non-GP-only, lead 1 → peace
          (
            _ownVsPartnerGame(
              ownProvinces: 9,
              partnerProvinces: 10,
            ...
            _gpPartner,
            '9 OW non-GP-only, lead 1 → peace',
          ),
          // deficit-band: 9 OW GP-only, lead 1 → null
          (
            _ownVsPartnerGame(
              ownProvinces: 9,
              partnerProvinces: 10,
            ...
            null,
            '9 OW GP-only, lead 1 → null (band uses kUnwinnableSoleGpMinProvinceDeficit)',
          ),
        ];
        for (final (game, snapshot, expected, label) in cases) {
          final canonical = unwinnableSoleGpFrontierPeaceTarget(
            game: game,
            snapshot: snapshot,
          );
          ...
          expect(
            ...
            canonical,
            reason:
                '`colonial_pressure.unwinnableSoleGpFrontierPeaceTarget` '
                'is a thin delegating stub for legacy callers; it must '
                'mirror the canonical helper exactly so the legacy '
                'branch-table fixture and the in-file consumer paths '
                'agree on the EXPAND-trap forced peace decision '
                '(fixture: $label).',
          );
          expect(
            canonical,
    ...
    test(
      'colonial_pressure.consolidateGainsSoleGpPeaceTarget mirrors the canonical helper',
      () {
        final cases = <(Game, AIWorldSnapshot, String?, String)>[
          // sole-GP-null branch (two GPs at war)
          (
            _ownVsPartnerGame(
              ownProvinces: kObserverConquestConsolidateMinOwProvinces,
              partnerProvinces: 1,
          ...
          // consolidate-min boundary: one short → null
          (
            _ownVsPartnerGame(
              ownProvinces: kObserverConquestConsolidateMinOwProvinces - 1,
              partnerProvinces: 1,
            ...
            null,
            'consolidate-min - 1 → null',
          ),
          // consolidate-min boundary: exact → enemy
          (
            _ownVsPartnerGame(
              ownProvinces: kObserverConquestConsolidateMinOwProvinces,
              partnerProvinces: 1,
            ...
            _gpPartner,
            'consolidate-min exact, huge lead → enemy',
          ),
          // lead boundary: one short → null
          (
            _ownVsPartnerGame(
              ownProvinces: kObserverConquestConsolidateMinOwProvinces,
              partnerProvinces:
            ...
            null,
            'lead == required - 1 → null',
          ),
          // lead boundary: exact → enemy
          (
            _ownVsPartnerGame(
              ownProvinces: kObserverConquestConsolidateMinOwProvinces,
              partnerProvinces:
        ...
        for (final (game, snapshot, expected, label) in cases) {
          final canonical = consolidateGainsSoleGpPeaceTarget(
            game: game,
            snapshot: snapshot,
          );
          final delegated = colonial_pressure.consolidateGainsSoleGpPeaceTarget(
            game: game,
            snapshot: snapshot,
          );
          expect(
            ...
            canonical,
            reason:
                '`colonial_pressure.consolidateGainsSoleGpPeaceTarget` '
                'is a thin delegating stub for legacy callers; it must '
                'mirror the canonical helper exactly (fixture: $label).',
          );
          expect(
            canonical,
            expected,
