...
void main() {
  ...
  group('observerGoalPhaseFor', () {
    test('expand when below observer OW quota', () {
      ...
      expect(observerGoalPhaseFor(snapshot: snap), ObserverGoalPhase.expand);
      expect(shouldSuppressNewWorldColonialOrders(snapshot: snap), isTrue);
    });
    ...
    test('colonial when at quota with acquisition targets', () {
      ...
      expect(observerGoalPhaseFor(snapshot: snap), ObserverGoalPhase.colonial);
      expect(shouldSuppressNewWorldColonialOrders(snapshot: snap), isFalse);
    });
    ...
    test('develop when at quota without colonial targets', () {
      ...
      expect(observerGoalPhaseFor(snapshot: snap), ObserverGoalPhase.develop);
  ...
  group('shouldFilterObserverPhaseWorkOrder', () {
    test(
      'flags purchase_land and build_improvement in newWorld during expand',
      () {
        const snap = AIWorldSnapshot(
          playerId: 'gp1',
          threats: ThreatSummary(),
  ...
  group('expandPhaseGpPeaceTargets', () {
    test('peaces non-blocker GP when two GPs at war in expand phase', () {
      ...
      expect(expandPhaseGpPeaceTargets(game: gameWithGp3, snapshot: snapshot), [
        'gp3',
      ]);
    });

    test('peaces every GP front when uninvaded minors remain below quota', () {
      final game = Game(
      ...
      expect(expandPhaseGpPeaceTargets(game: game, snapshot: snapshot), [
        'gp3',
      ]);
    });
  });
  ...
  group('COLONIAL personality colonial acquisition', () {
    test(
      'napoleon ranks declareWar above establishOverture henry reverses',
      () {
        final game = Game(
        ...
        expect(topTypeFor('napoleon'), DiplomaticOrderType.declareWar);
        expect(topTypeFor('henry'), DiplomaticOrderType.establishOverture);
      },
    );
  });
  ...
  group('developPhaseGpPeaceTargets', () {
    test('lists all at-war GPs in develop phase', () {
      ...
      expect(developPhaseGpPeaceTargets(game: game, snapshot: snapshot), [
        'gp2',
        'gp3',
      ]);
    });
