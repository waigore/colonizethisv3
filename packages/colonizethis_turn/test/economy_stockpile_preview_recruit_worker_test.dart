...
void main() {
  ...
  group('previewStockpileNetDeltaByCommodityForPlayer recruit worker '
    ...
    test('peasant recruit deducts fabric in pending build costs phase', () {
      final result = runRecruitWorkerPreview(
        ...
        orders: recruitWorkerOrdersFor(WorkerTier.peasant),
      );

      expect(result.delta[CommodityCatalog.fabric.id], -2);
      expect(
        result.phases[EconomyPreviewStockpilePhase
      ...
      expectPhaseDeltasSumToNet(
        game: result.game,
        playerId: 'p1',
        currentOrders: recruitWorkerOrdersFor(WorkerTier.peasant),
      );
    });

    test('apprentice recruit deducts paper and projects worker tier on the '
        'preview clone (treasury also debited)', () {
      final orders = recruitWorkerOrdersFor(WorkerTier.apprentice);
      final result = runRecruitWorkerPreview(
        player: economyPreviewRecruitWorkerPlayer(
      ...
      expect(result.delta[CommodityCatalog.paper.id], -2);
      expect(
        result.phases[EconomyPreviewStockpilePhase
      ...
      final updated = result.preview.playerById('p1')!;
    ...
    test('unaffordable recruit (no peasant) contributes no preview deltas', () {
      final result = runRecruitWorkerPreview(
        ...
        orders: recruitWorkerOrdersFor(WorkerTier.apprentice),
      );

      expect(result.delta, isEmpty);
      expect(
        result.phases[EconomyPreviewStockpilePhase.pendingBuildCosts],
        isEmpty,
      );
      final updated = result.preview.playerById('p1')!;
    ...
    test('recruit-worker order is applied before BuildUnitOrder in preview '
      ...
      final preview = applyEconomyPhasesForPreview(
        ...
        inputs: economyPreviewInputs(
          currentOrders: recruitThenBuildPeasantLeviesOrders(),
        ),
      );
    ...
    test('preview parity: sequential apprentice + peasant recruits deduct '
      ...
      final result = runRecruitWorkerPreview(
        ...
        orders: recruitWorkerOrdersFor(WorkerTier.apprentice, count: 2),
      );
      final updated = result.preview.playerById('p1')!;
      expect(updated.workerPool.peasants, 0);
      expect(updated.workerPool.apprentices, 1);
