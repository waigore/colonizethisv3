...
void main() {
  group('resource rules catalog defaults', () {
    group('defaultMarketPriceForCommodityId (Refs #3093)', () {
      ...
      test('returns the catalog base price for every manufactured commodity '
      ...
      test('manufactured base prices match the recipe input-cost subtotal', () {
        ...
        final int? timberPrice = rules.defaultMarketPriceForCommodityId(
          'timber',
        );
        ...
        final int? sugarCanePrice = rules.defaultMarketPriceForCommodityId(
          'sugarCane',
        );
        final int? tobaccoPrice = rules.defaultMarketPriceForCommodityId(
          'tobacco',
        );
        final int? fursPrice = rules.defaultMarketPriceForCommodityId('furs');
        final int? copperPrice = rules.defaultMarketPriceForCommodityId(
          'copper',
        );
        final int? tinPrice = rules.defaultMarketPriceForCommodityId('tin');
        final int? castIronPrice = rules.defaultMarketPriceForCommodityId(
          'castIron',
        );
      ...
      test('every non-riches catalog commodity resolves to a non-null, '
      ...
      test(
        'returns null for riches and spices (non-tradeable on the market)',
        () {
    ...
    group('manufacturedDefaultMarketPrice (Refs #3093)', () {
      test('exposes the nine tradeable manufactured commodity ids', () {
        final rules = ResourceRules.defaultRules;
        expect(rules.manufacturedDefaultMarketPrice.keys.toSet(), <String>{
          'lumber',
          'fabric',
          'castIron',
          'refinedSugar',
