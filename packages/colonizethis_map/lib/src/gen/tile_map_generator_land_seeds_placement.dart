...
///
/// Extracted from the former `tile_map_generator_land_seeds_placement_part`
/// `part` fragment so seed placement and the non-organic Voronoi assignment are
/// an independently importable, testable unit (see #3588).
library;
...
/// Per-continent seed-cluster placement (Pass 2) and the final non-organic
/// Voronoi land-budget assignment used by the seed-before-assignment path.
class LandSeedPlacement {
  const LandSeedPlacement._();
  ...
  static (List<(int x, int y)>, List<(int x, int y)>, List<int>) placeLandSeeds(
  ...
  static double _nextGaussian(Random rnd) {
  ...
  /// Per-continent land budget; assign to `kTileMapLandSentinel` by smallest
  /// effective distance (with optional Voronoi noise). Each cell at most one
  /// continent.
  static List<List<String>> assignLandByLandSeeds(
    TileMapLandSeedParams params,
