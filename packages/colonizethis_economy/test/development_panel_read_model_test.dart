...
typedef _DevelopmentPanelReadModelScenario = ({
  String label,
  void Function() run,
});

void _runOwnedProvinceListsImprovableGrain() {
...
void _runModelBuildsWithPendingWorkOrders() {
  expect(
...
void _runPlayerViewFiltersUnrevealedImprovableTiles() {
  final map = tileMapFromGrids(
    grid: const [
      ['p1', 'p1'],
    ],
...
void _runAssignedCiviliansIncludePendingBuilder() {
  final unit = Unit(
    id: 'b1',
...
List<_DevelopmentPanelReadModelScenario> developmentPanelReadModelScenarios() =>
    [
      (
        label: 'owned province lists improvable grain from Available parity',
        run: _runOwnedProvinceListsImprovableGrain,
      ),
      (
        label: 'model builds with pending work orders without throwing',
        run: _runModelBuildsWithPendingWorkOrders,
      ),
      (
        label: 'playerView filters unrevealed improvable tiles from counts',
        run: _runPlayerViewFiltersUnrevealedImprovableTiles,
      ),
      (
        label: 'assigned civilians include pending builder in region',
        run: _runAssignedCiviliansIncludePendingBuilder,
      ),
    ];
