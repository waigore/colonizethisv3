...
void main() {
  group('TradeOrderValidator.validate — empty / accept paths', () {
    ...
    test('single valid offer is accepted (stockpile covers, not riches, not '
        'mutually excluded)', () {
      final results = TradeOrderValidator.validate(
    ...
    test('single valid bid is accepted (within cargo, distinct-bid cap, not '
    ...
    test('second bid on same commodity does not consume a new cap slot '
    ...
    test('parallel result list is index-aligned to proposed orders', () {
      final results = TradeOrderValidator.validate(
        context: validatorCtx(),
        proposedOrders: [
  ...
  group('TradeOrderValidator.validate — rule 1: invalid quantity', () {
    ...
    test('invalidQuantity takes precedence over richesNotTradeable for a '
  ...
  group('TradeOrderValidator.validate — rule 2: riches not tradeable', () {
    test('every riches commodity id is rejected with richesNotTradeable '
        '(offer side)', () {
  ...
  group('TradeOrderValidator.validate — rule 3: mutual exclusion', () {
    test('commodity appearing as both bid and offer rejects both sides with '
        'mutualExclusion', () {
      final results = TradeOrderValidator.validate(
        context: validatorCtx(availableStockpileByCommodityId: {'timber': 50}),
        proposedOrders: [
    ...
    test('bid-on-A + offer-on-B is allowed (mutual exclusion is per-commodity, '
        'not per-player)', () {
      final results = TradeOrderValidator.validate(
