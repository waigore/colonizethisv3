...
void main() {
  group('AC #1 — owning-GP bid wins above priority tiers AND FTP', () {
    test('rival priority-1 bid loses to owning-GP priority-5 bid; rival '
    ...
    test('FTP-paired rival bid at same priority loses to owning GP; FTP '
    ...
    test('negative — owning GP does NOT bid: purchased-tile offer falls '
  ...
  group('AC #2 — relation 75 credits 10*20*0.75 = 150 treasury (full)', () {
    test('credits helper produces rate 0.75 + treasury 150.0 for gpA', () {
      final result = computeFirstRightCredits(
        ...
        purchasedTileIndex: _index([_attr(_tileK1, _gpA, _minorM1)]),
        relationScoreFor: (gp, src) => gp == _gpA && src == _minorM1 ? 75 : 0,
      );
  ...
  group('AC #4 — relation 0 credits 0 treasury (no overseas profit)', () {
    test('credits helper records audit row but transfers 0 treasury (Deal '
    ...
    test('negative — buyer == owning GP (D2 FRR-match path) excluded from '
