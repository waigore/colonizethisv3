...
Every land regiment belongs to one army, and an army occupies one province. Armies may contain mixed regiment types. In `UNIT20001`, expand an army to inspect its composition, split a non-empty group of regiments into a new army in the same province, or combine armies already stationed together.
...
- [ ] Lists the land regiment roster and its technology unlocks, including starting regiments and no-upgrade exceptions.
...
- [ ] Documents `MAP10001` army stack-marker taps (field Move vs Home-only roster) and `MAP20001` Military **Move** / **Invade** shortcuts with optional `DLG20002` army picker.
