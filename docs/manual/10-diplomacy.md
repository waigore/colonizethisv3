...
2. Choose an enabled action, set any required parameter, then confirm it. Most actions become pending orders and resolve in the Diplomacy phase before movement; use **Cancel** to withdraw a pending action. Invalid actions remain visible but disabled with their reason.
