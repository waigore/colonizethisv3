...
After you close turn news — or, when victory omits the newspaper, after you choose **View Final State** on `OVL20001` **Victory overlay** — `MAP10001` may briefly pulse a few places where last turn’s fights, captures, finished civilian work, or new sight landed. The camera moves to each place in turn; a short caption names what happened. Tap the map or **Skip** to end the sequence early. At most six places play this way; the full list stays in `OVL70001` **Player turn event feed**. Research, diplomacy, and market lines stay in news and the feed only.
...
- [ ] Documents last-turn spatial pulses on `MAP10001` after turn news (or after **View Final State** on victory), Skip, the six-beat cap, and that the feed keeps the full list.
