...
- **Old World** and **New World** each have their own land and water. Armies march only into a neighbouring province on the **same** map. Fleets sail from sea to sea and can touch a coast. The two maps meet **only** at the water with the yellow glow (paired sea zones). Ports and colonies reach home by sea paths **plus** those yellow-glow links — there is no land bridge.
- That yellow-glow water is the only passage between the two maps. Rest on it to read a line that the water is the passage to the other world. When **Show province names** is on, those sea names may also show a small passage icon to the **left** of the name, in addition to the yellow glow.
- Switch maps with the **Old World** / **New World** tabs on `MAP10001` **Empire overview / map area**. The small map in the bottom-right only moves what you see on the map you are already viewing. That same tab row also shows how many Old World provinces you hold out of 31 (Chapter 15).
...
2. `MAP20001` **Province sea-zone overlay** opens. The header reads **Province** or **Sea zone**. Political lists **Name**, **Owner**, standing with that court when your records know them (**At war** or **At peace**, plus **ALLIANCE** when you hold a formal treaty), **Sight**, **Region**, whether the place is a capital, then **Town development** (for example **2 of 4**) and a one-line note about town workshops.
3. On **Tile**, these rows stay visible: coordinates, **Terrain**, the town or capital line when this square is the town or capital (`The town of …` / `…, the capital of …`), resource, **Prospected**, **Improvement**, **Road / railroad** in plain words (**none**, **primitive road**, **improved road**, or **port or railroad**), and civilians (people you place on the map).
4. The default **Tile** rows already show that **Road / railroad** caption. Tap the transport line or **Tile details** to read **Port: None** / **Port: Present**, whether the tile still links to the capital (**Connected**), and **Extraction from this tile** (for example **2 of 4**) only when some of the yield reaches home. A tile that cannot extract still shows one default warning **Capital link: Not connected — will not extract** when it is not linked to the capital and would otherwise produce — you do not need **Tile details** for that warning, or for the road caption.
5. When a **Minor Nation** or **Tribe** (peoples you do not play) province bars exploration because you have no **Consulate** (a foothold with that court that lets you explore their land), Political also offers **Establish Consulate**. After you already hold a Consulate, Political offers **Establish Embassy** (a further foothold that lets you **Purchase land**) while that court is at peace and you do not yet hold an Embassy — so you can unlock **Purchase land** without leaving the map. Unavailable attempts explain what your court still lacks. When you are **At war** with the owner, Political also offers **Offer Peace** (same confirm as the Diplomacy list). A matching pending **Establish Consulate**, **Establish Embassy**, or **Offer Peace** changes the printed label to **Cancel**.
...
7. You can also right-click a tile (or press and hold on a touch screen) to open `MAP30001`, a small ring of nearby actions, without opening that panel first. The ring can show **Explore**, **Prospect**, **Build improvement**, **Build road**, **Purchase land**, **Upgrade town**, **Build port**, **Build railroad**, and **Build fort** when those shortcuts belong on the tile. When a ring or **More** action is blocked only because you have no matching civilian, the slot is an enabled **Train Explorer** / **Train Builder** / **Train Engineer** / **Train Merchant** / **Train Rail Builder**, not greyed. An **Explorer** is a civilian who reveals land and minerals; a **Builder** improves tiles; an **Engineer** builds roads, ports, and forts; a **Merchant** buys land; a **Rail Builder** upgrades roads to railroad. Shortcuts stay grey with a reason when you lack a Consulate, lack materials, have the wrong terrain, or hit another reason that is not “you have no such civilian.” At most five actions sit on the ring; the rest land under **More**. **More** always includes **Province details**. On a tight screen the ring is skipped and `MAP30002` **More tile actions** opens instead. `MAP20001` **Province sea-zone overlay** Tile also shows those **Train** labels, but still has older sentences that grey **Explore** / **Prospect** when you have no Explorer — those two sentences on that panel do not yet agree; until they match, treat the ring and **More** **Train** labels as the map shortcut, and do not treat a grey Explore on `MAP20001` as the last word.
...
On `MAP10001` **Empire overview / map area**, rest the pointer on a tile (without tapping) to read a compact panel: the **place name**, **who holds it** (`Owner:` or Unclaimed; sea tiles say they are a sea zone), and **how much your court can see** — Fully visible, Fogged — terrain only, or Unknown — no intel yet. On yellow-glow water, the panel adds a line that the water is the passage to the other world. The panel vanishes when the pointer leaves. While you are choosing a tile for civilian work, only that choose-a-tile message remains. On a touch device, tap the tile and read **Sight** on `MAP20001` **Province sea-zone overlay** Political (the first tab). Ownership on this readout stays true even on black unknown land; it does not name hidden terrain or resources.

After you confirm **Next turn** and turn news closes (or after **View Final State** on `OVL20001` **Victory overlay**, including **Campaign complete**), `MAP10001` may briefly highlight a few last-turn fights, captures, finished works, or discoveries so you see where the realm changed; Chapter 14 covers skip and the full feed.

### Terrain, resources, and what is hidden

Tiles carry **terrain** and optional **resources**. Goods only reach your **stockpile** (the goods stored at your capital) where you own the land, have improved it, still have a path home, and your court can take that yield (Chapters 6–7). Map rules matter:

- Some goods are **Old World** only (e.g. grain, meat, wool, horses), some **New World** only (e.g. sugar cane, tobacco, cotton, furs, spices, precious metals/gems), some **both** (e.g. timber, iron, copper, tin, coal).
...
Open **Map display options** (gear, `MAP10001`) to name what is painted. There are seven printed switches: **Show resources**, **Show improvements**, **Show roads and rails**, **Show province and sea borders**, **Show province names**, **Show province ownership**, and **Highlight land not bound to the capital**. All default **ON** except **Show province ownership**, which defaults **OFF**. **Show resources** paints resource icons and gold and brown discs. **Show improvements** paints `1 of 1` / `1 of 2` marks on your improved tiles. **Show roads and rails** paints roads and rails. Turning improvements off also turns roads off and greys that switch until improvements are on again. A shortcut button at the bottom-left of the map cycles four combinations (terrain only → resources → resources and improvements → all three) and jumps to terrain only from any other mix. Rest on it to read the current combination. Those choices persist when you save.
...
**Warning.** The yellow-glow water is the only passage between the two maps. Fleets and overseas ports can fail for reasons that look like empty ocean when that passage is missing or blocked.

## The other courts

Rival courts also prefer land that still links to their capital when they assign civilian work, and they push roads, ports, and claims along the same neighbouring provinces and sea paths you see. Rival **Builders** prefer certain goods tiles — wool and cotton, or timber, iron, and coal — rather than treating every goods tile the same. They are not playing a different map.
...
- [ ] Explains that Old World and New World are separate maps; armies march only to a neighbouring province on the same map; fleets sail sea to sea and can touch a coast; the maps meet only at yellow-glow water (passage between the two maps; optional sea-name icon to the left when **Show province names** is on).
- [ ] Documents map selection → `MAP20001` **Province sea-zone overlay** (header **Province** or **Sea zone**; Political **Town development** `N of 4` and town-workshops line) and points to town/port icons and land art under ownership colour.
- [ ] Documents `MAP30001` right-click / press-and-hold tile acts (**Train {type}** when the only block is no matching civilian; greyed-with-reason for Consulate / materials / terrain / other non-unit reasons; overlay Train-vs-grey sentences recorded without picking a winner) and `MAP30002` **More tile actions** **Province details** (ring skipped on a tight screen).
...
- [ ] Documents the default-on map highlight for owned land not bound to the capital (does not hatch unrevealed land).
- [ ] Explains all seven Map display options switches (defaults: all ON except **Show province ownership** OFF), the improvements-off → roads-off lock, and the bottom-left shortcut that cycles four combinations.
- [ ] Explains **1 of 1** / **1 of 2** improvement marks (muted-at-limit vs this court can still raise; not a Builder-ready cue) and the on-map improvement key.
...
- [ ] Documents pending **Establish Consulate** / **Establish Embassy** / **Offer Peace** becoming **Cancel**.
