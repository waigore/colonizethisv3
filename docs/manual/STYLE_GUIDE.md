...
Do not assume the reader already knows those terms. Counsel callouts may still use the existing archaic register; the meaning must still pass the reading-level test.

### Banned classes (not a closed list)

Treat these as **classes**. The examples show the class; any word in the same class is banned even if it is not listed. They are **not** banned in `## Sources` path bullets (those are machine-parseable paths, not player vocabulary).
...
Chapter 1 is the worked example. Use the **player wording** in body prose:
