...
7. **Grant Aid** requires an Embassy and sufficient treasury. Open **Grant or subsidy dialog**, choose a positive amount in £1000 steps (minimum £1000), and confirm the one-time transfer. It improves relations when resolved.
8. **Set Subsidy** uses the same **Grant or subsidy dialog** dialog. It is available only toward a Minor Nation or Tribe with an Embassy (not toward another Great Power). Choose 5%, 10%, 15%, or 20%; it has no upfront or per-turn treasury payment, but affects market terms and relations while active.
