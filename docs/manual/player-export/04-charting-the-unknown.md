...
2. The work is **free** and lasts **one** turn. Before you **Assign**, enabled **Prospect** shows `After this work: any mineral on this tile becomes known · Takes N turn(s)` (or **Takes 1 turn**). The line does not name a hidden good and does not promise that a deposit is there. The mineral is known to you only when that work **finishes**, after you confirm **Next turn**. Prospect-required minerals (iron, copper, tin, coal, silver, gold, gems, diamonds) stay hidden until then. After **Next turn**, **Player turn event feed** shows `{province} work completed! Prospect found {displayName}` or `{province} work completed! Prospect found no mineral`. Last-turn highlight captions on **Empire overview / map area** use the same survey line. Rival courts’ surveys do not appear on your feed.
