...
8. Tap **Breakdown** to open **Production commodity breakdown dialog**. The title is **Commodity breakdown**. Columns are **Commodity**, **Pending build costs**, **Extraction**, **Riches to treasury**, **Consumption**, **Production**, and **Total**, grouped under Food / Raw materials / Manufactured. Tap **Close** to dismiss. Changing Allocation or **Reset** refreshes an open dialog. The **Total** matches the Available **+N** / **−N** (or hides the change when the net is zero).
9. **Allocation** (right on a wide screen, below **Available** on a phone): one row per recipe. Set **desired output** with the slider or − / + / maximize / clear. Each unlocked row states how many whole batches you can still ask for this turn and what limits that cap (for example, “Up to 12, limited by Timber”). When the limit is a raw good (not labour and not the panel’s per-turn ceiling), tap that line to open **Development screen** already focused on that good so you can raise extraction. When the line says labour this turn is the limit, tap it to bring **Labour Controls** into view. Rest on that line (or press and hold) to read that it shows Labour Controls and that new workers staff next Production. Tapping it does not hire anyone. Locked recipes show `(locked)` and omit the cap line. Hover or long-press the cap line for a note that the number is what remains after other recipes, not your whole warehouse. Labour warnings appear when idle labour or materials cannot cover the ask.
10. After you confirm **Next turn**, the game tries to make that many whole batches from the warehouse and from workers who can work that turn. Tap **Reset** to clear every recipe. While the game is carrying out the turn, do not edit Allocation, **Reset**, **Breakdown**, or **Labour Controls**. You may still open **Counsel screen** to read advice; Apply / Agree stay hidden until edits are allowed again.

### Industry Counsel (**Counsel screen**)

1. From Production Allocation, tap **Counsel** in the header or a starred row’s counsel star to open **Counsel screen** on the Industry tab. Counsel also has **Military** and **Development** tabs; Industry is the tab you land on from Production.
...
5. On a missing-input card, tap **Open Development** to open **Development screen** already focused on that missing good (and land when Counsel knows a tile). That tap does not assign improve work.
6. Empty counsel shows **No pressing industry advice this turn.**

The same **Counsel screen** also hosts a **Trade** tab when opened from **Trade screen** — see Chapter 8 for Apply/Agree on trade advice.
