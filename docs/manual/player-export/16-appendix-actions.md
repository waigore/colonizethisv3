...
| `counter_spy` — Spy | Free; owned province. **Province sea-zone overlay** on Civilian opens Spy-only assignment for that province when eligible (protects the whole realm; one Spy is enough). | Ongoing empire-wide counter-espionage effect; spy checks occur in **Spy resolution**. |
