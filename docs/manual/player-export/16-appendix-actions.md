...
| Fleet mission | **Map:** tap human fleet marker (**Empire overview / map area**) → **Naval mission fleet picker dialog** (if needed) → **Naval mission menu dialog** → **Naval mission target dialog** for Blockade/Beachhead targets (rows show fog-honest defender or harbor intel). **Panel:** **Naval units panel** → **Mission** on an at-sea sea-going fleet (same flow). **Cancel pending mission** clears a staged mission. | Free. Patrol/Blockade/Beachhead/Defend require a sea-going fleet at sea; Blockade/Beachhead need an adjacent at-war province target. A fleet moves or takes one mission, never both. Join Home Fleet: dock at capital or **Transfer to home fleet** transfer — not the mission menu. | Assignment: **Movement**; interceptions: **Naval Interception & Naval Combat**. | Ch. 13 |
