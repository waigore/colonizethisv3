...
| Army move / invasion | **Military units panel** → **Move** on a non-Home Army → **Move army dialog** → destination; or **Province sea-zone overlay** Military **Move** / **Invade** (optional **Overlay army move picker** when several armies qualify — rows show each army’s regiment mix; Invade preselects the viewed province). Invasion confirmation may add Declare War. | Free. Home Army cannot leave capital; own destinations or adjacent valid foreign targets only; war must exist or be declared in the same draft. | **Diplomacy**, then **Movement**; battle follows if contested. | Chs. 11–12 |
