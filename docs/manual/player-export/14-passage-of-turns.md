...
2. Glance at the tab bar coin: current gold and a signed **Next-turn** forecast (when it is not zero). Tap the coin for a short panel of the details: current gold, that same forecast, and any **already committed** gold lines that are not zero (research you are funding, market bids, grants, overtures, recruit/train workers, train units, purchase land). Exact and Compact number formats live inside that panel. Empty research seats and funding **None** do not appear; Spies do not appear. The forecast also folds in extraction, riches turning to gold, market fills, and other Next-turn effects that are not listed as separate lines.
...
4. Tap **Next turn** in the top bar.
5. **Next turn confirmation** opens. The title is **End turn?** or **Proceed to next turn?** The body says **Turn {N} will end. Continue?**
6. If you already staged decrees this turn, the dialog shows **Staged this turn** with a short count by kind (army moves, trade, and so on). If you gave none, that list is simply absent — the court does not scold you for a quiet turn.
7. Tap **Review decrees** to read each staged line in ordinary words. A locate button beside each kind opens that panel or screen and closes the dialog **without** ending the turn, so you can still change your mind. **Yes** stays available while you read.
8. Five working civilians can do assigned tasks: Explorer, Builder, Engineer, Merchant, and Rail Builder. If any of yours have **no work order** for the next turn, the dialog may list them under **These civilians have no work order for the next turn:**. Each row shows type, location, **No work order**, and a go-to button.
9. Tap go-to on a listed civilian to leave without ending the turn. The map finds that person and opens **Civilian units panel**. A **Spy** is a civilian posted in a foreign court or at home; Spies are **not** listed here — foreign station, home counter-spy, and capital reserve are deliberate posts, not wasted capacity.
10. To stop seeing that no-work-order list, turn on **Don't show this warning again** and then tap **Yes**. Tapping **No** leaves the warning on. You can also change the same warning in **Settings** with **Warn when civilians have no work order on end turn**; that switch applies on the next end-turn confirm, without restarting the app.
...
A completed turn’s main report appears in **Turn news dialog**, unless victory takes precedence. The world gazette lists captures, wars, peace, overtures, and discoveries. When your own court had outcomes last turn — a refused decree, finished research, a battle you fought, market or realm accounts, or completed work — a **Your court** line names them in plain language even when the world report is empty. Tap **open Events** on that line to close turn news and show **Player turn event feed** on the map; **Close** alone does not open the feed. If nothing major happened in the world and your court was quiet too, the dialog says **No major events last turn.** Close the report, inspect the map and your empire screens, and prepare the next turn.
...
4. Completed work orders open **Civilian units panel** focused on the person who finished the task. When **your** Explorer finishes **Prospect**, that line names the mineral found (Iron, Coal, and the other printed goods names) or that the tile has no mineral. When **your** Explorer finishes **Explore**, that line says the province is now fully visible. Finished **Build improvement**, **Upgrade town**, **Build road**, **Build port**, **Build railroad**, **Build fort**, and **Purchase land** name what changed (goods if still linked, workshops, a port or railroad, siege posture in ordinary words, or purchase privileges) instead of only saying the job finished. Rival courts’ surveys stay off this list.
