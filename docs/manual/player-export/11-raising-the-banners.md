...
1. **From the map:** Open **Province sea-zone overlay** on a province. On the **Military** section, tap **Move** when a non-Home field army is stationed there, or **Invade** on a foreign province when a field army can conceivably reach it. If several armies qualify, **Overlay army move picker** asks which army acts; then **Move army dialog** opens. **Invade** preselects that province so you need not hunt it again.
