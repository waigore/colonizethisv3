...
1. On **Game screen**, tap the **Technology** icon on the left of the map. That opens **Technology screen**.
...
To station a Spy in a rival Great Power’s land: select a rival land tile → on **Province sea-zone overlay**, Civilian **Station spy** → on **Civilian units panel**, tap **Relocate** (that shortcut commits that tile), **or** use **Relocate** on the units panel then pick a tile. Before you commit, the overlay and shortcut panel tell you that a **rival Great Power** post may speed research (Minor and Tribe posts do not). While a Spy stays in a rival court, status shows **Holding intel: {province} — may speed research**; Minor and Tribe holds stay **Holding intel: {province}** only.

When a research seat is funded at **Low** or higher, and that rival has already unlocked the technology you are researching, the next **Next turn** adds about **+15% RP** per such rival (two rivals stack). On **Technology screen**, the green **+N RP** line already includes that bonus. Tap it for the breakdown; a **Spy insight** line names those courts (for example **Spy insight — France already knows this (+15%)**). The station-time gist does not name technologies or list which rivals qualify — open **Slots** after the Spy is in place for that detail.
