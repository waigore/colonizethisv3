...
1. Open `UNIT30001` **Naval units panel** from the left-side **Naval Units** icon, **or** open `MAP20001` **Province sea-zone overlay** on your capital. An empty Home Fleet harbor marker on the map still opens `UNIT30001`.
2. Select **Train** (header on `UNIT30001`, or Naval on the capital overlay) to open `UNIT60001` **Train naval dialog** (header **Train Naval**). The overlay path stays on the map. A short line under overlay **Train** says new ships join the Home Fleet here after **Next turn** and that the tap does not detach a squadron or assign a sail or mission. Overlay **Train** appears only on your own capital, not on other ports you own, not on a rival capital, and not when you are only watching.
...
- [ ] Documents opening `UNIT30001` **Naval units panel** from **Naval Units** on the left of the map, or `MAP20001` **Naval** **Train** on the capital overlay, then `UNIT60001` **Train naval dialog** for building ships into the Home Fleet, including treasury, materials, Peasant, and technology constraints. Overlay **Train** stays on the map and is capital-only. Results appear after **Next turn**.
