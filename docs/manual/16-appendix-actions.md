...
| Build civilian | `UNIT10001` **Civilian units panel** → **Train** → `UNIT40001` **Train civilians**, or map **Train Explorer** / **Train Builder** / **Train Engineer** / **Train Merchant** / **Train Rail Builder** on `MAP20001` / `MAP30001` / `MAP30002` when that work shortcut is greyed out only because you own none of that civilian (each row shows a short line under the type name that says what that civilian does). | Treasury + paper; Merchant/Rail Builder need their unlocks; capital tile required. Map Train does not assign Explore / Prospect / Build on the selected tile. | While work finishes after Next turn; unit appears at capital | Ch. 5 |
| Build regiment | `UNIT20001` **Military units panel** → **Train** → `UNIT50001` **Train Military**, or `MAP20001` **Province sea-zone overlay** Military **Train** on your capital (the overlay stays open; a short line says new regiments join the Home Army here after Next turn and that the tap does not split a field army or start a march). Each row shows the regiment’s category, a short combat-role line, food upkeep per turn, and build costs. | Treasury, materials, one reserved peasant, and regiment technology. | While work finishes after Next turn; joins Home Army | Ch. 11 |
| Build ship | `UNIT30001` **Naval units panel** → **Train** → `UNIT60001` **Train Naval**, or `MAP20001` **Province sea-zone overlay** Naval **Train** on your capital (the overlay stays open; a short line says new ships join the Home Fleet here after Next turn and that the tap does not detach a squadron or assign a sail or mission). Each row shows role, cargo or combat gist, food upkeep per turn, and build costs. | Treasury, materials, one reserved peasant, and ship technology where required. | While work finishes after Next turn; joins Home Fleet | Ch. 13 |
| Move fleet | **Map:** tap in-port sea-going fleet marker (`MAP10001`) → `DLG31004` then `DLG30001` when that fleet is at the capital and Transfer is also offered, else `DLG30001` at once; or at-sea marker → `DLG31001` **Sail / Move** → `DLG30001`. **Overlay:** `MAP20001` Naval **Sail / Move** on a revealed sea or owned port you occupy with a sea-going fleet (not the Home Fleet) → optional `DLG31003` → `DLG30001` (skips the mission menu and capital chooser). **Home Fleet with ships:** harbor marker or `MAP20001` **Detach and sail** → **Detach a squadron** → `DLG30001` for the new fleet. **Panel:** `UNIT30001` **Naval units panel** → **Move** on a sea-going fleet → `DLG30001` **Move Fleet** → legal destination (sea rows may show **Hostile patrol** / **Hostile blockade** / **Hostile fleets: N**, or **Fleets unknown** when you cannot see that water). | Free. Home Fleet cannot move; only one adjacent sea/port step; ports must be yours. Replaces that fleet’s pending mission. | After Next turn, during movement | Ch. 13 |
| Assign fleet mission | **Map:** tap at-sea human fleet marker (`MAP10001`) → `DLG31003` (if needed; rows show ship mix and any pending mission) → `DLG31001` **Assign mission** → `DLG31002` for Blockade/Beachhead targets (rows show a harbor line — port or empty harbor, or harbor unknown when you lack intel — and the Blockade caption that names the warehouse / capital-link cut, plus an extra line when the target is a capital port). Home Fleet with ships detaches then sails (see Move fleet); empty Home Fleet marker opens `UNIT30001`; in-port sea-going marker opens Move. **Province panel:** `MAP20001` **Province sea-zone overlay** Naval **Blockade** / **Beachhead** on a foreign coast at war (skip `DLG31001`; `DLG31003` when several fleets qualify; target dialog preselects the viewed province). **Sea panel:** same overlay on a revealed sea you occupy with a sea-going fleet offers **Patrol** / **Defend** (skip `DLG31001` and the target dialog; `DLG31003` when several fleets qualify; a short muted line under each stay-mission button names intercept vs hold before you tap, including when the buttons cannot be tapped). Owned blockaded ports show **Under blockade** on **Naval**; **Tile details** names blockade when an owned tile is not linked to the capital. **Panel:** `UNIT30001` → **Mission** on an at-sea sea-going fleet (same flow as the map marker). **Cancel pending mission** clears a staged mission. | Free. Patrol/Blockade/Beachhead/Defend require a sea-going fleet at sea; Blockade/Beachhead need an adjacent at-war province target. A fleet moves or takes one mission, never both. Join Home Fleet: dock at capital or `DLG40001` transfer — not the mission menu. | Assignment after Next turn during movement; interceptions during naval combat. Blockade also cuts that port’s link to its capital. | Ch. 13 |
...
|---|---|---|
| Explore — Explorer | Free; province must be partly revealed; on Minor/Tribe land with no Consulate+, `MAP20001` **Explore with explorer** and **Prospect with explorer** show visible but disabled with **Establish a consulate before exploring or prospecting** (see Diplomacy **Establish Consulate**). **`MAP30001` Explore** (right-click or press-and-hold) opens the same Explorer shortcut as `MAP20001`. Enabled **Explore** shows that the whole province becomes Fully visible when the work finishes, and how many turns that takes, before you **Assign**. | Takes up to three turns, then reveals the province. After Next turn, `OVL70001` **Player turn event feed** says the province is now fully visible. |
| Prospect — Explorer | Free; swamp, hills, mountain, or desert tiles that are visible and not yet prospected. **`MAP30001` Prospect** opens the same Explorer shortcut as `MAP20001`. Enabled **Prospect** shows that any mineral on this tile becomes known when the work finishes, and that it takes one turn, before you **Assign**. | One-turn completion marks the tile prospected. After Next turn, `OVL70001` **Player turn event feed** and the last-turn pulse name the mineral found or that the tile has no mineral. |
| Build improvement — Builder | Lumber + cast iron; the tile must have a resource; if the resource is a mineral, the tile must already be prospected; the next level must still be allowed by terrain and your technologies. **`MAP30001` Build improvement** opens the same Builder shortcut as `MAP20001`. Enabled controls show what the tile will send after the work (next-level goods if still linked, or that the road or town development is already the limit, or still nothing if not bound to the capital). From `GAME20001` **Production screen**, tap a recipe’s missing-input line when it names a raw good to open `GAME80001` **Development screen** already focused on that good; from Counsel **Open Development** on a missing-input card, the same focus. Tap **Assign** on Development to stage the work. | Raises improvement level. After Next turn, the Events feed names the goods in ordinary words if they still arrive, or that the road or town still limits them, or that the tile is still unbound. |
| Upgrade town — Builder | **National Bureaucracy**, materials, and the province town tile; town level cap 4. On your own towns, or on Minor/Tribe town tiles when at peace with **embassy-tier** overture; completing foreign work does not change ownership. **`MAP20001` Upgrade town** on the Political Town development row (and **`MAP30001` Upgrade town** when the ring opens on that town tile) opens Builder-only assignment for your owned province town tile when eligible (Assign from the panel can target embassy foreign towns). Enabled **Upgrade town** names whether workshops start, pause until level 4, or resume at double the level-2 rate, and that the work takes one turn. | Raises town development. After Next turn, the Events feed uses that same start / pause / resume wording for the new town level. |
| Build road — Engineer | Lumber + metal; terrain/road technology limits. **`MAP20001` Build road** and **`MAP30001` Build road** open Engineer-only assignment for the selected tile when eligible. Enabled controls show a payoff gist beside the cost. | Raises transport level. After Next turn, the Events feed says the tile is bound to the capital (or names the matching transport payoff). |
| Build port — Engineer | Lumber + metal; coastal town or river tile that can take that province’s seaboard port (human-owned land cardinally adjacent to a sea zone whose seaboard has no port yet). **`MAP20001` Build port** and **`MAP30001` Build port** open Engineer-only assignment for the selected coastal tile when eligible; Port status (**None** / **Present**) is under **Tile details**. `GAME80001` **Development screen** → header **Counsel** → `GAME90001` **Counsel screen** **Development** tab → **Agree** (when the courts would dig a port this turn). Enabled controls show a payoff gist. | Creates port and transport level 4. After Next turn, the Events feed says this coast now has a port. |
| Build fort — Engineer | Materials, town tile, and fort technology where required. **`MAP20001` Build fort** and **`MAP30001` Build fort** open Engineer-only assignment for the selected town tile when eligible. Enabled controls show the next siege posture and work duration before you assign. | Raises fort level. After Next turn, the Events feed names the new siege posture in ordinary words (open field, wood, stone, or modern). |
| Build railroad — Rail Builder | Steel ×2 + lumber ×2; road level 1–2, known terrain, matching rail technology. **`MAP20001` Build railroad** and **`MAP30001` Build railroad** open Rail-Builder-only assignment for the selected tile when eligible (not on transport 0 or 4). Enabled controls show a payoff gist beside the cost. | Sets railroad transport level 4. After Next turn, the Events feed says a railroad now stands on that tile. |
...
| Purchase land — Merchant | Embassy, peace, tile in a **Minor or Tribe** province that has a resource (not Great Power land, not your own); if the resource is a mineral, the tile must already be prospected; treasury ≥ 15× base price; not already purchased by any Great Power. **`MAP20001` Purchase land** and **`MAP30001` Purchase land** open Merchant-only assignment for the selected tile when eligible. When Embassy is missing, the disabled hint names Embassy; on a narrow screen it also points to Political **Establish Embassy**. Enabled controls show the payoff gist (first bid / overseas gold on tradeable sales, or riches into your treasury; land stays the court’s) beside the £ cost. | One-turn completion debits treasury and records the tile purchase. Privileges start when that work finishes. After Next turn, the Events feed names those privileges and does not say the land changed owner. |

### Diplomacy

Most begin at `GAME30001` **Diplomacy screen** → select a court (which opens `GAME30002` **Diplomacy detail screen**, or use list-row actions on `GAME30001`) → choose a printed action and confirm, then resolve after Next turn during diplomacy. The first Consulate stage also has a focused entry at `MAP20001` Political when that missing relationship blocks Explore or Prospect. **Establish Embassy** is also on Political once a Consulate is held and Embassy is not, while you are at peace with that Minor Nation or Tribe. After Embassy, Political also offers **Grant Aid** and **Set Subsidy** toward that Minor Nation or Tribe at peace; those open `DIPL20001` from the map. **Offer Peace** is also available on `MAP20001` Political when the province owner is at war with you.
...
| Establish Overture | `GAME30001`, or `MAP20001` Political **Establish Consulate** for a Consulate-gated Minor/Tribe province, or **Establish Embassy** on Political once Consulate is held and Embassy is not (at peace). One stage at a time; relation, treasury, target, and technology gates apply. The confirm names first-order unlocks for that stage (Consulate: Explore/Prospect and market fill preference toward Minors/Tribes; Embassy: Grant Aid, subsidies, land purchase, and possible intervention; NAP: later Join Empire offer and no automatic war ban; **Join Empire**: Minor/GP absorption of land, armies, and fleets with the court leaving the map; Tribe colony that keeps its own land and does not add to the 31 Old World victory count). Stages: Trade Consulate → Embassy → Non-Aggression Pact → **Join Empire** (last stage — Great Power: Empire Building tech, nearly defeated target, no treasury charge; Minor/Tribe: single £ cost). | Target accepts/rejects after Next turn during diplomacy; pending Consulate or Embassy may be cancelled from the same map control. History and Intelligence describe Tribe Join Empire as a colony, not absorption. |
| Grant Aid | Embassy; positive £1,000 steps; sufficient treasury. `GAME30001` row, or `MAP20001` Political after Embassy on that Minor/Tribe land at peace. `DIPL20001` **Grant or subsidy dialog** — **Submit** stages the pending transfer (Cost / Effect shown there; no second confirmation). Effect says a larger gift this turn does not improve standing further, and names whether the standing word stays or becomes. | Transfer after Next turn during diplomacy; standing change does not grow with the £ amount. |
| Set Subsidy | Embassy; Minor/Tribe only; 5–20% in steps of 5. `GAME30001` row, or `MAP20001` Political after Embassy. `DIPL20001` **Submit** stages the pending subsidy (no second confirmation). Cost / Effect name pay-more / receive-less percents on fills with that court. | No per-turn gold charge; those percents apply only to fills with that court. |
...
| Split / combine armies | `UNIT20001` **Military units panel** → **Split Army**, or select same-province armies → **Combine**. `MAP20001` Military **Combine** merges every army of yours in the viewed province after confirm. Map/province-panel detach-then-move also opens Split Army (**Detach a field army**) before `DLG20001`. | Immediate; split needs a non-empty regiment subset; combine requires same province. |
| Split / combine fleets | `UNIT30001` **Naval units panel** → **Split**, or select fleets sharing one port/sea zone → **Combine** (Home Fleet + one eligible source may open `DLG40001` subset transfer). `MAP20001` Naval **Combine** merges every human fleet in the viewed port or sea after confirm and never opens `DLG40001`. Map **Detach and sail** also opens Home Fleet split (**Detach a squadron**) before `DLG30001`. Home Fleet split shows remaining cargo holds versus this turn’s overseas load. | Immediate; non-Home split retains one ship; Home Fleet survives empty. |
| Transfer to Home Fleet | `UNIT30001` **Naval units panel** → eligible fleet **Transfer to Home Fleet**, or `MAP20001` **Naval** **Transfer to Home Fleet** on the owned capital (fleet in that harbor) or on a sea next to the capital (fleet at sea) → `DLG40001` → **Transfer**. Several overlay sources open a fleet picker first. | Immediate; selected hulls move home; the fleet must be in port at the capital or at sea beside the capital. A line shows remaining Home Fleet holds, this turn’s overseas load, and holds free for trade bids. |
