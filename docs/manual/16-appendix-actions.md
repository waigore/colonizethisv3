...
| Build improvement — Builder | Lumber + cast iron; the tile must have a resource; if the resource is a mineral, the tile must already be prospected; the next level must still be allowed by terrain and your technologies. **`MAP30001` Build improvement** opens the same Builder shortcut as `MAP20001`. Enabled controls show what the tile will send after the work (next-level goods if still linked, or that the road or town development is already the limit, or still nothing if not bound to the capital). | Raises improvement level. |
