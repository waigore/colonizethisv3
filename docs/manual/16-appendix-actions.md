...
| `build_port` — Engineer | Lumber + metal; valid coastal/river tile. **`MAP20001` Build port** shortcut on the Road / transport row opens Engineer-only assignment for the selected coastal tile when eligible; Port status reads None or Present. | Creates port and transport level 4. |
