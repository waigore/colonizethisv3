...
| Army move / invasion (`ArmyMoveOrder`) | **Map:** tap your army stack marker on `MAP10001` (town-tile bottom-right) → `DLG20001` (or `DLG20002` when several field armies share the tile — rows show each army’s regiment mix). Home-Army-only marker opens `UNIT20001`. **Overlay:** `MAP20001` Military **Move** / **Invade** (optional `DLG20002`; Invade preselects the viewed province). **Panel:** `UNIT20001` → **Move** on a non-Home Army → `DLG20001`. Invasion confirmation may add Declare War. | Free. Home Army cannot leave capital; own destinations or adjacent valid foreign targets only; war must exist or be declared in the same draft. | **Diplomacy**, then **Movement**; battle follows if contested. | Chs. 11–12 |
