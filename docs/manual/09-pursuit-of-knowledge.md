...
When a research seat is funded at **Low** or higher, and that rival has already unlocked the technology you are researching, the next **Next turn** adds about **+15% RP** per such rival (two rivals stack). On `GAME40001` **Slots**, the green **+N RP** line already includes that bonus. Tap it for the breakdown; a **Spy insight** line names those courts (for example **Spy insight — France already knows this (+15%)**). The station-time gist does not name technologies or list which rivals qualify — open **Slots** after the Spy is in place for that detail.
