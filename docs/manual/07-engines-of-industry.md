...
5. On a missing-input card, tap **Open Development** to open `GAME80001` **Development screen** already focused on that missing good (and land when Counsel knows a tile). That tap does not assign improve work.
...
- [x] Documents `GAME20001` / `PROD20001` flows: Available (sellable amount for tradeable goods, tap opens Trade Market), labour readiness, Allocation, desired output, bottleneck (commodity line opens Development; labour line focuses Labour Controls), Reset, Breakdown columns.
