...
Civilians are map units. Open `UNIT10001` **Civilian units panel** from the left empire rail, then **Train** to open `UNIT40001` **Train civilians dialog**. Each row shows a short muted role line under the type name (explore/prospect, improve/upgrade town, roads/ports/forts, foreign intel and home counter-espionage, purchase land, upgrade roads to railroad) so you can tell the six types apart before you commit treasury and paper.
